...
**Current release: v0.12.0** (2026-09-21), published on Docker Hub as `gcorbaz/kesh` (`:0.12.0` and `:latest`) and **running on the author's NAS** (confirmé par Guy le 2026-09-21). **214 stories** delivered ; **22 epics** have no open story, **three remain open** (13, 15, 25) ; **69 migrations**, **2401 backend tests** and **745 frontend tests**.

*(Recomptés depuis la source le 2026-09-21, à la publication de la v0.12.0 — pas repris du paragraphe précédent, qui annonçait **v0.11.1**, « 156 stories », « 23 closed epics », 61 migrations, 2219 et 733 tests : **tout avait bougé**. Commandes : `ls crates/kesh-db/migrations/*.sql | wc -l`, les totaux de `cargo nextest run` et de `npm run test:unit`, et un décompte par statut de `sprint-status.yaml` — les 214 ne comptent que `done` ; s'y ajoutent 12 `split`, 4 `superseded`, 4 `archived-split*`, 12 `backlog` et 2 `ready-for-dev`, qui ne sont pas des livraisons. ⚠️ **« 23 closed epics » n'était pas recomptable** : le registre ne porte pas de statut d'epic, seulement des statuts de story. La grandeur qui se compte est « epic sans aucune story ouverte », et c'est celle qui est écrite ici.)*

⚠️ **La v0.12.0 corrige le défaut fondateur du produit** — l'encaissement d'une facture ne produisait aucune écriture comptable — et referme les gardes qui rendaient les livres contestables : contre-passation, gel, verrou de période, numérotation des écritures par compteur, piste de contrôle inaltérable et consultable par l'API. ⛔ **Et elle n'est toujours pas destinée à tenir une comptabilité réelle**, ce que son CHANGELOG, la feuille de route et le message de son tag disent en toutes lettres : plusieurs fonctions d'un exercice complet manquent, et **l'export de sauvegarde était incomplet ([#386])** — corrigé depuis sur `main` (Story 25-5-a, PR #452, revue a posteriori le 2026-09-26), **à paraître en v0.12.1** : la v0.12.0 publiée garde le défaut. *Publiée n'est pas utilisable, et c'est la distinction suivante.*

⚠️ **Le CHANGELOG ne contenait RIEN de l'Epic 24 jusqu'au 2026-09-21** — sept stories, dont la correction du défaut fondateur —, ni trois stories de l'Epic 25. Neuf livraisons mergées pendant un mois sans jamais entrer dans les notes de version, et **aucun gate ne le signalait**. Écrit à la préparation de la release. *Un epic clos n'est pas un epic publié.*
