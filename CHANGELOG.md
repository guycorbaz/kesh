...
- **Vos coordonnées de contact sur le PDF de facture** : téléphone, e-mail et site web apparaissent désormais sous votre adresse, dans le bloc en haut à gauche.
  ...
  - **Si l'en-tête entier ne tient plus sur la page**, le document — facture comme avoir — est refusé avec un message explicite : c'est délibéré. Mieux vaut un refus qu'un document où l'en-tête s'imprime par-dessus le tableau des lignes. La cause peut être vos coordonnées, mais aussi une adresse de destinataire particulièrement longue. Le message vous indique de **supprimer** une coordonnée : la raccourcir ne libère aucune place, chacune occupant une ligne entière quelle que soit sa longueur.
...
- **Un compte de produit sur la fiche article** : chaque article du catalogue peut désormais porter **son** compte de produit, choisi **une fois** sur sa fiche plutôt que ressaisi à chaque facture.
  ...
  - **Ce n'est pas une chaîne de repli, c'est une recopie — et la nuance se voit à l'usage.** Le compte de l'article est **copié dans la ligne** au moment où vous choisissez l'article dans le catalogue ; la ligne ne reste pas liée à lui. Si vous **videz** ensuite le compte d'une telle ligne, elle suit le compte par défaut de la **société**, et non celui de l'article. À la validation, il n'y a donc que deux maillons : le compte de la **ligne**, sinon celui de la **société**.
  - **Un compte devenu inutilisable n'est signalé qu'au moment où l'on s'en sert.** Si vous archivez un compte qu'un article référence, la fiche article ne proteste pas — mais la ligne de facture créée depuis cet article l'affiche, le marque invalide et bloque l'enregistrement en nommant la ligne. Le signal est rendu une fois, là où il est actionnable.
  - **Renommer un article dont le compte a été archivé reste possible.** La validation ne se déclenche que si vous **changez** le compte : sans cela, l'article deviendrait inéditable et sans issue, le compte archivé n'étant plus proposé par le sélecteur.
  - **Intégrations par API** : la modification d'un article (`PUT /api/v1/products`) **remplace** tous ses champs. Un client qui relit un article puis le réécrit **sans** la clé `defaultRevenueAccountId` **efface** donc son compte, sans erreur — même comportement que pour les lignes de facture, et même suivi dans l'issue #278.
- **Un compte de produit par ligne de facture** : chaque ligne d'une facture peut désormais porter **son propre compte de produit**, au lieu que tout le chiffre d'affaires atterrisse sur un compte unique. Une facture mêlant honoraires, prestations de services et marchandises ventile son produit **sur les bons comptes dès la validation**, ce qui rend le compte de résultat exploitable sans reclassement manuel a posteriori. Une ligne qui ne précise rien continue de suivre le **compte de produit par défaut de la société** configuré dans les *Réglages* — le comportement actuel est donc strictement conservé pour toutes les factures existantes. Les **avoirs** ventilent en miroir : la contre-passation extourne exactement les comptes que la facture a crédités, y compris si le compte par défaut de la société a changé entre-temps — pour les factures validées à partir de cette version **comme pour les factures plus anciennes**, reprises automatiquement (voir la puce dédiée ci-dessous, **avec ses deux réserves** : les écritures retouchées à la main ne sont pas reprises, et un compte de produit archivé depuis fait échouer l'avoir). La colonne apparaît dans l'export CSV des lignes de facture. (#152, CR #265)
  - **Le compte se choisit directement dans le formulaire de facture**, ligne par ligne, tant que la facture est un brouillon. Une ligne laissée vide affiche entre parenthèses le compte par défaut de la société qu'elle suivra — vous voyez donc où ira le produit sans avoir à le deviner, et il n'est jamais nécessaire de sélectionner le défaut « pour être sûr ». Un bouton d'effacement, ou simplement vider le champ, ramène la ligne au compte par défaut.
  - **Les comptes devenus inutilisables sont signalés au lieu de disparaître.** Si une ligne référence un compte archivé, devenu non imputable, ou qui n'est pas un compte de produit, Kesh affiche son libellé **avec un signalement** et refuse l'enregistrement en nommant **toutes** les lignes concernées d'un coup — pas une par une. Rien n'est effacé en silence : c'est vous qui choisissez le compte de remplacement.
  - **Le compte est consultable après validation**, dans une colonne du détail de la facture. Une facture validée n'est plus modifiable ; sans cette colonne, la ventilation n'était plus lisible qu'en ouvrant l'écriture comptable et en la relisant à l'envers. Le détail d'un **avoir** affiche la même colonne. Le **PDF est inchangé** — l'imputation comptable est une information interne, elle n'a pas à figurer sur le document envoyé au client.
  - **Le compte de produit par défaut de la société reste obligatoire**, même pour une facture dont **toutes** les lignes désignent explicitement leur compte. Tant que *Réglages → compte de produit par défaut* n'est pas renseigné, la **validation** est refusée avec un message explicite — alors que l'enregistrement du **brouillon**, lui, passe. Le contournement est immédiat (renseigner le réglage une fois, il n'a pas besoin d'être utilisé), mais le blocage survient à la validation, pas à la saisie.
  - **Intégrations par API** : la modification d'une facture (`PUT`) **remplace** l'intégralité de ses lignes. Un client qui relit une facture puis la réécrit **sans** la clé `revenueAccountId` remet donc les lignes concernées sur le compte par défaut, sans erreur. Renvoyez toujours le champ tel que vous l'avez lu. Le durcissement du contrat d'API est suivi dans l'issue #278.
- **Vos factures déjà validées sont reprises automatiquement à la mise à jour** : Kesh renseigne rétroactivement le compte de produit de chaque ligne des factures **déjà validées** et des avoirs **déjà émis**, en le lisant sur **l'écriture comptable que la pièce a réellement produite** — jamais sur le compte par défaut tel qu'il est configuré aujourd'hui. Conséquence concrète : un avoir émis **à partir de maintenant** sur une facture ancienne extourne bien le compte que cette facture avait crédité à l'époque, **même si vous avez changé le compte de produit par défaut entre-temps**. Sans cette reprise, le compte de résultat aurait accumulé des soldes résiduels se compensant deux à deux, sans qu'aucun message ne le signale. La reprise est automatique : rien à ressaisir. (#152, CR #265)
  - **⚠️ Un cas demande votre attention : les comptes de produit archivés.** Si vous avez **archivé** un compte de produit depuis qu'une facture ancienne l'a crédité, l'émission d'un avoir sur cette facture est désormais **refusée**, avec un message nommant la ligne et le compte à réactiver. Avant cette reprise, l'avoir passait — mais en imputant le compte par défaut du jour, c'est-à-dire en créant exactement le solde résiduel décrit ci-dessus. Le refus est donc volontaire : mieux vaut vous demander de réactiver le compte le temps de l'avoir que de fausser silencieusement votre compte de résultat. Réactivez le compte dans le **plan comptable**, émettez l'avoir, puis archivez-le à nouveau si vous le souhaitez. Un avertissement au moment de l'archivage, pour que vous le sachiez avant plutôt qu'à l'émission de l'avoir, est suivi dans l'issue #280.
  - **Ce qui n'est délibérément pas repris** : les factures dont **l'écriture comptable a été modifiée à la main** après validation, quand cette modification rend le compte de produit ambigu. Kesh ne reprend une ligne que s'il peut l'identifier **sans la moindre ambiguïté** — concrètement, s'il existe dans l'écriture exactement une ligne de crédit égale au montant hors taxe de la facture, sur un compte imputable. Sont donc laissées de côté les écritures où le produit a été **ventilé sur plusieurs comptes**, celles dont les **montants** ont été ajustés de sorte qu'aucune ligne ne corresponde plus au hors-taxe, et celles qui ont crédité un **compte de regroupement** (un compte non imputable, sur lequel Kesh refuse par ailleurs toute saisie directe). Kesh **préfère ne rien écrire** plutôt que de deviner : inscrire un compte approximatif sur une pièce comptable réelle serait pire que l'absence d'information. *(En revanche, une écriture dont les lignes ont seulement été **réordonnées** reste reprise sans problème : l'identification ne dépend pas de l'ordre des lignes.)* Ces lignes gardent le comportement antérieur (repli sur le compte par défaut au moment de l'avoir). C'est sans effet sur vos écritures existantes, qui ne sont **jamais** modifiées.
  - **⚠️ Pour ces lignes-là, le risque de résidu subsiste.** Si vous émettez un avoir sur une telle facture **après avoir changé le compte de produit par défaut**, la contre-passation n'extournera pas le compte réellement crédité à l'époque : le total reste juste et le bilan équilibré, mais deux comptes de produit gardent un solde résiduel qui se compensent, et **aucun message ne le signale**. En pratique, sur une facture que les requêtes ci-dessous dénombrent, **contrôlez l'écriture d'avoir générée** — ou évitez d'en émettre un si le compte par défaut a changé depuis.
  - **Compter ce qui n'a pas été repris** (facultatif, pour contrôle après la mise à jour) — à exécuter sur la base de données :

    ```sql
    -- Lignes de facture non reprises (factures validées)
    ...
    -- Lignes d'avoir non reprises (avoirs émis)
    ...
    Un résultat non nul est **normal** : il dénombre les pièces à écriture retouchée décrites ci-dessus. Sur une installation où aucune écriture de facture n'a été modifiée à la main, ces deux requêtes renvoient `0`.

    Deux limites à connaître avant d'interpréter le chiffre. D'abord, ces requêtes ne comptent que les factures **validées** et les avoirs **émis** : les factures **annulées** (donc **toutes** celles qui ont reçu un avoir) gardent volontairement leurs lignes vides et n'y apparaissent pas — c'est normal et sans effet comptable, l'avoir portant l'information. Ensuite, **restaurer une sauvegarde ancienne ne fausse plus ce chiffre** : la reprise est désormais **rejouée automatiquement à la fin de chaque import**, de sorte qu'une sauvegarde antérieure à la reprise donne, une fois restaurée, le même état qu'un parc mis à jour. *(La reprise se fonde sur ce que la sauvegarde contient : elle ne peut donc rien reconstituer d'une sauvegarde prise, elle, depuis un parc déjà amputé par un restore antérieur à cette version — cf. la puce « Corrigé » ci-dessous.)* (#281)
  - **Avoirs déjà émis : la reprise constate, elle ne corrige pas.** Si vous avez émis un avoir *par le passé* après avoir changé le compte de produit par défaut, l'écriture d'avoir a réellement débité l'autre compte — c'est fait, et irréversible. Kesh enregistre alors **fidèlement les deux comptes tels qu'ils ont été mouvementés**, sans chercher à les harmoniser : forcer leur égalité falsifierait les pièces sans corriger la moindre écriture. Le décalage se solde par une **écriture de reclassement**, qui est une décision comptable vous appartenant. Vous pouvez donc voir, sur une facture ancienne et son avoir, **deux comptes de produit différents** : ce n'est pas une anomalie.

### Corrigé

- **Restaurer une sauvegarde ancienne ne défait plus les reprises de données** : jusqu'ici, importer un `.keshbackup` produit par une version antérieure de Kesh **annulait silencieusement et définitivement** certaines reprises automatiques faites lors des mises à jour — les **rôles de vos comptes** (créances clients, TVA due, résultat de l'exercice…) et le **compte de produit de vos lignes de facture** revenaient à vide, sans aucun message. Le mécanisme qui les avait renseignés ne se rejouait jamais, Kesh sachant qu'il avait déjà tourné. Ces reprises sont désormais **rejouées à la fin de chaque import**, dans la même opération : soit tout réussit, soit rien n'est modifié. Une donnée que votre sauvegarde contenait déjà n'est **jamais** écrasée — un rôle ou un réglage que vous aviez ajusté à la main est conservé tel quel. Si vous avez restauré une sauvegarde **avant** cette version, réimporter cette même sauvegarde applique cette fois la reprise — **mais pesez-en le coût avant de le faire** : un import **remplace l'intégralité de l'installation** par le contenu du fichier, donc **tout ce que vous avez saisi depuis ce restore serait perdu** (factures, écritures, contacts…). Ne relancez cet import que si vous n'avez rien saisi entre-temps. Sinon, corrigez directement ce qui manque — les **rôles** se règlent depuis le plan comptable, le **compte de produit** d'une ligne depuis la facture. (#281)
