...
- **Vos factures déjà validées sont reprises automatiquement à la mise à jour** : Kesh renseigne rétroactivement le compte de produit de chaque ligne des factures **déjà validées** et des avoirs **déjà émis**, en le lisant sur **l'écriture comptable que la pièce a réellement produite** — jamais sur le compte par défaut tel qu'il est configuré aujourd'hui. Conséquence concrète : un avoir émis **à partir de maintenant** sur une facture ancienne extourne bien le compte que cette facture avait crédité à l'époque, **même si vous avez changé le compte de produit par défaut entre-temps**. Sans cette reprise, le compte de résultat aurait accumulé des soldes résiduels se compensant deux à deux, sans qu'aucun message ne le signale. La reprise est automatique : rien à ressaisir. (#152, CR #265)
  ...
  - **Ce qui n'est délibérément pas repris** : les factures dont **l'écriture comptable a été modifiée à la main** après validation. Kesh ne reprend une ligne que s'il peut identifier le compte de produit **sans la moindre ambiguïté** — concrètement, s'il existe dans l'écriture exactement une ligne de crédit égale au montant hors taxe de la facture. Dès qu'une écriture a été retouchée (produit ventilé sur plusieurs comptes, lignes réorganisées, montants ajustés), Kesh **préfère ne rien écrire** plutôt que de deviner : inscrire un compte approximatif sur une pièce comptable réelle serait pire que l'absence d'information. Ces lignes gardent donc le comportement antérieur (repli sur le compte par défaut au moment de l'avoir). C'est sans effet sur vos écritures existantes, qui ne sont **jamais** modifiées.
  - **Compter ce qui n'a pas été repris** (facultatif, pour contrôle après la mise à jour) — à exécuter sur la base de données :
    ...
    Un résultat non nul est **normal** et sans gravité : il dénombre les pièces à écriture retouchée décrites ci-dessus. Sur une installation où aucune écriture de facture n'a été modifiée à la main, ces deux requêtes renvoient `0`.
