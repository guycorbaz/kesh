...
- **Un compte de produit par ligne de facture** : chaque ligne d'une facture peut désormais porter **son propre compte de produit**, au lieu que tout le chiffre d'affaires atterrisse sur un compte unique. Une facture mêlant honoraires, prestations de services et marchandises ventile son produit **sur les bons comptes dès la validation**, ce qui rend le compte de résultat exploitable sans reclassement manuel a posteriori. Une ligne qui ne précise rien continue de suivre le **compte de produit par défaut de la société** configuré dans les *Réglages* — le comportement actuel est donc strictement conservé pour toutes les factures existantes. Les **avoirs** ventilent en miroir : pour toute facture validée **à partir de cette version**, la contre-passation extourne exactement les comptes que la facture a crédités, y compris si le compte par défaut de la société a changé entre-temps. La colonne apparaît dans l'export CSV des lignes de facture. (#152, CR #265)
  ...
  - **Factures validées avant cette version** : elles conservent le comportement actuel — un avoir se replie sur le compte de produit par défaut **tel qu'il est au moment de l'avoir**. ⚠️ Si vous avez **changé ce compte par défaut** depuis la validation d'une facture ancienne, l'avoir n'extournera donc pas le compte réellement crédité à l'époque : le total reste juste et le bilan équilibré, mais deux comptes de produit gardent un solde résiduel qui se compensent. Aucun message ne le signale. En pratique, **évitez d'émettre un avoir sur une facture antérieure si le compte par défaut a changé entre-temps**, ou contrôlez l'écriture générée. Leur reprise automatique fait l'objet d'une mise à jour dédiée.
