...
// Config du proxy `/api` → backend kesh-api (port défaut configuré côté
// backend) partagée entre `vite dev` et `vite preview`. Sans preview proxy,
// Playwright (qui lance `npm run preview` sur :4173) ne peut pas acheminer
// `/api/v1/*` vers le backend kesh-api → tous les appels API du frontend
// et des tests retournent 404/401. Corrigé Story 6-4.
//
...
// `:5173/health` au lieu de `localhost/health` → 404 vite → `setDegraded()`
// fire → DegradedBanner permanent même quand kesh-api backend est healthy.
const apiProxy = {
	'/api': {
