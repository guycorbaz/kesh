...
async fn full_export_structure_manifest_and_integrity(pool: MySqlPool) {
    ...
    // manifest.json au root + dossier files/ + 31 data/<table>.ndjson
    ...
    // payment_batches, payment_batch_items + 1 table projets Story 19-1 : projects).
    assert!(
    ...
    assert_eq!(
        data_count, 31,
        "31 fichiers data/<table>.ndjson : {names:?}"
    );
