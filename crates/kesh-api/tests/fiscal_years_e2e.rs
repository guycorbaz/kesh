...
async fn reopen_via_pat_returns_403(pool: MySqlPool) {
    ...
    // Code-review Pass 3 (F3) — asserter le CODE prouve que le 403 ne vient pas
    // du middleware RBAC : la clé PAT porte le rôle Admin de son créateur, donc
    // `require_admin_role` passe. Le raisonnement reste entier ; seule la garde
    // qui refuse a changé.
    //
    // Story 22-4a (#167) — c'est désormais la COUCHE posée sur `admin_routes`
    // qui répond, avant le handler ; son `ensure_not_pat` (D3) subsiste sans
    // être atteint, et son câblage est prouvé par l'assertion de source de
    // `admin_pat_denied_e2e`.
    let body: serde_json::Value = resp.json().await.unwrap();
