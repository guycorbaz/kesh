...
error-journal-entry-linked-to-invoice = Questa registrazione contabile è stata generata da una fattura convalidata e non può essere eliminata direttamente. Annulla prima la convalida della fattura, oppure correggila con una nota di credito.
...
error-invoice-unvalidate-blocked-matched = La registrazione di questa fattura è riconciliata con una transazione bancaria: annullate prima la riconciliazione.
...
accounts-updating = Modifica…
accounts-retype-title = Cambiare il tipo del conto { $number }?
accounts-retype-warning = Questo conto presenta { $count } registrazione/i. Cambiarne il tipo riclassifica l’intera cronologia: gli importi passano dal conto economico al bilancio, o viceversa.
accounts-retype-closed-years = Esercizi chiusi interessati: { $years }. Il loro risultato cambierà senza che alcuna registrazione lo spieghi.
accounts-retype-confirm = Cambia comunque il tipo
...
invoices-settlements-title = Pagamenti
invoices-settlements-col-date = Data
invoices-settlements-col-amount = Importo
invoices-settlements-col-mode = Modalità
invoices-settlements-col-entry = Scrittura
invoices-settlements-entry-link = Vedi la scrittura
invoices-settlements-type-bank = Bonifico bancario
invoices-settlements-type-internal = Contanti o altro conto
invoices-settlement-cancel-button = Annulla il pagamento
...
settlement-cancel-blocked-fiscal-year-closed = Questo pagamento appartiene a un esercizio chiuso: un amministratore deve riaprire l'esercizio per poterlo annullare.
settlement-cancel-blocked-bank-match = Questo pagamento è riconciliato con una transazione bancaria: annulla prima la riconciliazione, da questa scheda o dal dettaglio dell'importazione bancaria.
settlement-cancel-blocked-account-archived = Un conto di questo pagamento è stato archiviato: riattivalo per poter annullare il pagamento.
...
reconciliation-cancel-blocked-account-archived = Un conto della scrittura di questa riconciliazione è stato archiviato: riattivalo per poter annullare la riconciliazione.
reconciliation-cancel-blocked-no-fiscal-year = Nessun esercizio aperto copre la data di oggi: crealo per poter annullare questa riconciliazione.
reconciliation-cancel-button = Annulla la riconciliazione
reconciliation-cancel-loading = Lettura della riconciliazione…
reconciliation-cancel-confirm-entry = Verrà registrata una scrittura di storno con la data di oggi, e la transazione tornerà da riconciliare.
reconciliation-cancel-confirm-invoice = Verrà registrata una scrittura di storno con la data di oggi, il pagamento della fattura { $invoice } verrà rimosso e la fattura tornerà da pagare; la transazione tornerà da riconciliare.
reconciliation-cancel-confirm-submit = Conferma l'annullamento
reconciliation-cancel-dismiss = Chiudi
reconciliation-cancel-done = Riconciliazione annullata.
reconciliation-cancel-entry-link = Vedi la scrittura
reconciliation-cancel-column = Riconciliazione
...
export-global-description = Esportate tutta la vostra contabilità in formato CSV in un file ZIP: piano dei conti, scritture, fatture di vendita e di acquisto, note di credito, pagamenti, progetti, contatti, dati bancari e registro di audit. Utilizzate questa esportazione per archiviare i dati per 10 anni (CO svizzero art. 958f) o per migrare verso un altro software.
...
export-global-content-includes = L'esportazione contiene 30 tabelle: piano dei conti, esercizi, scritture e relative righe, contatti e persone di contatto, prodotti, progetti, fatture di vendita e righe, note di credito e righe, pagamenti, fatture fornitori e righe, documenti fornitori importati, lotti di pagamento e voci, conti bancari, importazioni e transazioni, aliquote IVA, impostazioni di fatturazione e sollecito, regole di riconciliazione, profili di importazione, registro di audit — e un manifesto metadata.json con l'hash SHA-256 di ogni file.
export-global-content-excludes = Deliberatamente non incluso: i contatori di numerazione (i numeri assegnati figurano già sui documenti), le chiavi API e i token di sessione (sono segreti), il registro degli utenti (il registro di audit riporta il nome dell'autore al momento della scrittura), i modelli di e-mail e lo stato di installazione (configurazione, non contabilità). I giustificativi (file) non sono ancora inclusi.
...
supplier-invoices-settlement-cancel-confirm = Annullare questo pagamento? Una scrittura di storno con la data di oggi sarà registrata nel libro mastro, e la fattura tornerà aperta, da pagare.
supplier-invoices-settlement-cancel-batch-warning = Questa fattura figura nell'ordine di pagamento n. { $batch }, confermato il { $date }. Se la banca ha eseguito quest'ordine, è già stata pagata: correggete allora con un nuovo pagamento diretto, mai con un nuovo ordine.
supplier-invoices-settlement-cancelled = Pagamento annullato: la scrittura di storno è stata registrata e la fattura è di nuovo aperta.
supplier-invoices-settlement-entry-link = Vedi la scrittura di pagamento
supplier-invoices-settlement-cancel-blocked-not-paid = Questa fattura fornitore non è pagata: non c'è alcun pagamento da annullare.
...
#
#
audit-log-entity-account = Conto
audit-log-entity-api-key = Chiave API
audit-log-entity-bank-account = Conto bancario
audit-log-entity-bank-imports = Importazione bancaria
audit-log-entity-bank-profiles = Profilo bancario
audit-log-entity-bank-transaction = Transazione bancaria
audit-log-entity-company = Società
audit-log-entity-company-dunning-settings = Impostazioni di sollecito
audit-log-entity-company-invoice-settings = Impostazioni di fatturazione
audit-log-entity-contact = Contatto
audit-log-entity-contact-person = Persona di contatto
audit-log-entity-credit-note = Nota di credito
audit-log-entity-dunning-level = Livello di sollecito
audit-log-entity-email-template = Modello di e-mail
audit-log-entity-export = Esportazione
audit-log-entity-fiscal-year = Esercizio contabile
audit-log-entity-imported-supplier-invoice = Fattura importata
...
audit-log-entity-reconciliation-rules = Regola di assegnazione
audit-log-entity-report = Rapporto
audit-log-entity-supplier-invoice = Fattura fornitore
audit-log-entity-user = Utente
audit-log-entity-vat-rate = Aliquota IVA
#
audit-log-action-journal-entry-created = Registrazione creata
...
audit-log-action-account-archived = Conto archiviato
audit-log-action-account-reactivated = Conto riattivato
audit-log-action-account-retyped = Tipo di conto modificato
audit-log-action-fiscal-year-created = Esercizio contabile creato
audit-log-action-fiscal-year-updated = Esercizio contabile aggiornato
audit-log-action-fiscal-year-closed = Esercizio contabile chiuso
audit-log-action-fiscal-year-reopened = Esercizio contabile riaperto
audit-log-action-books-locked = Periodo contabile bloccato
audit-log-action-books-unlocked = Periodo contabile sbloccato
audit-log-action-books-restored = Blocco del periodo ripristinato dopo il ripristino
audit-log-action-vat-rate-created = Aliquota IVA creata
audit-log-action-vat-rate-updated = Aliquota IVA aggiornata
audit-log-action-vat-rate-deactivated = Aliquota IVA disattivata
audit-log-action-invoice-created = Fattura creata
audit-log-action-invoice-updated = Fattura aggiornata
audit-log-action-invoice-validated = Fattura convalidata
audit-log-action-invoice-unvalidated = Fattura annullata (convalida)
audit-log-action-invoice-cancelled = Fattura annullata
audit-log-action-invoice-deleted = Fattura eliminata
audit-log-action-invoice-emailed = Fattura inviata per e-mail
audit-log-action-invoice-paid = Fattura pagata
audit-log-action-invoice-partially-settled = Fattura parzialmente pagata
audit-log-action-invoice-settlement-cancelled = Pagamento annullato
audit-log-action-invoice-reminder-sent = Sollecito inviato
audit-log-action-invoice-reminder-cancelled = Sollecito annullato
audit-log-action-invoice-dunning-paused = Solleciti sospesi
audit-log-action-invoice-dunning-resumed = Solleciti ripresi
audit-log-action-credit-note-created = Nota di credito creata
audit-log-action-dunning-level-created = Livello di sollecito creato
audit-log-action-dunning-level-updated = Livello di sollecito aggiornato
audit-log-action-dunning-level-deleted = Livello di sollecito eliminato
audit-log-action-company-dunning-settings-updated = Impostazioni di sollecito aggiornate
audit-log-action-company-invoice-settings-updated = Impostazioni di fatturazione aggiornate
audit-log-action-supplier-invoice-created = Fattura fornitore creata
audit-log-action-supplier-invoice-cancelled = Fattura fornitore annullata
audit-log-action-supplier-invoice-paid = Fattura fornitore pagata
audit-log-action-supplier-invoice-settlement-cancelled = Pagamento fornitore annullato
audit-log-action-imported-supplier-invoice-created = Fattura importata creata
audit-log-action-imported-supplier-invoice-completed = Fattura importata completata
audit-log-action-imported-supplier-invoice-discarded = Fattura importata scartata
audit-log-action-imported-supplier-invoice-reactivated = Fattura importata riattivata
audit-log-action-payment-batch-generated = Lotto di pagamento creato
audit-log-action-payment-batch-confirmed = Lotto di pagamento confermato
audit-log-action-payment-batch-cancelled = Lotto di pagamento annullato
audit-log-action-bank-account-created = Conto bancario creato
audit-log-action-bank-account-updated = Conto bancario aggiornato
audit-log-action-bank-account-archived = Conto bancario archiviato
audit-log-action-bank-import-created = Importazione bancaria effettuata
audit-log-action-bank-profile-created = Profilo bancario creato
audit-log-action-bank-profile-updated = Profilo bancario aggiornato
audit-log-action-bank-profile-deleted = Profilo bancario eliminato
audit-log-action-reconciliation-accepted = Riconciliazione accettata
audit-log-action-reconciliation-cancelled = Riconciliazione annullata
audit-log-action-reconciliation-rejected = Riconciliazione rifiutata
audit-log-action-reconciliation-manual-matched = Riconciliazione manuale effettuata
audit-log-action-reconciliation-split-applied = Transazione suddivisa
audit-log-action-reconciliation-rule-created = Regola di assegnazione creata
audit-log-action-reconciliation-rule-updated = Regola di assegnazione aggiornata
audit-log-action-reconciliation-rule-deleted = Regola di assegnazione eliminata
audit-log-action-reconciliation-rule-applied = Regola di assegnazione applicata
audit-log-action-contact-created = Contatto creato
audit-log-action-contact-updated = Contatto aggiornato
audit-log-action-contact-archived = Contatto archiviato
audit-log-action-contact-person-created = Persona di contatto creata
audit-log-action-contact-person-updated = Persona di contatto aggiornata
audit-log-action-contact-person-archived = Persona di contatto archiviata
audit-log-action-product-created = Prodotto creato
audit-log-action-product-updated = Prodotto aggiornato
audit-log-action-product-archived = Prodotto archiviato
audit-log-action-project-created = Progetto creato
audit-log-action-project-updated = Progetto aggiornato
audit-log-action-project-archived = Progetto archiviato
audit-log-action-project-unarchived = Progetto riattivato
audit-log-action-user-created = Utente creato
audit-log-action-user-updated = Utente aggiornato
audit-log-action-user-disabled = Utente disattivato
audit-log-action-user-role-changed = Ruolo modificato
audit-log-action-user-password-reset = Password reimpostata
audit-log-action-api-key-created = Chiave API creata
audit-log-action-api-key-revoked = Chiave API revocata
audit-log-action-auth-password-reset-requested = Reimpostazione della password richiesta
audit-log-action-auth-password-reset-completed = Reimpostazione della password completata
audit-log-action-admin-break-glass-reset = Reimpostazione d'emergenza dell'amministratore
...
audit-log-action-admin-full-import = Backup importato
audit-log-action-exports-global = Esportazione globale effettuata
audit-log-action-report-generated = Rapporto generato
audit-log-action-report-exported = Rapporto esportato
#
audit-log-actor-type-user = Utente
audit-log-actor-type-api-key = Chiave API
#
audit-log-csv-header-id = N.
audit-log-csv-header-created-at = Data (UTC)
audit-log-csv-header-actor = Autore
audit-log-csv-header-user-id = ID autore
audit-log-csv-header-actor-type = Tipo di autore
audit-log-csv-header-action = Azione
audit-log-csv-header-entity-type = Tipo di entità
audit-log-csv-header-entity-id = ID entità
...
invoice-unvalidate-confirm-title = Annullare la convalida della fattura?
invoice-unvalidate-confirm-body = La sua registrazione contabile sarà eliminata e la fattura tornerà in bozza. Il numero è conservato: riconvalidandola, riprenderà lo stesso.
invoice-unvalidate-keeps-number = Numero conservato:
...
invoice-validate-success-no-number = Fattura convalidata
