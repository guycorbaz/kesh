...
create = Crea
creating = Creazione…
email-templates-admin-only = Accesso riservato agli amministratori.
...
email-templates-restore-confirm-body = Il vostro testo personalizzato per questa combinazione sarà eliminato e sostituito dal modello predefinito. Questa azione è irreversibile.
...
email-templates-subtitle = Personalizzate il contenuto delle e-mail inviate ai vostri clienti, per lingua. Se non modificate nulla, viene utilizzato automaticamente un modello predefinito.
...
email-templates-variables-hint = Inserite queste variabili nell'oggetto o nel corpo; saranno sostituite all'invio.
...
onboarding-already-finalized = La configurazione iniziale è già stata completata. Ricaricate la pagina.
onboarding-field-name-hint = azienda — altrimenti compilate nome + cognome
onboarding-finalize-incomplete = Il completamento non è stato confermato. Ricaricate la pagina.
onboarding-save = Salva
payment-batches-back = Indietro
...
payment-batches-confirm-hint = Una volta eseguito il bonifico nell'e-banking, confermate per contabilizzare i pagamenti.
...
payment-batches-err-bank = Selezionate un conto bancario di origine.
payment-batches-err-invoices = Selezionate almeno una fattura.
...
projects-empty = Nessun progetto. Create il vostro primo progetto per iniziare.
...
projects-subtitle = Raggruppate le vostre spese e i vostri ricavi per progetto (ristrutturazione, investimento) per analizzarli separatamente.
...
save = Salva
saving = Salvataggio…
settings-email-templates-link = Personalizzate il contenuto delle e-mail inviate ai vostri clienti (oggetto e corpo, per lingua).
