...
error-api-key-admin-forbidden = Administrationsrouten sind über einen API-Schlüssel nicht zugänglich, unabhängig von der Rolle seines Erstellers. Verwenden Sie die Weboberfläche.
...
error-journal-entry-linked-to-invoice = Diese Buchung stammt aus einer freigegebenen Rechnung und kann nicht direkt gelöscht werden. Heben Sie zuerst die Freigabe der Rechnung auf oder korrigieren Sie sie mit einer Gutschrift.
...
accounts-updating = Änderung…
accounts-retype-title = Kontotyp von Konto { $number } ändern?
accounts-retype-warning = Dieses Konto weist { $count } Buchung(en) auf. Eine Änderung des Kontotyps klassifiziert die gesamte Historie neu: Die Beträge wechseln von der Erfolgsrechnung in die Bilanz oder umgekehrt.
accounts-retype-closed-years = Betroffene abgeschlossene Geschäftsjahre: { $years }. Deren Ergebnis ändert sich, ohne dass eine Buchung dies erklärt.
accounts-retype-confirm = Kontotyp trotzdem ändern
...
journal-entry-form-col-project = Projekt
journal-entry-project-archived = Projekt archiviert
journal-entry-project-none = — Keines
...
journal-entries-filter-account = Konto
journal-entries-filter-account-all = Alle
...
contact-duplicate-others-count-one = und 1 weiterer
...
contact-form-client-number-hint = Erscheint auf der Rechnungs-PDF, damit Ihr Kunde sie ihrem Lieferantendossier zuordnen kann.
...
contact-filter-search-placeholder = Nach Name, E-Mail, Kundennummer oder UID ohne Trennzeichen suchen…
...
contact-error-client-number-duplicate = Ein Kontakt mit dieser Kundennummer existiert bereits
...
invoices-description-error-empty = Der Buchungstext ist leer
invoices-description-error-too-long = Der Buchungstext überschreitet { $max } Zeichen (aktuell: { $actual })
invoices-description-error-control-char = Steuerzeichen nicht zulässig
invoices-description-error-unknown-placeholder = Unbekannter Platzhalter: {"{"}{ $name }{"}"}
invoices-description-error-no-placeholder = Der Buchungstext muss mindestens einen bekannten Platzhalter enthalten ({"{"}YEAR{"}"}, {"{"}INVOICE_NUMBER{"}"}, {"{"}CONTACT_NAME{"}"})
invoices-format-error-empty = Das Nummerierungsformat ist leer
invoices-format-error-too-long = Das Format überschreitet { $max } Zeichen (aktuell: { $actual })
invoices-format-error-bad-chars = Das Format enthält unzulässige Zeichen
invoices-format-error-bad-padding = Ungültiges Padding {"{"}SEQ:{ $n }{"}"} — muss zwischen 1 und { $max } liegen
invoices-format-error-unknown-placeholder = Unbekannter Platzhalter: {"{"}{ $name }{"}"}
invoices-format-error-no-placeholder = Das Format muss mindestens einen bekannten Platzhalter enthalten ({"{"}YEAR{"}"}, {"{"}FY{"}"}, {"{"}SEQ{"}"}, {"{"}SEQ:NN{"}"})
invoices-format-error-rendered-too-long = Das Format würde eine Nummer mit { $len } Zeichen erzeugen (max. { $max })
...
invoice-settle-button = Zahlung erfassen
invoice-settle-button-short = Erfassen
invoice-settle-dialog-title = Zahlung erfassen
invoice-settle-dialog-body = Die Zahlung erzeugt ihre Buchung, unabhängig von der Zahlungsart.
invoice-settle-type-label = Zahlungsart
invoice-settle-type-bank = Banküberweisung
invoice-settle-type-internal = Bar oder anderes Konto
invoice-settle-bank-label = Bankkonto
invoice-settle-account-label = Konto
invoice-settle-account-placeholder = — Konto wählen
invoice-settle-amount-label = Betrag
invoice-settle-date-label = Zahlungsdatum
invoice-settle-confirm = Zahlung erfassen

## Story 25-3-a-1 (#414) — annuler un règlement client
invoices-settlements-title = Zahlungen
invoices-settlements-col-date = Datum
invoices-settlements-col-amount = Betrag
invoices-settlements-col-mode = Art
invoices-settlements-col-entry = Buchung
invoices-settlements-entry-link = Buchung anzeigen
invoices-settlements-type-bank = Banküberweisung
invoices-settlements-type-internal = Bar oder anderes Konto
invoices-settlement-cancel-button = Zahlung stornieren
...
settlement-cancel-blocked-fiscal-year-closed = Diese Zahlung gehört zu einem abgeschlossenen Geschäftsjahr: Ein Administrator muss das Geschäftsjahr wieder öffnen, damit sie storniert werden kann.
settlement-cancel-blocked-bank-match = Diese Zahlung ist mit einer Banktransaktion abgeglichen: Heben Sie zuerst den Abgleich auf, auf diesem Beleg oder im Detail des Bankimports.
settlement-cancel-blocked-account-archived = Ein Konto dieser Zahlung wurde archiviert: Reaktivieren Sie es, um die Zahlung stornieren zu können.
...
reconciliation-cancel-blocked-account-archived = Ein Konto der Buchung dieses Abgleichs wurde archiviert: Reaktivieren Sie es, um den Abgleich aufheben zu können.
reconciliation-cancel-blocked-no-fiscal-year = Kein offenes Geschäftsjahr umfasst das heutige Datum: Legen Sie es an, um diesen Abgleich aufheben zu können.
reconciliation-cancel-button = Abgleich aufheben
reconciliation-cancel-loading = Abgleich wird gelesen…
reconciliation-cancel-confirm-entry = Eine Gegenbuchung mit heutigem Datum wird erfasst, und die Transaktion wird wieder abzugleichen sein.
reconciliation-cancel-confirm-invoice = Eine Gegenbuchung mit heutigem Datum wird erfasst, die Zahlung der Rechnung { $invoice } wird entfernt und die Rechnung wird wieder offen sein; die Transaktion wird wieder abzugleichen sein.
reconciliation-cancel-confirm-submit = Aufhebung bestätigen
reconciliation-cancel-dismiss = Schliessen
reconciliation-cancel-done = Abgleich aufgehoben.
reconciliation-cancel-entry-link = Buchung anzeigen
reconciliation-cancel-column = Abgleich
invoice-settle-success-full = Zahlung erfasst — Rechnung beglichen
invoice-settle-success-partial = Teilzahlung erfasst
invoice-error-settled-on-required = Zahlungsdatum erforderlich
...
bank-import-profile-labels-actions = Aktionen
...
bank-accounts-error-qr-iban-not-qr = Diese IBAN ist keine QR-IBAN. Wenn Ihnen Ihre Bank keine spezielle QR-IBAN für QR-Rechnungen ausgestellt hat, lassen Sie dieses Feld leer: Ihre normale IBAN genügt.
bank-accounts-help-qr-iban = Nur auszufüllen, wenn Ihnen Ihre Bank eine spezielle QR-IBAN für QR-Rechnungen ausgestellt hat (Sondernummer mit einer Kennung 30000–31999). Andernfalls lassen Sie dieses Feld leer: Ihre normale IBAN genügt zur Erstellung von QR-Rechnungen.
...
reconciliation-manual-project-label = Analyseprojekt (optional)
reconciliation-manual-project-none = — Keines
reconciliation-rules-default-project-archived = Projekt archiviert
reconciliation-rules-default-project-none = — Keines
reconciliation-rules-labels-default-project = Standard-Analyseprojekt
reconciliation-split-add-line = + Zeile hinzufügen
reconciliation-split-bank-account-not-configured = Das Bankkonto ist nicht konfiguriert. Verknüpftes Buchhaltungskonto unter /bank-accounts konfigurieren.
reconciliation-split-error-no-proposal = Keine Transaktion ausgewählt
reconciliation-split-project-none = — Keines
reconciliation-split-remove-line = -
reconciliation-split-th-account = Konto
reconciliation-split-th-amount = Betrag
reconciliation-split-th-description = Beschreibung
reconciliation-split-th-project = Projekt
reconciliation-split-value-date-label = Valutadatum
reconciliation-split-error-min-lines = Mindestens { $min } Zeilen erforderlich
reconciliation-split-error-max-lines = Maximal { $max } Zeilen
reconciliation-split-error-account-required = Zeile { $line }: Konto erforderlich
reconciliation-split-error-amount-positive = Zeile { $line }: Betrag > 0 erforderlich
reconciliation-split-error-description-too-long = Zeile { $line }: Beschreibung zu lang (max. { $max })
...
reports-fiscal-year-label = Geschäftsjahr
reports-generate = Erzeugen
reports-project-expenses = Ausgaben pro Projekt
reports-project-expenses-col-account = Konto
reports-project-expenses-col-amount = Betrag
reports-project-expenses-empty = Für diesen Zeitraum sind diesem Projekt keine Ausgaben zugeordnet.
reports-project-expenses-subtotal = Zwischensumme
reports-project-expenses-title = Ausgaben pro Projekt
reports-project-expenses-total = Total Ausgaben
reports-project-mode-cumulative = Kumuliert seit Beginn
reports-project-mode-fiscal-year = Geschäftsjahr
reports-project-mode-label = Zeitraum
reports-project-return = Rendite pro Projekt
reports-project-return-col-cost = Investierte Kosten
reports-project-return-col-net = Nettoergebnis
reports-project-return-col-project = Projekt
reports-project-return-col-return = Rendite
reports-project-return-col-revenue = Erträge
reports-project-return-empty = Für diesen Zeitraum sind diesem Projekt keine Bewegungen zugeordnet.
reports-project-return-title = Rendite pro Projekt
reports-project-return-total = Total
reports-project-selector-label = Projekt
reports-project-selector-placeholder = — Projekt wählen
...
reports-filename-general-ledger = hauptbuch
...
export-global-description = Exportieren Sie Ihre gesamte Buchhaltung als CSV in einer ZIP-Datei: Kontenplan, Buchungen, Ausgangs- und Eingangsrechnungen, Gutschriften, Zahlungen, Projekte, Kontakte, Bankdaten und Audit-Protokoll. Nutzen Sie diesen Export zur Archivierung über 10 Jahre (OR Art. 958f) oder für den Wechsel zu einer anderen Software.
...
export-global-content-includes = Der Export enthält 30 Tabellen: Kontenplan, Geschäftsjahre, Buchungen und deren Positionen, Kontakte und Kontaktpersonen, Produkte, Projekte, Ausgangsrechnungen und Positionen, Gutschriften und Positionen, Zahlungen, Eingangsrechnungen und Positionen, importierte Lieferantenbelege, Zahlungsläufe und deren Posten, Bankkonten, Importe und Transaktionen, MWST-Sätze, Rechnungs- und Mahneinstellungen, Abgleichregeln, Importprofile, Audit-Protokoll — sowie ein metadata.json-Manifest mit SHA-256-Hash jeder Datei.
export-global-content-excludes = Bewusst nicht enthalten: die Nummernzähler (die vergebenen Nummern stehen bereits auf den Belegen), API-Schlüssel und Sitzungstokens (Geheimnisse), das Benutzerverzeichnis (das Audit-Protokoll führt den Namen des Urhebers zum Zeitpunkt der Erfassung), E-Mail-Vorlagen und Installationsstatus (Konfiguration, keine Buchhaltung). Belegdateien sind noch nicht enthalten.
...
common-admin-only = Zugriff nur für Administratoren.
...
dunning-delete-error = Löschen fehlgeschlagen.
dunning-grace-error = Die Karenzfrist muss eine positive ganze Zahl sein.
dunning-settings-conflict = Die Einstellungen wurden zwischenzeitlich geändert, neu geladen.
...
invoice-detail-project-label = Analyseprojekt
invoice-field-project = Analyseprojekt (optional)
invoice-project-archived = Projekt archiviert
invoice-project-current = Aktuelles Projekt
invoice-project-none = — Keines
...
settings-company-contact-conflict = Versionskonflikt — die Daten wurden neu geladen, bitte erneut versuchen.
settings-company-contact-conflict-reload-failed = Versionskonflikt und Neuladen fehlgeschlagen — laden Sie die Seite neu.
...
settings-invoicing-numbering-title = Nummerierung
settings-invoicing-seq-range = (NN zwischen 1 und 10)
...
settings-invoicing-load-error = Fehler beim Laden
...
imported-supplier-invoices-completed = Rechnung erstellt.
imported-supplier-invoices-completed-hint = Rechnung #{$id} erfasst.
imported-supplier-invoices-completed-reload-failed = Import ausgeführt, die Liste konnte aber nicht neu geladen werden — aktualisieren Sie die Seite.
imported-supplier-invoices-discard = Verwerfen
imported-supplier-invoices-discard-confirm = Diese importierte Rechnung verwerfen? Der Beleg bleibt erhalten.
imported-supplier-invoices-discard-conflict = Diese Rechnung wurde bereits von einer anderen Sitzung vervollständigt oder verworfen.
imported-supplier-invoices-discard-failed = Die Rechnung konnte nicht verworfen werden.
imported-supplier-invoices-discarded = Rechnung verworfen.
imported-supplier-invoices-doc-failed = Download nicht möglich.
imported-supplier-invoices-doc-gone = Der Beleg wurde nicht wiederhergestellt.
imported-supplier-invoices-empty = Keine Rechnung zu vervollständigen.
imported-supplier-invoices-err-amount = Die Summe der Zeilen ({$actual}) stimmt nicht mit dem QR-Betrag ({$expected}) überein.
imported-supplier-invoices-err-currency = Währung nicht unterstützt (nur CHF).
imported-supplier-invoices-err-fiscal-year = Kein offenes Geschäftsjahr deckt dieses Datum ab.
imported-supplier-invoices-err-form = Prüfen Sie den Lieferanten, das Datum und jede Zeile (Beschreibung, Beträge, Aufwandskonto).
imported-supplier-invoices-err-generic = Unerwarteter Fehler.
imported-supplier-invoices-err-iban-ref = Widerspruch zwischen IBAN und QRR-Referenz.
imported-supplier-invoices-err-not-found = Importierte Rechnung nicht gefunden.
imported-supplier-invoices-err-not-pending = Diese Rechnung wurde bereits vervollständigt oder verworfen.
...
imported-supplier-invoices-import-running = Ein Import läuft bereits. Versuchen Sie es in Kürze erneut.
...
imported-supplier-invoices-no-source-doc = Zu dieser Rechnung wurde kein Beleg importiert.
imported-supplier-invoices-reference = Ref.
...
imported-supplier-invoices-sum-ttc = Total der Zeilen inkl. MWST
imported-supplier-invoices-target = QR-Ziel
imported-supplier-invoices-title = Rechnungen importieren
imported-supplier-invoices-to-complete = Zu vervollständigende Rechnungen
imported-supplier-invoices-view-doc = Beleg anzeigen
imported-supplier-invoices-view-source = Ursprüngliche Rechnung anzeigen
imported-supplier-invoices-error-unsupported-file-type = Dateityp nicht unterstützt
imported-supplier-invoices-error-file-too-large = Datei zu gross
imported-supplier-invoices-error-symlink-rejected = Symbolischer Link abgelehnt
imported-supplier-invoices-error-duplicate = Bereits importiert (Duplikat)
imported-supplier-invoices-error-no-qr-code-found = Keine QR-Rechnung erkannt
imported-supplier-invoices-error-invalid-spc-payload = QR unlesbar (kein SPC-Format)
imported-supplier-invoices-error-invalid-iban = IBAN des Gläubigers ungültig
imported-supplier-invoices-error-pdf-render-error = PDF unlesbar
imported-supplier-invoices-error-file-read-error = Datei konnte nicht gelesen werden
...
supplier-invoices-settlement-cancel-confirm = Diese Zahlung stornieren? Eine Gegenbuchung mit dem heutigen Datum wird im Hauptbuch erfasst, und die Rechnung wird wieder offen und zu bezahlen.
supplier-invoices-settlement-cancel-batch-warning = Diese Rechnung ist im Zahlungsauftrag Nr. { $batch } enthalten, bestätigt am { $date }. Hat die Bank diesen Auftrag ausgeführt, wurde sie bereits bezahlt: Korrigieren Sie dann mit einer neuen direkten Zahlung, nie mit einem neuen Zahlungsauftrag.
supplier-invoices-settlement-cancelled = Zahlung storniert: Die Gegenbuchung wurde erfasst und die Rechnung ist wieder offen.
supplier-invoices-settlement-entry-link = Zahlungsbuchung anzeigen
supplier-invoices-settlement-cancel-blocked-not-paid = Diese Lieferantenrechnung ist nicht bezahlt: Es gibt keine Zahlung zu stornieren.
supplier-invoices-col-date = Datum
supplier-invoices-col-due = Fälligkeit
supplier-invoices-col-number = Nr.
supplier-invoices-col-status = Status
supplier-invoices-col-qty = Menge
supplier-invoices-col-vat = MWST
supplier-invoices-col-supplier = Lieferant
supplier-invoices-col-total = Inkl. MWST
supplier-invoices-detail-project = Projekt
supplier-invoices-detail-reference = Referenz
supplier-invoices-detail-title = Lieferantenrechnung
supplier-invoices-empty = Keine Lieferantenrechnung erfasst.
supplier-invoices-err-lines = Jede Zeile erfordert eine Beschreibung, einen Betrag und ein Aufwandskonto.
supplier-invoices-err-supplier = Wählen Sie einen Lieferanten.
supplier-invoices-field-date = Rechnungsdatum
...
supplier-invoices-field-supplier = Lieferant
supplier-invoices-form-close = Schliessen
supplier-invoices-line-account = Konto
supplier-invoices-line-desc = Beschreibung
supplier-invoices-line-ht = Exkl. MWST
supplier-invoices-line-total = Total exkl. MWST
supplier-invoices-lines = Zeilen
...
supplier-invoices-pay-internal = Internes Konto (Kasse, Karte…)
supplier-invoices-pay-submit = Bezahlen
supplier-invoices-pay-title = Rechnung bezahlen
supplier-invoices-pay-transfer = Banküberweisung
supplier-invoices-project-none = — Keines
supplier-invoices-save = Speichern
supplier-invoices-scan = QR-Rechnung scannen
supplier-invoices-scan-detected = Gläubiger erkannt
supplier-invoices-scan-failed = Dieses Bild konnte nicht gelesen werden.
supplier-invoices-scan-hint = Laden Sie ein Bild der QR-Rechnung hoch, um IBAN, Referenz und Betrag vorauszufüllen.
supplier-invoices-scan-no-qr = Auf diesem Bild wurde kein QR-Code erkannt.
...
credit-notes-col-date = Datum
credit-notes-col-description = Beschreibung
credit-notes-col-line-total = Total exkl. MWST
credit-notes-col-number = Gutschrift-Nr.
credit-notes-col-qty = Menge
credit-notes-col-status = Status
credit-notes-col-total = Total exkl. MWST
credit-notes-col-unit-price = Stückpreis
credit-notes-col-vat = MWST %
credit-notes-confirm-body = Eine vollständige Gutschrift wird erstellt und sofort verbucht: Sie storniert die Buchung dieser Rechnung (der Kundensaldo geht auf null zurück) und die Rechnung erhält den Status „storniert“. Diese Aktion ist endgültig. Fortfahren?
credit-notes-create-button = Gutschrift erstellen
credit-notes-create-error = Erstellung der Gutschrift fehlgeschlagen
credit-notes-created = Gutschrift erstellt
credit-notes-download-pdf = Drucken / PDF herunterladen
credit-notes-empty = Keine Gutschriften. Erstellen Sie eine Gutschrift aus einer validierten Rechnung, um diese zu stornieren.
credit-notes-title = Gutschriften
credit-notes-detail-title = Gutschrift
credit-notes-view-entry = Buchung anzeigen
credit-notes-view-invoice = Stornierte Rechnung anzeigen
credit-notes-view-list = Gutschriften anzeigen
...
email-templates-body-label = Nachrichtentext
email-templates-conflict = Versionskonflikt — die Vorlage wurde neu geladen
email-templates-load-error = Fehler beim Laden
email-templates-save-error = Fehler beim Speichern
email-templates-restore-error = Fehler beim Wiederherstellen
email-templates-lang-tablist = Sprache
email-templates-restore = Standard wiederherstellen
email-templates-restore-confirm-action = Standard wiederherstellen
email-templates-restore-confirm-body = Ihr angepasster Text für diese Kombination wird gelöscht und durch die Standardvorlage ersetzt. Diese Aktion ist unwiderruflich.
email-templates-restore-confirm-title = Standardvorlage wiederherstellen?
email-templates-restored = Standardvorlage wiederhergestellt
email-templates-restoring = Wird wiederhergestellt…
email-templates-saved = Vorlage gespeichert
email-templates-subject-label = Betreff
email-templates-subtitle = Passen Sie den Inhalt der an Ihre Kunden gesendeten E-Mails pro Sprache an. Wenn Sie nichts ändern, wird automatisch eine Standardvorlage verwendet.
email-templates-title = E-Mail-Vorlagen
email-templates-unknown-variables = Die Vorlage enthält unbekannte Variablen
email-templates-unknown-variables-list = Unbekannte Variablen:
email-templates-variables-hint = Fügen Sie diese Variablen in den Betreff oder den Text ein; sie werden beim Versand ersetzt.
email-templates-variables-title = Verfügbare Variablen
nav-credit-notes = Gutschriften
nav-email-templates = E-Mail-Vorlagen
nav-projects = Projekte
nav-supplier-invoices-import = Rechnungen importieren
...
onboarding-mode-error = Fehler bei der Moduswahl
onboarding-demo-load-error = Fehler beim Laden der Demodaten
onboarding-setup-start-error = Fehler beim Starten der Einrichtung
onboarding-org-type-error = Fehler bei der Typwahl
onboarding-accounting-lang-error = Fehler bei der Wahl der Buchhaltungssprache
onboarding-name-required = Name (oder Vorname + Name) erforderlich
onboarding-address-save-error = Fehler beim Speichern der Angaben
onboarding-finalize-error = Abschluss fehlgeschlagen — bitte erneut versuchen.
onboarding-bank-required = Bankname und IBAN sind erforderlich
onboarding-bank-save-error = Fehler beim Speichern des Bankkontos
onboarding-bank-skip-error = Fehler beim Überspringen des Bankkontos
onboarding-save = Speichern
payment-batches-cancel = Stapel stornieren
payment-batches-cancel-confirm = Diesen Stapel stornieren? Die Rechnungen werden wieder zahlbar.
payment-batches-col-date = Ausführung
payment-batches-col-id = Stapel
payment-batches-col-status = Status
payment-batches-col-total = Gesamtbetrag
payment-batches-confirm = Stapel bestätigen
payment-batches-confirm-hint = Sobald die Überweisung im E-Banking ausgeführt ist, bestätigen Sie, um die Zahlungen zu verbuchen.
payment-batches-confirm-title = Zahlung bestätigen
payment-batches-confirmed-at = Bestätigt am
payment-batches-created = Stapel erstellt
payment-batches-detail-date = Ausführungsdatum
payment-batches-detail-title = Zahlungsstapel
payment-batches-download = pain.001-Datei herunterladen
payment-batches-empty = Kein Zahlungsstapel.
payment-batches-err-bank = Wählen Sie ein Quell-Bankkonto.
payment-batches-err-invoices = Wählen Sie mindestens eine Rechnung.
payment-batches-field-bank = Quell-Bankkonto
payment-batches-field-date = Gewünschtes Ausführungsdatum
payment-batches-form-close = Schliessen
payment-batches-generate = Stapel erstellen
payment-batches-item-invoice = Rechnung
payment-batches-line-amount = Betrag
payment-batches-lot = Stapel
payment-batches-msg-id = MsgId
payment-batches-new = Überweisungsstapel erstellen
payment-batches-no-open = Keine offene Lieferantenrechnung.
payment-batches-open = Datei öffnen und herunterladen
payment-batches-payment-date = Effektives Zahlungsdatum
payment-batches-rejected = Nicht enthaltene Rechnungen:
payment-batches-select-invoices = Per Überweisung zu zahlende Rechnungen
projects-archive = Archivieren
projects-archive-error = Vorgang nicht möglich.
projects-archived = Projekt archiviert.
projects-archived-tag = archiviert
projects-created = Projekt erstellt.
projects-edit = Bearbeiten
projects-empty = Kein Projekt. Erstellen Sie Ihr erstes Projekt, um zu beginnen.
projects-err-code = Der Projektcode ist erforderlich.
projects-err-name = Der Projektname ist erforderlich.
projects-field-code = Code
projects-field-description = Beschreibung (optional)
projects-field-end = Ende
projects-field-name = Name
projects-field-parent = Übergeordnetes Projekt (optional)
projects-field-start = Beginn
projects-form-create = Neues Projekt
projects-form-edit = Projekt bearbeiten
projects-load-error = Projekte können nicht geladen werden.
projects-new = Neues Projekt
projects-parent-none = — Keines (Stammprojekt)
projects-save = Speichern
projects-save-error = Speichern nicht möglich.
projects-show-archived = Archivierte Projekte anzeigen
projects-subtitle = Fassen Sie Ihre Ausgaben und Erträge nach Projekt zusammen (Renovation, Investition), um sie separat auszuwerten.
...
# --- Hauptbuch (Story 24-1) ---
reports-ledger-title = Hauptbuch
reports-ledger-tab = Hauptbuch
reports-ledger-description = Der Kontoauszug: was den Saldo ausmacht, Zeile für Zeile.
reports-ledger-account-label = Konto
reports-ledger-all-accounts = Alle bebuchten Konten
reports-ledger-from = Von
reports-ledger-to = Bis
reports-ledger-opening = Eröffnungssaldo
reports-ledger-closing = Schlusssaldo
reports-ledger-movements-total = Total der Bewegungen
reports-ledger-column-piece = Beleg
reports-ledger-column-journal = Journal
reports-ledger-column-counterpart = Gegenkonto
reports-ledger-column-running = Laufender Saldo
reports-ledger-archived = archiviert
reports-ledger-unnatural = Saldo entgegen der Kontonatur
reports-ledger-unnatural-hint = Dieses Konto weist einen Saldo auf der entgegengesetzten Seite auf. Bitte prüfen.
reports-ledger-fy-break = Geschäftsjahresabschluss — der Saldo beginnt neu bei null
reports-ledger-no-movement = Keine Bewegung in dieser Periode. Der Eröffnungssaldo bleibt offen.
reports-ledger-empty = Keine Konten für diese Periode.
reports-ledger-open-from-balance = Details im Hauptbuch ansehen
reports-ledger-truncated = Es werden nur die ersten { $shown } von { $total } Zeilen angezeigt. Der Export enthält alle.
reports-ledger-include-zero = Konten ohne Bewegung einschliessen
reports-ledger-instruction = Wählen Sie einen Zeitraum und klicken Sie auf Erstellen. Das Konto ist optional.

# --- Journal d'audit — route, vocabulaire et export (Story 25-1c-a) ---
# ----------------------------------------------------------------------
#
#
audit-log-entity-account = Konto
audit-log-entity-api-key = API-Schlüssel
audit-log-entity-bank-account = Bankkonto
audit-log-entity-bank-imports = Bankimport
audit-log-entity-bank-profiles = Bankprofil
audit-log-entity-bank-transaction = Banktransaktion
audit-log-entity-company = Unternehmen
audit-log-entity-company-dunning-settings = Mahneinstellungen
audit-log-entity-company-invoice-settings = Fakturierungseinstellungen
audit-log-entity-contact = Kontakt
audit-log-entity-contact-person = Kontaktperson
audit-log-entity-credit-note = Gutschrift
audit-log-entity-dunning-level = Mahnstufe
audit-log-entity-email-template = E-Mail-Vorlage
audit-log-entity-export = Export
audit-log-entity-fiscal-year = Geschäftsjahr
audit-log-entity-imported-supplier-invoice = Importierte Rechnung
...
audit-log-entity-reconciliation-rules = Zuordnungsregel
audit-log-entity-report = Bericht
audit-log-entity-supplier-invoice = Lieferantenrechnung
audit-log-entity-user = Benutzer
audit-log-entity-vat-rate = MWST-Satz
#
audit-log-action-journal-entry-created = Buchung erstellt
...
audit-log-action-account-archived = Konto archiviert
audit-log-action-account-reactivated = Konto reaktiviert
audit-log-action-account-retyped = Kontotyp geändert
audit-log-action-fiscal-year-created = Geschäftsjahr erstellt
audit-log-action-fiscal-year-updated = Geschäftsjahr aktualisiert
audit-log-action-fiscal-year-closed = Geschäftsjahr abgeschlossen
audit-log-action-fiscal-year-reopened = Geschäftsjahr wieder eröffnet
audit-log-action-books-locked = Buchungsperiode gesperrt
audit-log-action-books-unlocked = Buchungsperiode entsperrt
audit-log-action-books-restored = Periodensperre nach Wiederherstellung gesetzt
audit-log-action-vat-rate-created = MWST-Satz erstellt
audit-log-action-vat-rate-updated = MWST-Satz aktualisiert
audit-log-action-vat-rate-deactivated = MWST-Satz deaktiviert
...
audit-log-action-invoice-paid = Rechnung bezahlt
audit-log-action-invoice-partially-settled = Rechnung teilweise bezahlt
audit-log-action-invoice-settlement-cancelled = Zahlung storniert
audit-log-action-invoice-reminder-sent = Mahnung gesendet
audit-log-action-invoice-reminder-cancelled = Mahnung storniert
audit-log-action-invoice-dunning-paused = Mahnungen pausiert
audit-log-action-invoice-dunning-resumed = Mahnungen fortgesetzt
audit-log-action-credit-note-created = Gutschrift erstellt
audit-log-action-dunning-level-created = Mahnstufe erstellt
audit-log-action-dunning-level-updated = Mahnstufe aktualisiert
audit-log-action-dunning-level-deleted = Mahnstufe gelöscht
audit-log-action-company-dunning-settings-updated = Mahneinstellungen aktualisiert
audit-log-action-company-invoice-settings-updated = Fakturierungseinstellungen aktualisiert
audit-log-action-supplier-invoice-created = Lieferantenrechnung erstellt
audit-log-action-supplier-invoice-cancelled = Lieferantenrechnung storniert
audit-log-action-supplier-invoice-paid = Lieferantenrechnung bezahlt
audit-log-action-supplier-invoice-settlement-cancelled = Lieferantenzahlung storniert
audit-log-action-imported-supplier-invoice-created = Importierte Rechnung erstellt
audit-log-action-imported-supplier-invoice-completed = Importierte Rechnung vervollständigt
audit-log-action-imported-supplier-invoice-discarded = Importierte Rechnung verworfen
audit-log-action-imported-supplier-invoice-reactivated = Importierte Rechnung reaktiviert
audit-log-action-payment-batch-generated = Zahlungsstapel erstellt
audit-log-action-payment-batch-confirmed = Zahlungsstapel bestätigt
audit-log-action-payment-batch-cancelled = Zahlungsstapel storniert
audit-log-action-bank-account-created = Bankkonto erstellt
audit-log-action-bank-account-updated = Bankkonto aktualisiert
audit-log-action-bank-account-archived = Bankkonto archiviert
audit-log-action-bank-import-created = Bankimport durchgeführt
audit-log-action-bank-profile-created = Bankprofil erstellt
audit-log-action-bank-profile-updated = Bankprofil aktualisiert
audit-log-action-bank-profile-deleted = Bankprofil gelöscht
audit-log-action-reconciliation-accepted = Abstimmung akzeptiert
audit-log-action-reconciliation-cancelled = Abgleich aufgehoben
audit-log-action-reconciliation-rejected = Abstimmung abgelehnt
audit-log-action-reconciliation-manual-matched = Manueller Abgleich durchgeführt
audit-log-action-reconciliation-split-applied = Transaktion aufgeteilt
audit-log-action-reconciliation-rule-created = Zuordnungsregel erstellt
audit-log-action-reconciliation-rule-updated = Zuordnungsregel aktualisiert
audit-log-action-reconciliation-rule-deleted = Zuordnungsregel gelöscht
audit-log-action-reconciliation-rule-applied = Zuordnungsregel angewendet
audit-log-action-contact-created = Kontakt erstellt
audit-log-action-contact-updated = Kontakt aktualisiert
audit-log-action-contact-archived = Kontakt archiviert
audit-log-action-contact-person-created = Kontaktperson erstellt
audit-log-action-contact-person-updated = Kontaktperson aktualisiert
audit-log-action-contact-person-archived = Kontaktperson archiviert
audit-log-action-product-created = Produkt erstellt
audit-log-action-product-updated = Produkt aktualisiert
audit-log-action-product-archived = Produkt archiviert
audit-log-action-project-created = Projekt erstellt
audit-log-action-project-updated = Projekt aktualisiert
audit-log-action-project-archived = Projekt archiviert
audit-log-action-project-unarchived = Projekt reaktiviert
audit-log-action-user-created = Benutzer erstellt
audit-log-action-user-updated = Benutzer aktualisiert
audit-log-action-user-disabled = Benutzer deaktiviert
audit-log-action-user-role-changed = Rolle geändert
audit-log-action-user-password-reset = Passwort zurückgesetzt
audit-log-action-api-key-created = API-Schlüssel erstellt
audit-log-action-api-key-revoked = API-Schlüssel widerrufen
audit-log-action-auth-password-reset-requested = Zurücksetzen des Passworts angefordert
audit-log-action-auth-password-reset-completed = Zurücksetzen des Passworts abgeschlossen
audit-log-action-admin-break-glass-reset = Notfall-Zurücksetzung des Administrators
audit-log-action-company-updated = Unternehmen aktualisiert
...
audit-log-action-admin-full-export = Vollständige Sicherung exportiert
audit-log-action-admin-full-import = Sicherung importiert
audit-log-action-exports-global = Globaler Export durchgeführt
audit-log-action-report-generated = Bericht erstellt
audit-log-action-report-exported = Bericht exportiert
#
audit-log-actor-type-user = Benutzer
audit-log-actor-type-api-key = API-Schlüssel
#
audit-log-csv-header-id = Nr.
audit-log-csv-header-created-at = Datum (UTC)
audit-log-csv-header-actor = Urheber
audit-log-csv-header-user-id = Urheber-ID
audit-log-csv-header-actor-type = Urhebertyp
audit-log-csv-header-action = Aktion
audit-log-csv-header-entity-type = Entitätstyp
audit-log-csv-header-entity-id = Entitäts-ID
audit-log-csv-header-api-key-id = API-Schlüssel
...
invoice-unvalidate-confirm-body = Ihre Buchung wird gelöscht und die Rechnung wird wieder zum Entwurf. Die Nummer bleibt erhalten: bei erneuter Freigabe erhält sie dieselbe.
invoice-unvalidate-keeps-number = Beibehaltene Nummer:
...
invoice-validate-success-no-number = Rechnung validiert
