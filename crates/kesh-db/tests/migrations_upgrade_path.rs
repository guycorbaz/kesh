...
async fn upgrade_path_preserves_data(pool: MySqlPool) {
    ...
    // + projects_analytics (Story 19-1) = 39.
    let total = kesh_db::MIGRATOR.migrations.len();
    assert_eq!(
        total, 39,
        "39 migrations attendues (26 historiques + _kesh_version Story 10-2 + companies_is_stub Story v011-2 + bank_accounts_archived Story v014-1 + api_keys & audit_log_actor Story 17-2a + users_email & password_reset_tokens Story 17-4a + vat_rates_crud Story 11-1 + vat_accounts_config Story 18-1a + credit_notes Story 12-1 + supplier_invoices Story 12-2 + payment_batches Story 12-3 + projects_analytics Story 19-1)"
    );
    ...
    // d'upgrade et incrémentent donc le `N` soustrait (de 4 à 6 à 8 à 10 à 11 à 12 à 13 à 14 à 15 à 16
    // avec projects_analytics Story 19-1), sans déplacer la frontière des 23 migrations historiques.
