...
-- ============================================================================
--
-- Les TROIS colonnes TVA de `company_invoice_settings` sont `NULL` sur TOUTE
-- installation : l'INSERT d'onboarding ne les énumère pas, le lazy-create
-- insère `(company_id)` seul, et aucune migration ne les renseigne
...
-- n'est JAMAIS candidate, et le backfill NO-OPE INTÉGRALEMENT — migration en
-- succès, reliquat « très élevé ». Ce mode de défaillance est
-- RIGOUREUSEMENT INDISCERNABLE DU SUCCÈS, puisque le paragraphe précédent
-- pré-autorise un reliquat élevé comme comportement conservateur normal. Seul
-- le test dédié `backfills_when_vat_config_is_null` l'attrape.
--
...
-- occasions de propager un NULL. L'exclusion de la créance est elle-même
-- redondante — sur l'écriture facture la créance est un DÉBIT, déjà éliminée
-- par (1) ; sur l'écriture d'avoir elle est un crédit, éliminée par le (1)
-- miroir. Conservée pour lisibilité et défense en profondeur.
--
-- ============================================================================
