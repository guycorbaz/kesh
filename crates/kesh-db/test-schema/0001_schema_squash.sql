...
CREATE TABLE `audit_log` (
  ...
  `company_id` bigint(20) DEFAULT NULL COMMENT 'Story 25-1c-zero — société de l''acteur AU MOMENT de l''écriture. Pointeur logique, sans FK (companies est remplacée au restore). NULL = société indéterminable, état permanent.',
  PRIMARY KEY (`id`),
  KEY `idx_audit_log_entity` (`entity_type`,`entity_id`),
  KEY `idx_audit_log_user_date` (`user_id`,`created_at` DESC),
  KEY `idx_audit_log_company_date` (`company_id`,`created_at`),
