...
Le risque résiduel se matérialiserait dans un **dossier contentieux où l'authenticité d'un export Kesh serait formellement contestée par une partie adverse** (typiquement : contrôle fiscal approfondi avec suspicion de fraude, litige civil sur la comptabilité d'une PME en liquidation, procédure pénale économique). Dans ces cas, un audit_log + SHA-256 *peut* être contesté comme « auto-référentiel » — un acteur malveillant disposant d'accès admin Kesh pourrait théoriquement falsifier *à la fois* l'export ET son hash. La QES + horodatage qualifié LSCSE supprimerait ce vecteur (tiers signataire indépendant). **Ce type de scénario contentieux reste marginal pour le tissu PME suisse** (estimation qualitative, ordre de grandeur de quelques % au plus — la jurisprudence publique disponible et les commentaires fiduciaires consultés ne signalent pas de cas typique où la signature des exports comptables d'un logiciel de gestion PME aurait été l'enjeu décisif d'une contestation) ; pour la grande majorité des cas (contrôles AFC routine, audit comptable normal, vérifications fiscales standard), l'audit_log + SHA-256 est accepté comme « procédé technique » suffisant au sens d'OLICo Art. 9 al. 1.b.
...
9. **Recommandation revue juridique externe avant publication v0.1** : **strictement optionnelle et non-bloquante** — le verdict (b) considère que Kesh est conforme OLICo Art. 9 al. 1.b pour publication v0.1. La revue externe est une mesure de prudence additionnelle (« defence in depth ») qui permet de sécuriser une éventuelle remise en cause par un juriste fiduciaire ou un client institutionnel particulièrement exigeant. Coût estimé CHF 1000-3000 PME. **Pas de contradiction avec le verdict (b)** : Kesh peut publier v0.1 sans cette revue ; la recommandation existe pour les contextes où Project Lead Guy souhaite un niveau de sécurité supplémentaire. **Urgence** : Project Lead Guy à décider hors story 9-5-4.
