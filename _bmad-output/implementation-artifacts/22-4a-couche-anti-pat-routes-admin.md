...
- [x] **T7 — Le gate complet, rejoué au DERNIER commit de la boucle de revue** (§ *Test Locally First*). Le `2177/2177` a été mesuré au commit de dev ; les passes 1 et 2 de revue ont modifié du code de test **après** lui. La § *Pendant une boucle de revue* autorise le gate ciblé entre les passes — elle impose le gate complet **avant le `git push`** qui ouvrira la PR.
  - [x] `cargo fmt --all -- --check` + `cargo clippy --workspace --all-targets -- -D warnings` — verts au dernier commit (`b0e10ef4`), rejoués le 2026-08-14 après la passe 4.
  - [x] gate backend complet — **`make test` (`cargo test --workspace --locked -- --include-ignored`, `RUST_TEST_THREADS=8`), exécuté par Guy le 2026-08-14 (16:05-16:2x), VERT, 0 échec**, sur l'arbre au commit `b0e10ef4`. ⚠️ Périmètre : ce run remplace le nextest profil `ci` prévu — celui lancé par la session a été arrêté en vol au profit du run de Guy (deux gates workspace simultanés sur le même MariaDB se contaminent), et c'est l'arbitrage de Guy que son run fasse foi. `--include-ignored` est un périmètre **plus large** que le profil `ci`.
  - [x] frontend : `npm run check` (0 erreur), `lint-i18n-ownership` (PASS), `test:unit` (**512/512**, 34 s), `build` — verts le 2026-08-14, sous `mem-guard`.
  - [x] suite E2E — **le run du 2026-08-13 (181 ✓ / 38 ✘ / 19 ignorés, cf. § E2E du Dev Agent Record) vaut pour l'état final, preuve par diff** : `git diff --stat 77761941..HEAD -- 'crates/*/src' frontend/src` est **vide** — depuis ce run, seuls un fichier de TEST Rust, les story files et la doc ont changé, aucun n'entrant dans le binaire ni dans le build servi à Playwright. Rejouer la suite aurait mesuré exactement le même code de production.
