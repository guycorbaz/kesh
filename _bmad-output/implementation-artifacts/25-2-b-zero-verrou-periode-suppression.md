...
| 2026-09-19 | revue P2 — **boucle close** | **Passe 2, CIBLÉE** (une lentille Opus, prompt versionné `25-2-b-zero-review-prompt-p2.md`), sur le seul commit `1c9ca464` : **0 au-dessus de LOW, 3 LOW**, cinq axes exercés — mutations non rejouées par la lentille, interdites. Corrigés quoique LOW, parce que peu coûteux : (1) le jour était calculé **deux fois**, si bien que le test du seuil aurait rougi à tort en franchissant minuit — et paniqué en franchissant le 1er janvier ; calculé une seule fois, la borne dérivée d'un recul en jours ; (2) la liste des refus du manuel omettait aussi les **rappels** (défaut antérieur, #260) : ajoutés, PDF contrôlé à plat ; (3) les doc-comments d'étapes de `delete_by_id` et `delete_in_tx` décrivent désormais toutes les gardes et leur lieu. Trois mutations rejouées, toutes tuées. **Gate complet : 2401/2401**, base remise à zéro. Trend : **P1 1 HIGH / 2 MEDIUM / 1 LOW → P2 3 LOW**. ⛔ Aucune remédiation des deux passes ne touche une ligne de code de production : l'E2E de `0734092b` (213 / 10, 7 KF-029 + 3 pollution vérifiée) **reste valable** — le binaire servi est inchangé depuis. |
