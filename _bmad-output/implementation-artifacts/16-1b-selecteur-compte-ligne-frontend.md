...
- **AC1** — `AccountAutocomplete` accepte `allowClear?: boolean` (défaut `false`) : quand `true`, un bouton d'effacement accessible (`aria-label` traduit, respectant `disabled`) appelle `onSelect(null)` (D8). Le bouton **ne remet pas le texte canonique à vide lui-même** — il n'y en a plus qu'un, **dérivé** de `value`. *(Amendé en passe 2 de revue : la refonte structurelle du champ a supprimé l'état `query` mutable et le `$effect` qui l'écrivait. Le bouton se contente désormais de jeter le **brouillon** de saisie, ce qui fait retomber l'affichage sur la valeur liée. L'invariant visé — le champ ne peut pas contredire `value` — est tenu par construction au lieu d'être maintenu à la main.)*
...
- [x] [Review][Decision] **CRITICAL — vider le champ puis cliquer « Enregistrer » persistait l'ANCIEN compte.** La réconciliation était différée de 150 ms ; séquence réelle : `blur` (timer armé) → `click` → `onSubmit` lit les lignes **dans la même tâche**, où le compte valait encore l'ancien. Le `PUT` partait avec l'ancienne valeur pendant que l'écran affichait « suit le défaut société », et la redirection démontait le composant avant l'échéance. **Écriture comptable fausse, en silence, avec l'UI affirmant activement le contraire** — exactement le piège qu'AC1-bis existe pour fermer, rouvert par le délai que j'avais introduit. Aucun test ne pouvait le voir : les quatre tests `allowClear` attendaient `afterBlurDelay()` **avant** d'assertionner, sautant précisément la fenêtre de défaillance. *(Blind Hunter.)*
- [x] [Review][Patch] **HIGH — un `blur` avant l'arrivée des comptes EFFAÇAIT le compte de la ligne.** Tant que `accounts` est vide, le libellé ne se résout pas, le champ paraît vide, et la réconciliation y voyait un effacement volontaire. Le compte disparaissait sans aucun geste destructif, et le placeholder « suit le défaut société » **confirmait** l'effacement à l'écran. *(Edge Case Hunter + Acceptance Auditor.)*
- [x] [Review][Patch] **HIGH — mon patch de passe 1 recréait l'incohérence qu'il prétendait lever.** Le bandeau « défaut société inutilisable » n'exemptait pas `postable` : il se déclenchait donc sur le cas même que 16-1a D3-bis existe pour absorber, en affirmant que « les lignes ne pourront pas être validées » — **faux, le backend accepte**. Dans le même formulaire, avec le même compte, la ligne explicite n'avait aucun marqueur et le bandeau criait l'inverse. *(Acceptance Auditor.)*
- [x] [Review][Patch] **HIGH — ma garde anti-enfermement ne levait pas l'enfermement.** `onSubmit` compare `freshSettings.X !== invoiceSettings?.X` ; avec `invoiceSettings` à `null`, c'est `undefined`, donc **toujours vrai** : tout submit ultérieur retournait « Rechargez la page ». J'avais supprimé la forme silencieuse du blocage, pas le blocage. Mon test s'arrêtait à « le bouton est cliquable » sans jamais vérifier qu'un clic aboutit. *(Acceptance Auditor.)*
- [x] [Review][Patch] **HIGH — marqueur sans blocage quand les réglages échouent.** La garde de la passe 1 n'avait été posée que sur le blocage : le champ criait « Compte invalide » pendant que le bouton restait actif, soit la violation littérale de l'invariant que `account-validity.ts` proclame. *(Blind Hunter + Edge Case Hunter, et trouvé indépendamment par l'orchestrateur avant les rapports.)*
- [x] [Review][Patch] **MEDIUM — « (défaut) » ment sur une facture validée ou annulée.** Le raisonnement qui justifie la fonction jumelle côté avoir — « l'écriture est déjà passée, annoncer un repli serait un mensonge sur une pièce comptable » — vaut **identiquement** côté facture. Or `invoiceRevenueAccountLabel` ne recevait pas le statut. Population réelle : le backfill de 16-1a-bis laisse délibérément à `NULL` les pièces dont l'écriture a été retouchée, et exclut les `cancelled`. J'avais appliqué la bonne règle d'un seul côté sans voir qu'elle valait des deux. *(Edge Case Hunter.)*
- [x] [Review][Patch] **MEDIUM — le message d'AC8 était inatteignable.** Il ne vivait que dans le `title` d'un bouton `disabled`, or `disabled:pointer-events-none` empêche tout survol et l'élément n'est pas focusable. L'AC exige que le message **nomme** les lignes fautives ; il n'était lisible par personne. Rendu en `role="alert"`. *(Blind Hunter + Acceptance Auditor.)*
- [x] [Review][Patch] **MEDIUM — le finding « câblage des écrans de détail » n'était fermé qu'à moitié.** La passe 1 n'avait couvert que `invoices/[id]` ; muter `fetchAccounts(true)` dans `credit-notes/[id]` ne cassait toujours rien — sur le volet **le plus exposé**, un avoir portant par construction des comptes hérités d'une facture ancienne. E2E étendu, mutation vérifiée (`Received: "#7"`). *(Blind Hunter + Acceptance Auditor.)*
- [x] [Review][Patch] **MEDIUM — le sélecteur proposait des comptes qu'il marquait ensuite invalides.** `requiredAccountType` ne filtrait que le marqueur : l'utilisateur pouvait choisir « 4000 — Achats » dans un sélecteur de compte de **produit**, ce qui marquait aussitôt la ligne et désactivait l'enregistrement. L'interface conduisait à l'état bloquant qu'elle venait de créer. *(Blind Hunter.)*
- [x] [Review][Patch] **MEDIUM — les champs n'avaient aucun nom accessible**, et le `placeholder` qui en tenait lieu **ment** : il vaut « … (défaut société) » sur **toutes** les lignes, y compris celles portant un compte explicite différent. La passe 1 avait ajouté la *description* (`aria-describedby`) en laissant le *nom* absent. *(Edge Case Hunter.)*
- [x] [Review][Patch] **MEDIUM — le patch `aria-describedby` de la passe 1 n'avait aucun test** : il était retirable sans qu'aucune assertion ne rougisse. Violation littérale de « un patch de review vient AVEC son test ». *(Blind Hunter.)*
- [x] [Review][Patch] **MEDIUM — « (défaut) » n'était dans aucun catalogue.** Un utilisateur DE/EN/IT lisait du français sur l'écran de détail, alors que le **même concept** est traduit dans le formulaire. *(Blind Hunter + Edge Case Hunter.)*
- [x] [Review][Patch] **MEDIUM — six décomptes faux**, tous nés de patches qui ajoutaient sans mettre à jour : 5 clés → **7**, 12 tests → **20**, 12 → **20**, 9 → **10**, 4 sites → **5**, 9 mutations → **10**. Recomptés à la source. *(Blind Hunter + Acceptance Auditor.)*
- [x] [Review][Patch] **MEDIUM — mon récapitulatif de sévérité contredisait mes propres étiquettes** (« 2 HIGH, 5 MEDIUM, 2 LOW » pour 3 HIGH / 4 MEDIUM / 1 LOW-MEDIUM / 1 LOW), et l'erreur était propagée dans `sprint-status.yaml`. Ce n'est pas cosmétique : les règles d'itération et de splitting arbitrent sur la **trajectoire de sévérité**. *(Blind Hunter.)*
- [x] [Review][Patch] **LOW — numérotation de comptes de l'E2E fragile** : `suffix.slice(-3)` pouvait chevaucher le séparateur et produire un numéro `"NaN"`, et la plage 3200-3499 recoupe le plan comptable seedé. Déplacée hors plan, dérivée du seul tirage aléatoire. *(3 lentilles.)*
- [x] [Review][Patch] **LOW — AC1 énonçait un invariant que le code violait.** La refonte le rend caduc : il n'y a plus de texte canonique mutable, donc plus rien à ne pas écrire. AC amendée. *(Acceptance Auditor.)*

**La refonte, en une phrase** : le texte affiché cesse d'être un `$state` écrit à la fois par un `$effect` et par la frappe, puis « réconcilié » avec retard. Il devient **dérivé** de la valeur liée, avec un brouillon transitoire séparé, et le `blur` devient **immédiat** — possible parce que les items du dropdown préviennent le `blur` par `preventDefault()`. Le délai de 150 ms n'avait d'autre rôle que de contourner cette course : il disparaît, et avec lui le timer jamais nettoyé, le CRITICAL, et l'effacement au chargement.

**Un trou de couverture découvert par une mutation mal choisie.** Ma première mutation de la garde `draft === null` faisait *lever une exception* au lieu d'effacer : le test passait pour la mauvaise raison. Refaite honnêtement (brouillon `null` traité comme chaîne vide), elle laissait **les 15 tests verts** — la garde n'était couverte par rien, alors qu'elle protège le geste le plus banal d'un formulaire : **tabuler à travers un champ sans rien taper**. Test ajouté ; la mutation le tue désormais.

**Confirmation involontaire de l'issue #280** : en écrivant l'E2E côté avoir, l'émission a d'abord échoué parce que j'avais archivé le compte **avant**. C'est exactement le refus arbitré en passe 1 de la 16-1a-bis, observé en conditions réelles.


**Passe 1 de `bmad-code-review`** — 2026-07-31, Sonnet ×3 lentilles (Blind Hunter / Edge Case Hunter / Acceptance Auditor), orchestration + ground-truth Opus 5, diff aplati des 5 commits. **9 findings retenus : 3 HIGH, 4 MEDIUM, 1 LOW-MEDIUM, 1 LOW.** *(Récapitulatif corrigé en passe 2 : il annonçait « 2 HIGH, 5 MEDIUM, 2 LOW », en contradiction avec les étiquettes des 9 items eux-mêmes. Ce n'est pas cosmétique — la § Review Iteration Rule et la § Règle de splitting arbitrent sur la trajectoire de sévérité, qu'un HIGH escamoté fausse.)* L'essentiel porte sur le travail de la passe d'implémentation.

- [x] [Review][Decision] **Le compte de produit par défaut de la société n'était jamais vérifié lui-même** — les lignes à `null` le suivent sans le nommer, donc elles étaient exclues d'office du contrôle par ligne. Le **même compte** rendait un verdict opposé selon qu'il était désigné explicitement (marqué invalide) ou implicitement (rien du tout), et l'utilisateur ne découvrait le problème qu'à la **validation**, rejetée par le backend. **Arbitrage Guy : signaler sans bloquer** — enregistrer un brouillon reste permis puisque le backend l'accepte, c'est la validation qui échouera ; bloquer ici enfermerait l'utilisateur pour un réglage qui n'est pas forcément de son ressort. Avertissement dédié + clé i18n dans les 4 catalogues. *(Edge Case Hunter, HIGH.)*
- [x] [Review][Patch] **RÉGRESSION INTRODUITE PAR L'IMPLÉMENTATION — un échec de `getInvoiceSettings()` enfermait l'utilisateur** [InvoiceForm.svelte] — l'exemption `postable` du défaut société dépend de `invoiceSettings` ; tant qu'il vaut `null` (chargement en cours **ou fetch échoué**), l'exemption est absente, une ligne désignant explicitement un défaut non-postable est marquée invalide, le bouton se désactive… et `onSubmit`, **seul chemin qui rafraîchit les réglages**, ne peut plus s'exécuter. Aucune sortie sans recharger la page. Aggravant : `settingsError` était affecté mais **rendu nulle part** (`grep` ne retournait que la déclaration et l'affectation) — défaut préexistant, mais inoffensif avant que je ne le rende bloquant. Cette régression violait la règle que ce fichier **énonce lui-même** : le frontend ne doit jamais bloquer ce que le backend accepte. Garde ajoutée + erreur rendue. *(Edge Case Hunter, HIGH.)*
- [x] [Review][Patch] **Le câblage des deux écrans de détail n'était testé par RIEN** — l'extraction du helper `account-label.ts` a créé un angle mort : ses 9 tests couvrent la **fonction pure**, jamais la page. Muter `fetchAccounts(true)` → `fetchAccounts()` sur `invoices/[id]` ou `credit-notes/[id]` ne faisait échouer **aucun** test — c'est le piège D11 exactement, déplacé d'un cran, sur ce que la story appelle « le geste de contrôle n°1 d'une fiduciaire ». Fermé par extension de l'E2E : un compte est **archivé après validation**, la page rechargée, le libellé doit rester lisible. **Mutation vérifiée** : `Received: "#7"` au lieu du nom du compte. *(Acceptance Auditor, HIGH.)*
- [x] [Review][Patch] **AC11-bis violée, et ma formulation la masquait** — la spec exige d'ouvrir une **issue GitHub** pour l'écart de parité i18n et de la référencer. J'avais écrit « non traité, **comme la spec l'exige** » : la spec exige de ne pas **traduire**, pas de ne rien faire. L'auditeur l'a vérifié via `gh api` — aucune issue n'existait. **Issue #283** ouverte, écart recompté à **57** clés (fr-CH 1239 vs 1182). *(Acceptance Auditor, MEDIUM.)*
- [x] [Review][Patch] **CONVERGENCE — le bouton d'effacement ne faisait rien quand `value` valait déjà `null`** [AccountAutocomplete.svelte] — `onSelect(null)` ne change alors rien pour le parent, le `$effect` ne se redéclenche pas, et le `preventDefault` du bouton a supprimé le `blur` qui aurait réconcilié le texte. Le champ gardait une saisie libre **contredisant la valeur liée** — l'invariant que le composant proclame en commentaire. Aucun des trois tests `allowClear` ne le voyait : tous démarraient avec `value: 1`. *(Blind Hunter + Edge Case Hunter, MEDIUM.)*
- [x] [Review][Patch] **CONVERGENCE — le message d'invalidité n'était pas lié au champ** [AccountAutocomplete.svelte] — `aria-invalid` posé sans `aria-describedby` : un lecteur d'écran annonce « champ invalide » sans jamais énoncer **pourquoi** (WCAG 3.3.1). `id` + `aria-describedby` ajoutés, via `$props.id()` et non `crypto.randomUUID()` — ce dernier est `undefined` hors contexte sécurisé, donc sur un déploiement LAN en HTTP. *(Blind Hunter + Edge Case Hunter, MEDIUM.)*
- [x] [Review][Patch] **Assertion morte dans mon propre test AC5-bis** [InvoiceForm.test.ts] — `expect(queryByText('Ventes closes')).toBeNull()` ne prouvait rien : le dropdown n'était jamais ouvert, donc l'assertion serait vraie **même si** le compte archivé y était proposé à tort. Corrigée en ouvrant réellement la liste (et en vidant la requête, sans quoi le filtre ne propose rien). *(Blind Hunter, MEDIUM.)*
- [x] [Review][Patch] **Il y a 5 sites de construction de `LineState`, pas 4** [InvoiceForm.svelte] — la branche « facture neuve » d'`initLines()` n'initialisait pas `revenueAccountId`. Sans conséquence (`undefined` se comporte comme `null` partout où le champ est lu), mais l'omission reposait sur cette coïncidence plutôt que sur une intention, et mes commentaires numérotaient « site N/4 ». Initialisation explicite + renumérotation. *(Edge Case Hunter, LOW.)*
- [x] [Review][Patch] **AC9 nomme la suppression de ligne, aucun test ne l'exerçait** [InvoiceForm.test.ts] — test ajouté sur la suppression de la ligne **du milieu**, le cas où un keying par index ferait glisser les comptes d'un cran. *(Acceptance Auditor, LOW-MEDIUM.)*

**Écarté (1)** :

- *Le blocage est inactif pendant le chargement des comptes* (Blind Hunter, MEDIUM) — exact, mais **c'est le comportement correct**. Le backend applique la même validation à `create` et `update` (vérifié par l'Edge Case Hunter) : si l'utilisateur enregistre pendant cette fenêtre, il reçoit un message serveur explicite plutôt qu'un blocage client. Bloquer sur une liste non chargée reviendrait à interdire par ignorance — exactement le défaut corrigé côté réglages ci-dessus.

**4 mutations de vérification**, chacune tuant le ou les bons tests : garde réglages retirée → le test de l'échec de réglages **seul** ; avertissement du défaut désactivé → le test du signalement **seul** ; bouton ne vidant plus sur `value` nul → le test dédié **seul** ; `fetchAccounts(true)` → `fetchAccounts()` **sur l'écran de détail** → l'E2E, avec `Received: "#7"`.
...
**Discrimination des tests prouvée par mutation — 10 mutations, aucune supposition.** *(Le texte annonçait « 9 » pour une table de 10 lignes — erreur de comptage relevée en passe 2 de revue, corrigée ici sans toucher au reste de cette entrée datée.)* Un test vert du premier coup ne prouve rien : il peut l'être parce que le code est correct, ou parce qu'il n'observe rien. Chaque garde de la story a donc été mutée dans le code livré, la suite rejouée, et le fichier restauré.
...
| `revenueAccountId` retiré de `reloadFromServer` (site 4/5 depuis la passe 1 de revue, 4/4 à l'époque) | AC9-bis **seul** |
...
**AC11-bis — la formulation initiale était fausse, et la passe 1 de revue l'a démasquée.** J'avais écrit « non traité, comme la spec l'exige » : la spec exige de ne pas **traduire** dans cette story, **pas** de ne rien faire. Elle demande explicitement d'ouvrir une issue GitHub et de la référencer ici. Ce n'avait pas été fait — l'Acceptance Auditor l'a vérifié via `gh api`. Corrigé : **issue #283** ouverte (`known-failure` + `technical-debt`), écart recompté à **57** clés (fr-CH 1239 vs 1182 pour les trois autres). La 16-1b **n'a pas creusé l'écart** : ses 6 clés nouvelles sont dans les 4 catalogues, vérifié par comptage.
...
- `frontend/src/lib/features/accounts/account-label.test.ts` — **10** tests (AC14-bis).
- `frontend/src/lib/components/invoices/InvoiceForm.test.ts` — **20** tests (AC14). **Premier test de ce composant**, qui n'en avait aucun.
...
- `frontend/src/lib/features/journal-entries/AccountAutocomplete.test.ts` — **20** tests (AC13) + mock i18n reciblé.
...
- `frontend/src/lib/components/invoices/InvoiceForm.svelte` — chargement des comptes, sélecteur par ligne, placeholder, blocage global, **5** sites de `LineState` (T3, T4, T5).
...
- `crates/kesh-i18n/locales/{fr,de,en,it}-CH/messages.ftl` — **7** clés × 4 locales (5 en `dev-story`, +1 en passe 1, +1 en passe 2). *(Recompté à la source en passe 2 : la File List annonçait 5, chaque passe de revue ayant ajouté une clé sans mettre à jour le décompte.)*
