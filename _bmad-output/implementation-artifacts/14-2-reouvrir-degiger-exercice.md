...
- **And (user-facing, finding P3-F1 MEDIUM)** : le **dialogue de confirmation de clôture** ne doit plus mentir sur l'irréversibilité **absolue**. Mettre à jour `fiscal-year-close-confirmation-body` **× 4 locales** (`fr-CH:671`, `de-CH:640`, `it-CH:640`, `en-CH:640`) + le **fallback svelte hardcodé** (`+page.svelte:405`) : retirer *« ne pourra plus être enregistré »* absolu → formulation qui **préserve le garde-fou sans mentir**, p.ex. FR *« Cette action clôture l'exercice : aucune écriture ne pourra plus y être saisie **tant qu'il reste clôturé** ; seul un administrateur peut le rouvrir (motif tracé). Confirmer ? »*. Reformuler aussi `fiscal-year-close-confirmation-action` (`fr-CH:672` *« Clôturer définitivement »*) → *« Clôturer »* (× 4 locales). C'est **exactement** la même hygiène anti-mensonge que P1-H1 (manuel) et D6 (message log-only), appliquée à l'assertion **la plus visible** (montrée à chaque clôture).
...
- [ ] **T6 — Frontend** : `ReopenFiscalYearRequest` (types) + `reopenFiscalYear` (api) ; page `settings/fiscal-years/+page.svelte` — `isAdmin` derived, bouton Réouvrir (`{#if fy.status==='Closed'}` + `{#if isAdmin}`, désactivé + tooltip **nommant le plus proche postérieur clos** si LIFO client, P3-F4), état + `submitReopen()` (clone `submitClose:166-201`), modal motif (mirror `DunningPauseDialog` + champ requis `ManualReminderDialog`) avec `maxlength=REOPEN_MOTIF_MAX` + `clientError` sur vide/trop long (P1-M5). **+ Corriger le fallback svelte hardcodé du dialogue de clôture `:405`** (irréversibilité absolue → « tant qu'il reste clôturé », P3-F1). — AC-E/F/J.
...
Prochaine étape : **Passe 4 (Sonnet ×3, contexte frais, confirmation post-P3-F1)**.
