...
- [ ] **T-C3** Backup auto pré-import (DC5) : **acquérir d'abord le verrou d'installation** (pattern Story 17-1 — `SELECT … FOR UPDATE` sur `_kesh_version` ou `GET_LOCK('kesh_full_import', N)`) avant backup+restore, relâché après COMMIT/rollback (AC13). Puis `build_keshbackup()` (sans audit, O-4) → `KESH_ADMIN_BACKUP_DIR` (défaut `/tmp`, `create_dir_all` si absent ; échec écriture → `500` diagnostique, jamais d'import sans backup). Chemin loggé serveur. Rollback depuis ce backup si restore hors-transaction échoue. (AC: 13, 17)
...
| `MIGRATOR` | `kesh-db/src/lib.rs:22` (`sqlx::migrate!("./migrations")`), run `main.rs:137-141` | **Non utilisé post-restore.** ~~`MIGRATOR.run()`~~ = no-op structurel (AC15 / O-2) : `_sqlx_migrations` destination préservée ⇒ aucune migration rejouée. Compat schéma garantie par le check colonnes bidirectionnel AC12c (T-C2) |
...
| 2026-06-08 | validate Pass 4 | Sonnet 4.6 | **Vérification convergence + régressions des patches P3** : 3 findings (0C+0H+1M+2L). **P4-1 MEDIUM** (régression P3) : ligne tableau Dev Notes `MIGRATOR` disait encore « run post-restore (AC15) » contredisant AC15 réécrit → corrigée. **P4-2 LOW** verrou installation (AC13) non répercuté dans T-C3 → ajouté. **P4-3 LOW** invariant « ≥1 admin source » (MIN=NULL → rollback 500) documenté dans AC16. Ground-truth `role='Admin'` VARCHAR confirmé. Prochaine : Pass 5 Haiku 4.5 (confirmation 0 > LOW attendue). |
| 2026-06-08 | validate Pass 5 | Haiku 4.5 | **CONVERGENCE CONFIRMÉE : 0 finding > LOW.** Patches P3/P4 vérifiés en place par grep (AC16 user_id source, DC11, AC15 MIGRATOR barré, T-C3 verrou, tableau MIGRATOR corrigé). 0 hallucination Haiku. **Cycle CONVERGÉ en 5 passes** Sonnet→Haiku→Opus→Sonnet→Haiku, trend > LOW **21→16→10→3→0** (~50 patches). Spec umbrella **ready-for-dev**. Split A–F acté. Prochaine : `bmad-create-story 17-3a` (story-zéro backend export, pose le format) — la parente passera `split`. |
