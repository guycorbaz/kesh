...
4. **Expression SQL canonique** (constante partagée `pub(crate)` dans `kesh-db`, pas de duplication inline) — **DEUX formes, mêmes 2 arguments d'arrondi** :
   ...
   - **Forme agrégat multi-factures** (`due_dates_summary`) — PAS de sous-requête corrélée dans un `SUM()` externe (piège SQL) : **table dérivée jointe** :
