...
| 2026-06-08 | code-review Pass 1 | Sonnet 4.6 (3 reviewers) | Blind Hunter + Edge Case Hunter + Acceptance Auditor. 0 CRITICAL, 1 HIGH théorique (injection backtick, INFORMATION_SCHEMA = source sûre → garde défensive ajoutée), MEDIUM/LOW. **Patches** : F1 manifest sans `\n` final (`to_vec_pretty` n'en ajoute pas, contraire au format normatif — push `b'\n'` + test) ; **précision sub-seconde** (toutes colonnes timestamp = `DATETIME(3)` ⇒ `%.f` au lieu de sec-only, fidélité backup) ; race fichier temp (compteur atomique `SEQ`) + nettoyage si open échoue + warn si unlink échoue ; `.env.example` `KESH_ADMIN_EXPORT_INMEM_MB` ; test RBAC `Consultation` ajouté ; garde anti-backtick `export_table` ; `admin_backup` → `pub(crate)` (surface secret réduite). **Defer documenté** : pic mémoire in-memory (v0.3), instanceId=0 défensif, ORDER BY absent (SHA intra-export OK), locale fr-CH cosmétique. Tests ciblés verts (3+1+7). Prochaine : Pass 2 Haiku 4.5. |
| 2026-06-08 | code-review Pass 2 | Haiku 4.5 | **CONVERGENCE : 0 finding > LOW.** Tous les patches Pass 1 vérifiés par grep ground-truth (manifest `\n`, `%.f` datetime, `SEQ` atomique + cleanup, garde backtick, `pub(crate)`, anti-PAT). 0 hallucination. 1 LOW résiduel accepté (test exclusion colonne générée couvre table vide pas peuplée — exclusion garantie par construction via `column_names`). **Cycle CONVERGÉ 2 passes** Sonnet→Haiku (trend > LOW : Pass 1 ~4 MEDIUM → Pass 2 0). Status → done. |
