...
- **DC5 — TVA dans les dépenses : HT (défaut)** [À CONFIRMER Guy — non bloquant] : « Dépenses par projet » somme les lignes `Expense` = **montants HT** (la TVA récupérable est postée sur un compte `Asset` 1170, donc hors périmètre `Expense`). Pour un assujetti, le HT est le vrai coût (TVA neutre). Pour un **privé non-assujetti**, il n'y a pas de TVA récupérable séparée (tout est en charge TTC) → le HT = TTC de fait. **Défaut retenu : HT (= somme des lignes Expense)**, cohérent et simple. Le « Coût investi » du rendement inclut les lignes `Asset` taguées → capte la TVA/immobilisations portées à l'actif. *(Si Guy veut le TTC explicite dans Dépenses, extension : ajouter les lignes `Asset` de TVA récupérable taguées — hors v1.)*
...
16. Tests agrégation (`crates/kesh-report/tests/` ou `crates/kesh-api/tests/` — `#[sqlx::test(migrator)]`) : (a) Dépenses — projet avec 2 sous-projets, écritures Expense taguées mixtes → sections + sous-totaux + total corrects ; lignes de contrepartie taguées (banque/TVA) **exclues** (DC1) ; (b) mode cumulé traverse 2 exercices ; (c) Rendement — coût investi (Asset+Expense) + revenus + résultat net + rendement % ; rendement null si coût investi = 0 ; (d) rollup : rapport racine inclut enfants, rapport sous-projet = lui seul ; (e) drill-down : entries contributrices correctes ; (f) projet inconnu/cross-company → 404 ; (g) scope vide (aucune ligne taguée) → rapport vide non-erreur.
...
4. **Lignes de contrepartie taguées** : le tag document-level (19-3/19-4/19-5) tague TOUTES les lignes (charge + TVA + banque/dette). Le filtre `account_type = 'Expense'` (Dépenses) exclut mécaniquement banque/dette/TVA. Vérifier en test (AC16a). Pour le Rendement, `Asset` inclut la banque — **attention** : la ligne banque d'un règlement taguée gonflerait le coût investi. **Mitigation DC** : le coût investi vise les comptes d'**immobilisation/actif d'investissement**, pas la trésorerie. → **Piège critique** : filtrer les comptes `Asset` de trésorerie (classe 10xx banque/caisse) hors « coût investi » ? À trancher : voir Question 2 pour Guy. Défaut prudent v1 : coût investi = `Expense` seul + `Asset` **hors trésorerie** (exclure numéros commençant par `10`), OU documenter que le rendement suppose des immobilisations taguées à l'actif. **Le dev implémente `Expense` + `Asset` et flag ce point ; Guy tranche.**
...
## Questions pour Guy (à confirmer — défauts pris, non bloquants sauf Q2)

1. **DC5 — TVA dans Dépenses (HT vs TTC)** : défaut **HT** (somme des lignes Expense). OK, ou tu veux le TTC (ajouter la TVA récupérable taguée) ? *(Pour un non-assujetti c'est déjà TTC de fait.)*
2. **Piège 4 — Coût investi & trésorerie** : le « coût investi » du Rendement somme `Asset` + `Expense`. Or les lignes de **banque/caisse** (Asset classe 10xx) d'un règlement taguées par propagation gonfleraient le coût. Faut-il **exclure la trésorerie** (comptes 10xx) du coût investi, ou considères-tu que seuls les vrais comptes d'immobilisation/investissement sont taggés à l'actif dans ton usage ? *(Défaut prudent proposé : exclure classe 10xx du coût investi. Impacte le calcul → à confirmer.)*
3. **Drill-down** : niveau écriture inline expandable (AC4) suffit, ou tu veux un lien vers le journal filtré par projet (nécessiterait un filtre projet sur le journal, hors scope v1) ?
