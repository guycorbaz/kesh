...
- [ ] **T1 — Migration & schéma DB** (AC: 1,2,3,5,8,10,11,14 ; DC1,DC3,DC4,DC6)
  ...
  - [ ] **Extension `company_invoice_settings` — TOUS les touch-points** [M5/M6 + O-M1 passe 3] ; en oublier un = bug silencieux (pas un échec de compile) :
    - `repositories/company_invoice_settings.rs` : constante `COLUMNS`, `settings_snapshot_json`, les deux `get_or_create_default[_in_tx]`, **`is_no_op_change()`** (sinon un PUT ne modifiant que ce champ est silencieusement ignoré, version non bumpée), **l'`UPDATE … SET` SQL + le bind** de `update()` (sinon jamais persisté).
...
- **12-2b** — Backend enregistrement (entités + helper `generate_purchase_journal_lines` **dans kesh-db** [O-C1, PAS kesh-core] + `create` single-step + tests repo dont test « TTC achat == TTC règlement == solde 2000 à 0 » [O-M2]).
