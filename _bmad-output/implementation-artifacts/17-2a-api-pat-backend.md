...
### Pass 1 code-review (Opus 4.8 — Blind Hunter + Edge Case Hunter + Acceptance Auditor)

- [x] [Review][Defer] **PAT (Admin, read-write) atteint les routes admin user-management** — `/api/v1/users` (`list`/`create`/`get`/`update`/`disable`/`reset-password`) + `/api/v1/company/invoice-settings` gardées par `require_admin_role` **seul** (`lib.rs:102-126`), sans `ensure_not_pat`. Un PAT `read-write` dont le créateur est Admin peut créer un user / reset un password → auto-propagation partielle contournant DC6. Affecte uniquement les clés créées par un Admin (Comptable bloqué par `require_admin_role`). **Décision Project Lead : catégorie B — accepté comme limitation documentée v0.3 (L3), tracé KF-036 ([#167](https://github.com/guycorbaz/kesh/issues/167), `known-failure`+`technical-debt`+`v0.2-milestone`).** Raison : un Admin créant un PAT read-write lui délègue sciemment ses pouvoirs ; risque réel faible (clés Admin rares, révocation de la clé d'origine reste efficace). Source : edge (ground-truth grep confirmé `lib.rs:102-126` + absence `ensure_not_pat`).
- [ ] [Review][Patch] name > 255 → HTTP 500 au lieu de 400 [crates/kesh-api/src/routes/api_keys.rs:120]
- [ ] [Review][Patch] base62 longueur variable (octets de tête nuls → token 23-27 chars ; entropie ≥160 bits préservée mais claim "fixe ~27" + test `len()>=20` faux) [crates/kesh-api/src/auth/api_key.rs:310]
- [ ] [Review][Patch] `expires_at` dans le passé accepté → clé mort-née (201 + secret mais auth immédiatement exclue) [crates/kesh-api/src/routes/api_keys.rs:117]
