...
- L4. Les 32 fails E2E pré-existants restent à investiguer/réparer dans une story dédiée Epic 10 ou KFs séparées. Triage initial suggère : axe race conditions (au moins 3 tests), bank-* state de seeding insuffisant (au moins 11 tests), autres causes diverses (au moins 18 tests). Non-bloquant Story 10-3 puisque le but T6.4 = "0 régression sur baselines existantes" — la régression n'existe pas (validée adversarialement par stash → toujours fail). La baseline "verte" sur l'ensemble n'a vraisemblablement jamais existé localement post-Epic 8 (les bugs `__dirname` + `testMatch` empêchaient toute exécution complète depuis Epic 8). CI principale ne run pas l'E2E (cf. CLAUDE.md `Test Locally First §E2E (Playwright) — Pas exécuté par la CI principale`). **KFs créées Pass 3 code review (Pass 3 patches commit `313ab94`)** : [#107 KF-030 bank-* baseline regression](https://github.com/guycorbaz/kesh/issues/107) (13 tests Epic 8 territory, seed state insuffisant) + [#108 KF-031 axe race conditions](https://github.com/guycorbaz/kesh/issues/108) (3 tests, hydration Svelte vs axe scan timing). Pour les 14 autres E2E fails (invoices/journal-entries + onboarding/mode-expert) → KFs existantes [#96 KF-028](https://github.com/guycorbaz/kesh/issues/96) + [#97 KF-029](https://github.com/guycorbaz/kesh/issues/97) (Epic 9.5 closure).

- L5. **Pass 4 LOW-P4-2** : `loadI18nMessages()` n'est appelé qu'au boot du root layout `+layout.svelte:onMount` (Pass 3 H3). Si l'utilisateur arrive sur `/login` (unauth), `_messages` reste vide (auth gate `authState.accessToken !== null` skip) → DegradedBanner affiche fallback FR figé peu importe le locale browser. Après login + navigation vers `/(app)/*`, le root layout reste monté (SPA Svelte 5), donc `onMount` ne re-run pas → `loadI18nMessages` n'est jamais appelé pour cette session post-login non plus. **Impact** : utilisateurs DE/IT/EN voient le banner en français pour TOUTE la session. **Correctif v0.2** : (a) faire de `/api/v1/i18n/messages` un endpoint public (suppression du `require_auth`) **OU** (b) appeler `loadI18nMessages` dans `+layout.svelte:onMount` sans auth gate + accepter le path 401→refresh→clearSession comme acceptable au tout 1er boot (alors que les boots subséquents auront un JWT valide). Catégorie B v0.2, à créer en story dédiée Epic 11 ou hors-story dans une issue KF dédiée si non-bloquant.
...
### Code review cycle CYCLE CONVERGED Pass 4 — 2026-05-24

`bmad-code-review 10-3` lancé sur branche `story/10-3-resilience-frontend-db-inaccessible` vs `main` (diff aplati 2125 lignes Pass 2 → 2190 lignes Pass 4 après LOW patches). **Cycle 4-passes adversarial** : rotation `Sonnet 4.6 → Haiku 4.5 → Opus 4.7 → Sonnet 4.6` selon CLAUDE.md §"Review Iteration Rule".

Chaque passe : 3 subagents parallèle (`Blind Hunter` + `Edge Case Hunter` + `Acceptance Auditor`), fresh context, discipline grep ground-truth obligatoire pour CRITICAL/HIGH (§"Haiku-specific guardrails — grep ground-truth obligatoire").

**Trend** :

| Pass | Modèle | Raw findings > LOW | Patches appliqués | Notes |
|------|--------|--------------------|-------------------|-------|
| Pass 1 | Sonnet 4.6 | 5 MEDIUM | 11 (5 MEDIUM + 6 LOW) | F1 vite proxy `/health`, F5+F6 `AbortSignal.timeout(2000)` boot-ping + pollHealth, F10 AC #14bis+#14ter spec, F11 story file Status+checkboxes, F2 TIMEOUT test, F3 testMatch comment, F4 health_endpoint comment, F7 res.body.cancel(), F8 unhandledRejection pattern, F9 E2E timeout 7→10s |
| Pass 2 | Haiku 4.5 | 0 > LOW (6 dismissed via grep ground-truth) | 2 LOW | BH2-L1+ECH2-1 dead code comment rewrite (TS type-narrowing safety), BH2-M1 redundant `if (apiHealth.isDegraded)` removed (clearDegraded idempotent). **6 faux-positifs CRITICAL/HIGH réfutés grep** : 2 auditor refute lui-même (loop bounds + boot-ping timeout), 4 sévérités escaladées rejetées (dead code = LOW pas CRITICAL, dangling pollHealth = LOW pas HIGH, concurrent pollHealth = LOW pas HIGH, AbortSignal Node compat rejet). |
| Pass 3 | Opus 4.7 | **3 HIGH** + 3 MEDIUM | 5 patches + 2 KFs | H1 `clearDegraded` only on `res.ok` (401/4xx/500 ne prouvent rien sur DB), H2 epoch counter `pollHealth` race mitigation, H3 `loadI18nMessages` dans `+layout.svelte:onMount` avec auth gate (AC #17 violation : DegradedBanner FR fallback figé sans cet appel), M2 fetchWithTimeout init.signal override docstring, M1 KFs créées `#107 KF-030 bank-* baseline` + `#108 KF-031 axe race conditions`. **Pattern Opus catch-architectural confirmé** (memory `project_10_2/9_2b_validate_converged`) : 3 HIGH ratés par Sonnet+Haiku car cross-fonction (clearDegraded contract cross-file, race timing 2 acteurs, runtime i18n loading flow). |
| Pass 4 | Sonnet 4.6 | **0 > LOW** ✅ | 4 LOW | L1 comment rephrase api-client.ts dead code, L2 limitation L5 spec i18n post-login gap, L4 typo `CARGO_PKG_MANIFEST_DIR → CARGO_MANIFEST_DIR` spec, L5 KFs #107 #108 references in L4. **CYCLE CONVERGED**. |

**Total cycle** : ~18 patches code + 2 GitHub KF Issues + 9 LOW patches doc/cosmetic + 10 faux-positifs dismissed = **~27 actionable items handled** sur 4 passes, **0 régression** introduite (validation Test Locally First après chaque passe : fmt+clippy+test workspace+npm check+lint-i18n+test:unit 263 ✓+build+E2E db-resilience 3/3 PASS).

**Commits cycle review** (sur `story/10-3-resilience-frontend-db-inaccessible`) :
- `8d625db` fix(story-10-3): Pass 1 code review — 11 patches (5 MEDIUM + 6 LOW)
- `f6fafaa` fix(story-10-3): Pass 2 Haiku review — 2 LOW patches (dead code + redundant guard)
- `313ab94` fix(story-10-3): Pass 3 Opus review — 3 HIGH + M2 doc patches (architectural)
- (à venir) fix(story-10-3): Pass 4 Sonnet — CYCLE CONVERGED + 4 LOW clarity patches

**Decisions finales** :
- Story 10-3 ready pour squash-merge PR : 0 finding > LOW après 4 passes, ~960 lignes net + ~76 lignes patches review.
- Sprint-status : `review` (inchangé, bump `done` au démarrage de la prochaine story per `feedback_avoid_parallel_prs`).
- KFs ouvertes : #107 + #108 (créées Pass 3) à traiter dans Epic 10.5+ ou Epic 11 selon arbitrage.
- Limitation L5 (i18n post-login) → catégorie B v0.2 documentée, à créer KF dédiée si non-traité par Epic 11.
