...
- [ ] **T-A1** Migration `ADD COLUMN email` sur `users` (+ index `idx_users_email`), style `IF NOT EXISTS`. Étendre entité `User` (`pub email`) + les **5 SELECT** retournant `User` : consts `FIND_BY_ID_SQL`/`FIND_BY_USERNAME_SQL`/`LIST_SQL` (`users.rs:14/17/20`) **+ inline `list_by_company:228` + `find_by_id_in_company:256`** (P2 F1) + INSERT `create_in_tx`. **Ajouter `email` à `NewUser` (`entities/user.rs:148`) et corriger les 35 littéraux `NewUser { … }` (26 tests + `setup.rs`/`users.rs`/`bootstrap.rs`)** = `email: None` (F2). (AC: 1, 5)
...
- **Pass 2 (Haiku 4.5, 2026-06-10)** — **1 HIGH, 0 autre > LOW**. **0 hallucination** (toutes les fixes P1 re-vérifiées et confirmées correctes par grep). Patch :
  ...
  - Trend P2 : **1H** → patch appliqué → **Pass 3 (Opus) à lancer**.
