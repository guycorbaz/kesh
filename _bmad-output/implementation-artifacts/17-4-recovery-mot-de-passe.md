...
- **17-4b** Couche email/SMTP + config — module `mail/` (DC1), lib `lettre` (DC2), env vars `KESH_SMTP_*` + `KESH_FEATURE_FORGOT_PASSWORD` + `KESH_PUBLIC_BASE_URL`, fail-fast boot, **trait `Mailer` injectable** (testabilité, no-SMTP en test). N'ajoute que le **champ `mailer` défauté dans le corps de `new_for_tests`** (signature inchangée → call-sites de test intacts ; seuls ~2 littéraux-exception + `main.rs` modifiés, P4-1). **Dépend de 17-4a** (refactor `new_for_tests`), **PAS parallélisable** (P3 F2).
...
- **DC3 — Stockage token : SHA-256 hashé indexé, générateur PAT réutilisé** *(FIGÉE, calque PAT 17-2a DC1 ; résolu validate P1 F5/F14)*. **Réutiliser le générateur existant `crates/kesh-api/src/auth/api_key.rs`** (OsRng + encodage **base62** inline, `PAT_ENTROPY_BYTES`) — **NE PAS** ajouter de dép `base64` ni réinventer (base64 n'est pas une dép directe de kesh-api). Option dev : **généraliser la fn de génération (préféré — ajouter un param `prefix: &str`, ~1 ligne)** ; le fallback « dupliquer » implique en réalité de promouvoir `base62_fixed_width`/`base62_encode` `pub` + ~25 lignes (`api_key.rs:48-88`), pas 6 (P4-3) → préférer la généralisation. Stocké en DB **uniquement** sous forme `SHA-256` hex `CHAR(64)` (`token_hash`, `UNIQUE`, même `sha256_hex` que PAT). Le brut ne vit que dans l'URL de l'email. Entropie ≥160 bits suffit (bearer credential ; pas d'Argon2 per-lookup — cf. 17-2a DC1).
...
- **DC5 — Rate-limiting : réutiliser `RateLimiter` par IP** *(FIGÉE)*. Une **instance distincte** dédiée à forgot-password (seuils propres, ex. 5/15min), montée comme l'instance login (`crates/kesh-api/src/middleware/rate_limit.rs`). Dépassement → `429`. Pas de `tower_governor`. ⚠️ **Sémantique différente du login (validate P3 F1)** : `RateLimiter` n'incrémente QUE via `record_failed_attempt` et `reset()` au succès (`rate_limit.rs:127`/auth login success path). Or forgot-password retourne **toujours `200`** (DC4) → il n'y a pas de « failed » → un simple `check_rate_limit` serait **inerte** (compteur jamais incrémenté, throttle inopérant, anti-énum de masse défait). **MUST** : sur **chaque** requête forgot-password (et reset-password) : (1) `check_rate_limit(ip)` → `429` si bloqué, puis (2) `record_failed_attempt(ip)` **inconditionnellement** (chaque requête consomme un slot de quota), et (3) **NE JAMAIS** appeler `reset(ip)`. **Câblage (P4-2)** : ajouter un **champ dédié `rate_limiter_recovery: Arc<RateLimiter>` à `AppState`** (instance distincte de `rate_limiter` login), défauté dans le corps de `new_for_tests` (P4-1, signature inchangée) + construit dans `main.rs`. **Seuils hardcodés** (5 req / fenêtre 15 min / blocage 30 min) — **non-configurables par env var dans ce cœur** (vars `KESH_RECOVERY_RATE_LIMIT_*` = limitation L5 v0.2+). Responsabilité : **tâche T-C1**.
...
8. **Env vars SMTP** (`config.rs`, patterns `config.rs:467` string / `:701` bool / `:747` int borné) :
   ...
   - `KESH_PUBLIC_BASE_URL` (opt string, ex. `https://kesh.example.com` — **requis si feature on**, sert à construire le lien magique absolu de l'email ; **appliquer `.trim_end_matches('/')`** après trim+filter-empty pour éviter le double-slash `//reset-password`, P4-4),
...
10. **Module `mail/`** (DC1) : `Mailer` **trait** (`async fn send_password_reset(&self, to: &str, reset_url: &str, locale) -> Result<(), AppError>`) + impl `SmtpMailer` (lettre, lit la config SMTP) + impl `NoopMailer`/`MockMailer` (tests, capture le mail sans I/O réseau). Le `Mailer` est porté par `AppState` (`Arc<dyn Mailer>`). **Clés Fluent de l'email (rendu backend, distinctes des clés de page frontend AC22)** : ajouter `email-password-reset-subject` + `email-password-reset-body` (avec placeholder `{ $resetUrl }`/`{ $ttlMinutes }`) **×4 locales** dans `crates/kesh-i18n/locales/{fr,de,it,en}-CH/messages.ftl`, rendues via `config.locale` (DC10) — responsabilité **T-B3/T-B4**, PAS T-D6 (P5). ⚠️ **Explosion struct-literals (validate P1 F3)** : ajouter un champ à `AppState` (`lib.rs:42`) casse les **40 sites** `AppState { … }` (dont 34 fichiers de test + `main.rs:224` + `middleware/auth.rs:267`). Mitigation **convergée (P4-1)** : le champ `mailer` est **défauté `Arc::new(NoopMailer)` dans le CORPS de `new_for_tests` (signature inchangée)** → call-sites de test intacts ; seuls `main.rs` + les 2 littéraux-exception sont modifiés (cf. T-A7/T-B3, stratégie autoritaire). Contenu email formaté inline + i18n Fluent (DC10).
...
15. **Exposition feature-flag (DC9)** : `GET /health` expose `forgotPasswordEnabled: bool` (champ config-dérivé, présent dans les **deux branches** 200/503 de `health.rs` — indépendant de l'état DB) pour que le frontend conditionne l'affichage du lien.
...
- [ ] **T-A7** Refactor de fondation (P3 F2 + P4 P4-1) : migrer les littéraux `AppState { … }` **migrables** vers `AppState::new_for_tests` (`lib.rs:60`) — fait **ici** (avec l'ajout `email` à `NewUser` T-A1) pour éviter toute collision 17-4b/c. ⚠️ **2 exceptions NON-migrables** (gardent un littéral `AppState { }`) car `new_for_tests` hardcode `users_exist=true` (`lib.rs:71`) et un seul `Config` builder : `tests/setup_admin_e2e.rs:81` (a besoin de `users_exist` **variable** pour les tests gate 423) et `src/middleware/auth.rs:267` (`test_state` avec `Config::from_fields_for_test`). `src/main.rs:224` reste littéral (prod). **Stratégie anti-churn (recommandée) : les nouveaux champs `AppState` (mailer en 17-4b, rate_limiter_recovery en 17-4c) seront ajoutés avec un DÉFAUT dans le CORPS de `new_for_tests` (signature inchangée) → les call-sites de test ne changent pas ; seuls les ~2 littéraux-exception + `main.rs` ajoutent les champs manuellement.** (AC: pré-requis 10)
...
- [ ] **T-B3** Module `mail/` (DC1) : trait `Mailer` (`Send+Sync`) + `SmtpMailer` (lettre rustls DC2) + `NoopMailer`/`MockMailer` (test). Ajouter le champ `mailer: Arc<dyn Mailer>` à `AppState`, **défauté `Arc::new(NoopMailer)` dans le corps de `new_for_tests`** (signature inchangée, P4-1) + dans `main.rs` (vrai `SmtpMailer`/`NoopMailer` selon feature) + les **2 littéraux-exception** (`setup_admin_e2e.rs:81`, `auth.rs:267`). Tests mail-capture (17-4e) : muter le champ `pub` `state.mailer = Arc::new(MockMailer)` post-construction. Dép `lettre` (DC2). (AC: 10)
- [ ] **T-B4** `AppError::SmtpSendFailed` + i18n `error-smtp-send-failed` ×4 + **clés email `email-password-reset-subject`/`-body` ×4** (rendu backend, P5). (AC: 10, 11)

### Partie C — Backend endpoints

- [ ] **T-C1** `POST /auth/forgot-password` (public, gated feature) : ajouter champ `rate_limiter_recovery` à `AppState` (défaut dans `new_for_tests` + `main.rs` + 2 littéraux-exception, P4-2) ; rate-limit (`check`+`record` inconditionnel, DC5), lookup DC6, génération token (générateur PAT base62, DC3) + SHA-256, envoi Mailer en `tokio::spawn` détaché, `200` anti-énum, audit si match. (AC: 12)
...
- **Pass 3 (Opus 4.8, 2026-06-10, catch-architectural)** — **1 HIGH, 2 MEDIUM, 2 LOW**. + **8 hypothèses réfutées par ground-truth** (documentées pour ne pas être re-chassées) : SPA deep-link `/reset-password?token=` **fonctionne** (adapter-static `fallback: index.html` + `lib.rs:96/525`), pas de fuite token en logs (aucun `TraceLayer`/`req.uri()`), i18n depuis tâche détachée OK (`Arc<I18nBundle>` global), `/health` additif non-breaking (`api-health.svelte.ts:59` parse permissif), `"password_change"` réutilisable confirmé, ordering fire-and-forget correct, base62 PAT URL-safe, `audit_log.user_id` NOT NULL. Patches :
  ...
  - Trend P3 : **1H/2M/2L** → patches appliqués.
- **Pass 4 (Sonnet 4.6, 2026-06-10)** — **0 CRITICAL, 0 HIGH, 2 MEDIUM, 2 LOW** (régressions de la restructure P3 + sweep final). Vérifié CLEAN : split série a→b cohérent, MUST rate-limiter correct, `generate_pat()` réutilisable (`(token_clair, key_hash)` + `sha256_hex` pub). Patches :
  - **P4-1 MEDIUM** — `new_for_tests` hardcode `users_exist=true` (`lib.rs:71`) → **2 sites non-migrables** (`setup_admin_e2e.rs:81` users_exist variable + `auth.rs:267` Config builder distinct) gardent un littéral. **Stratégie anti-churn adoptée** : nouveaux champs `AppState` (mailer, rate_limiter_recovery) **défautés dans le CORPS de `new_for_tests`** (signature inchangée → call-sites intacts). T-A7/T-B3 corrigés.
  - **P4-2 MEDIUM** — DC5 « instance distincte » sans champ/config/tâche → **champ dédié `rate_limiter_recovery` ajouté à `AppState`** (T-C1), seuils **hardcodés** 5/15min/30min (config = L5 v0.2+).
  - **P4-3 LOW** — DC3 : « dupliquer ~6 lignes » → en réalité ~25 (base62_fixed_width/encode) → préférer généraliser.
  - **P4-4 LOW** — `KESH_PUBLIC_BASE_URL` : `.trim_end_matches('/')` (anti double-slash).
  - Trend P4 : **2M/2L** → patches appliqués.
- **Pass 5 (Haiku 4.5, 2026-06-10)** — **1 MEDIUM, 0 autre > LOW**. **0 hallucination** (6 checks de cohérence P4 confirmés PASS : stratégie défaut-dans-corps `new_for_tests`, 2 exceptions non-migrables ground-truthées, `rate_limiter_recovery`, DC3, ordre série, trailing-slash). Patch :
  - **P5-1 MEDIUM** — Clés Fluent de l'**email backend** (`email-password-reset-subject`/`-body` ×4) non assignées à une tâche (AC22/T-D6 ne couvrent que les pages frontend) → ajoutées à T-B3/T-B4.
  - Trend P5 : **1M** → patch appliqué.
- **Pass 6 (Opus 4.8, 2026-06-10, convergence-confirming)** — **0 CRITICAL, 0 HIGH, 0 MEDIUM, 2 LOW → CONVERGED**. Les **6 checks holistiques PASS** (cohérence whole-spec après 5 rounds ; 17-4a story-zéro indépendamment mergeable + build/tests verts standalone ; flux end-to-end sans maillon manquant ; sécurité incl. side-channel timing INSERT borné par rate-limit = tradeoff standard accepté ; migrations non-breaking ordering OK). 2 LOW cosmétiques appliqués : LOW-1 (texte AC10 périmé aligné sur stratégie P4-1) + LOW-2 (`/health` deux branches 200/503).
  - **VERDICT : CYCLE VALIDATE CONVERGÉ en 6 passes** (rotation Sonnet→Haiku→Opus→Sonnet→Haiku→Opus, **trend > LOW : 12→1→3→2→1→0**, ~26 patches). 0 hallucination sur les passes Haiku (P2 + P5, toutes les vérifs ground-truthées). Catches notables : **P3 Opus catch-architectural F1 (RateLimiter inerte sur endpoint always-200, raté par Sonnet+Haiku)** + P3 F2 (faux parallélisme 17-4a//b sur 18 fichiers test) + P4 P4-1 (`new_for_tests` non-généralisable → stratégie défaut-dans-corps). **12 DC tous FIGÉS, split A–F acté. Spec ready-for-dev.** Prochaine : `bmad-create-story 17-4a` (extraire Partie A) puis `bmad-dev-story 17-4a` (Opus 4.8).
