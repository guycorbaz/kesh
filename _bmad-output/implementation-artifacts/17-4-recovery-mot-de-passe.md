...
1. **Migration `email`** : `ALTER TABLE users ADD COLUMN IF NOT EXISTS email VARCHAR(255) NULL` (dialecte MariaDB, style `migrations/20260419000001_invoice_paid_at.sql`). Index non-unique `idx_users_email (email)` pour le lookup recovery. **Backward-compatible** (existants → `email = NULL`). Entité `User` (`entities/user.rs:108`) étendue de `pub email: Option<String>` (`FromRow` dérivé). ⚠️ **Propagation obligatoire (validate P1 F7)** : `users.rs` n'utilise PAS de constante `COLUMNS` mais **3 constantes SQL string** `FIND_BY_ID_SQL` (`users.rs:14`), `FIND_BY_USERNAME_SQL` (`:17`), `LIST_SQL` (`:20`) qui listent les colonnes en dur → **les 3 doivent inclure `email`** (sinon `query_as::<_, User>` échoue au runtime, colonne absente du result-set). De même l'INSERT de `create_in_tx`.
...
- [ ] **T-A1** Migration `ADD COLUMN email` sur `users` (+ index `idx_users_email`), style `IF NOT EXISTS`. Étendre entité `User` (`pub email`) + les 3 SQL-string-consts `FIND_BY_ID_SQL`/`FIND_BY_USERNAME_SQL`/`LIST_SQL` (`users.rs:14/17/20`) + INSERT `create_in_tx`. **Ajouter `email` à `NewUser` (`entities/user.rs:148`) et corriger les 35 littéraux `NewUser { … }` (26 tests + `setup.rs`/`users.rs`/`bootstrap.rs`)** = `email: None` (F2). (AC: 1, 5)
...
- [ ] **T-B3** Module `mail/` (DC1) : trait `Mailer` + `SmtpMailer` (lettre rustls DC2) + `NoopMailer`/`MockMailer` (test). Câbler `Arc<dyn Mailer>` dans `AppState` + param `new_for_tests` défaut `NoopMailer` + **corriger les 40 sites `AppState { … }`** (F3). Dép `lettre` (DC2). (AC: 10)
...
- **Pass 1 (Sonnet 4.6, 2026-06-10)** — 13 findings : **3 CRITICAL, 5 HIGH, 4 MEDIUM, 1 LOW** (F13 auto-réfuté par l'agent, non compté). **Tous ground-truthés** par l'orchestrateur (grep/read), 0 faux-positif. Patches appliqués :
  ...
  - Trend : **P1 = 3C/5H/4M** → patches appliqués → **Pass 2 (Haiku) à lancer** (LLM≠, contexte frais). Critère d'arrêt : 0 > LOW ou 8 passes (CLAUDE.md Review Iteration Rule).
