...
6. **`invoice.currency = tx.currency` (currency filter, MP3-2 Pass 3 patch)** : ajouté au WHERE du repo pour empêcher qu'une tx EUR 200 et une invoice CHF 200 se croisent en candidate alors que les devises divergent. Le helper `propose_matches` ne calcule pas de currency_score car elles doivent strictly match au repo. Cohérent avec la décision v0.1 mono-devise CHF mais défensif contre les cas mixtes (entreprise multi-devises pré-Story 11 TVA).
    ...
    F: AsyncFnOnce(&mut sqlx::Transaction<'_, sqlx::MySql>) -> Result<T, ReconciliationError>,
{
    ...
    if let Err(e) = release_result {
        tracing::error!(
            ...
            "RELEASE_LOCK failed — caller MUST detach the connection \
             to avoid pool poisoning (lock leak)"
        );
...
**Caller pattern post-`with_account_lock`** : sur `Err(LockReleaseFailed)`, le handler doit `tx.rollback().await.ok()` puis `pool.close().await` ou explicitement `drop(connection)` pour forcer la déconnexion du physique. Pattern à documenter dans T5.3/T5.4.
...
0ter. **Pré-flight `bank_transactions` ownership batch (MP3-3 Pass 3 patch)** : un seul `SELECT id FROM bank_transactions WHERE company_id = ? AND bank_account_id = ? AND id IN (?, ?, ...)` (1 query batch). Vérifier que la liste retournée matche `proposals[*].bankTransactionId` taille pour taille. Si mismatch → `400 Validation` avec `details.reason = "bank_transactions_account_mismatch"` + liste des IDs orphelins. Ferme la fenêtre A3-6 (cross-account proposal dans le savepoint après lock acquis). Pattern : 1 SELECT IN avant lock = O(1) vs N×O(1) après lock.
...
10. **Audit log `invoice.paid` symétrique à Story 5-4 `mark_as_paid` (HP3-3 Pass 3 — dual audit pour cohérence des consumers existants)** :
    - `entity_type = "invoice"`, `entity_id = invoice.id`
    - `details = { paid_at, paid_by_user_id, paid_via: "reconciliation", reconciliation_audit_id, before: { paid_at: null, version }, after: { paid_at, version } }` (snapshot avant/après comme Story 5-4 `invoice_snapshot_json` — `paid_via` discrimine le path d'invocation, `reconciliation_audit_id` link vers l'entrée step 9 pour traçabilité bidirectionnelle).
    - Une entrée par proposal acceptée.
    - **Justification** : tout consumer audit qui filtre `WHERE action='invoice.paid'` (BI dashboards, exports comptables, audit query patterns Story 5-4) doit voir les invoices payées via réconciliation, sinon drift silencieux. Cf. **L35**.
...
3. Pour chaque ID, dans la même tx : vérifier `status == 'pending'` (sinon `failed: [{ error_code: 'RECONCILIATION_ALREADY_RECONCILED' }]`), puis `UPDATE bank_transactions SET auto_match_rejected_at = NOW(3), updated_at = NOW(3), version = version + 1 WHERE id = ? AND company_id = ? AND status = 'pending'`. La condition `status = 'pending'` dans le WHERE est redondante avec le check applicatif mais protège contre une race théorique. Le check `bank_account_id` est déjà fait au pré-flight 1ter.
...
67. **(MP3-2 Pass 3 — currency mismatch filter)** Given tx EUR 200 et invoice CHF 200 dans la fenêtre temporelle/montant, When GET proposals, Then `candidates: []` (le repo filtre `WHERE invoice.currency = tx.currency` strict). *Test E2E HTTP : `post_accept_filters_currency_mismatch`.*
...
- [ ] T3.1 — Créer `crates/kesh-db/src/repositories/reconciliation.rs` :
  ...
  pub async fn find_unpaid_invoices_for_window<'e, E>(
      ...
      tx_date: NaiveDate,
      tx_amount: Decimal,
      tx_currency: &str, // MP3-2 Pass 3 patch
      window_days: i64,
      ...
      E: sqlx::Executor<'e, Database = MySql>,
  {
      ...
      ).bind(company_id).bind(tx_currency)
...
- [ ] T5.6 — Tests E2E HTTP `crates/kesh-api/tests/reconciliation_e2e.rs` (≥ 6) :
  ...
  17. `post_accept_filters_currency_mismatch` (MP3-2 Pass 3 patch — AC nouvelle #67) : tx EUR 200, invoice CHF 200 dans la fenêtre → repo retourne `[]` (filtre currency strict), GET response candidates: [].
