...
**Conséquence acceptée** : la connexion poisoned (lock encore tenu) retourne au pool ; le lock advisory sera libéré au plus tard à la fin de session MariaDB. Cf. **L22** clarifié HP5-7 Pass 5 — timeout effectif dépend du runtime (Docker pod restart ~10s, process long-running `wait_timeout` MariaDB 8h par défaut, configurable).
...
| L38 | Pas de filtre currency au repo (S4-1 Pass 4 CRITICAL revert + LP5-1 Pass 5 dependency tracking explicit) | La table `invoices` n'a pas de colonne `currency` en v0.1 (mono-CHF, multi-devise reportée Story 11). Le filtre currency au repo est garanti implicitement par l'absence de factures non-CHF. AC #67 marqué `#[ignore = "v0.1 mono-CHF, see L38"]` avec body placeholder `panic!()`. **Dépendance Story 11 trackée** : (a) Story 11 doit ajouter colonne `invoices.currency CHAR(3) NOT NULL DEFAULT 'CHF'` ; (b) Story 11 doit ré-ajouter le filtre WHERE au repo `find_unpaid_invoices_for_window` + le paramètre `tx_currency: &str` ; (c) Story 11 doit un-`#[ignore]` AC #67 et le rendre passing avant merge. **Process** : créer une **issue GitHub bloquant Story 11 release** avec template `known_failure.yml` ou `feature_request.yml`, label `dependency:story-8-4`, mentionnant cette spec ligne L38. À ouvrir post-merge 8-4 OU pré-merge Story 11 (selon timing). Risque résiduel : si Story 11 oublie ce trackage, les tx EUR/USD pourraient silencieusement matcher invoices CHF — découvert seulement post-prod. |
