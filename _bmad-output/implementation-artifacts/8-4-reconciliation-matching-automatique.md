    ...
    F: AsyncFnOnce(&mut sqlx::Transaction<'_, sqlx::MySql>) -> Result<T, ReconciliationError>,
{
    ...
    // HP3-1 (Pass 3 review) — RELEASE_LOCK explicite avec gestion de
    // l'erreur. Si le RELEASE échoue (ex. connexion poisoned), on log
    // et on signale un FATAL au caller : la transaction sera rollback
    // par le caller (savepoint OU tx.rollback explicite), mais surtout
    // **la connexion ne doit PAS retourner au pool avec un lock actif**
    // sinon le prochain user hérite du GET_LOCK. Fix : retourner une
    // erreur distincte `LockReleaseFailed` que le caller traite en
    // appelant `connection.detach()` (drop + force ferme la connexion,
    // libérant le lock à la fin de session côté MariaDB).
...
**Caller pattern post-`with_account_lock`** (HP4-1 Pass 4 CORRECTION) : sur `Err(LockReleaseFailed)`, le handler **doit** :
1. Laisser `tx_outer` être drop (le `Drop` impl `sqlx::Transaction` fait un rollback implicite et retourne la connexion au pool).
2. Mapper l'erreur en `500 Internal` côté HTTP.
3. **NE PAS appeler `pool.close().await`** — cette méthode ferme l'**ENTIRE** pool sqlx (toutes les connexions concurrentes), causant un outage API total. C'était une erreur de wording du patch HP3-1 Pass 3.
4. **NE PAS appeler `connection.detach()` directement** — cette méthode est sur `PoolConnection`, pas accessible via `&mut Transaction<'_, MySql>` en sqlx 0.8. La connexion poisoned (lock encore tenu) retourne au pool ; le lock advisory sera libéré naturellement à la fin de session MariaDB (idle timeout par défaut 8h, configurable `wait_timeout`). Cf. **L22** documente ce comportement comme acceptable v0.1.
...
0ter. **Pré-flight `bank_transactions` ownership batch (MP3-3 Pass 3 patch + MP4-2 Pass 4 mitigation TOCTOU)** : un seul `SELECT id FROM bank_transactions WHERE company_id = ? AND bank_account_id = ? AND id IN (?, ?, ...)` (1 query batch). Vérifier que la liste retournée matche `proposals[*].bankTransactionId` taille pour taille. Si mismatch → `400 Validation` avec `details.reason = "bank_transactions_account_mismatch"` + liste des IDs orphelins. Pattern : 1 SELECT IN avant lock = O(1) vs N×O(1) après lock. **Note TOCTOU (MP4-2 Pass 4)** : ce pré-flight ne vérifie QUE l'ownership (account match), PAS le `status = 'pending'` — ce dernier reste vérifié INSIDE le lock dans le helper `accept_one` step 4 pour fermer la fenêtre TOCTOU entre 0ter et step 1 (lock acquisition). Une race entre 0ter (status=pending) et step 4 (status=reconciled par concurrent) résulte en `failed: [{ errorCode: 'RECONCILIATION_ALREADY_RECONCILED' }]` côté response, comportement nominal du savepoint flow.
...
8. **Liaison écriture comptable (H1 Pass 1 patch — redesign v0.1 « link only » + HP3-3 Pass 3 — types & dual audit) — UPDATE inline dans la même tx** :
   ...
   - `UPDATE invoices SET paid_at = ?, version = version + 1, updated_at = NOW(3) WHERE id = ? AND company_id = ? AND version = ?` (`version = ?` = optimistic lock defense-in-depth, cf. **L29**).
...
9. Audit log `reconciliation.accepted` (HP3-2 Pass 3 patch — `entity_type` + `entity_id` spécifiés) :
...
10. **Audit log `invoice.paid` lightweight (HP3-3 Pass 3 + MP4-1+MP4-4 Pass 4 — divergence schéma documentée + LAST_INSERT_ID resolution)** :
    ...
    - **`reconciliation_audit_id` resolution (MP4-1 Pass 4)** : appeler `let reconciliation_audit_id: i64 = sqlx::query_scalar("SELECT LAST_INSERT_ID()").fetch_one(&mut *tx_outer).await?;` IMMÉDIATEMENT après l'INSERT du step 9. `LAST_INSERT_ID()` MariaDB est per-session, retourne l'ID de la dernière `INSERT` sur la connexion courante, n'est PAS perturbé par les SAVEPOINT (cf. doc MariaDB). Valable dans le contexte de `tx_outer` sans risque d'inter-proposal collision car chaque savepoint loop itère séquentiellement.
...
67. ~~**(MP3-2 Pass 3 — currency mismatch filter)**~~ **REFORMULÉ Pass 4 S4-1 CRITICAL** : v0.1 est mono-CHF (PRD §FR3 multi-currency reporté Story 11). La table `invoices` n'a PAS de colonne `currency`, donc le filtre `currency` au repo est unimplementable et inutile. Aucun test ne peut générer une invoice EUR. **Test marqué `#[ignore]`** avec commentaire « Pré-condition Story 11 multi-currency : v0.1 garantit implicitement currency = CHF, test à activer post-Story 11 ». *Test E2E HTTP `post_accept_filters_currency_mismatch` : `#[ignore = "v0.1 mono-CHF, see L38"]`.*
...
71. **(HP3-3 Pass 3 — dual audit invoice.paid)** Given POST accept happy path, When le commit, Then 2 entrées audit_log distinctes : `(action='reconciliation.accepted', entity_type='bank_transaction', entity_id=42)` ET `(action='invoice.paid', entity_type='invoice', entity_id=101, details.paid_via='reconciliation', details.reconciliation_audit_id=<id de la première entrée>)`. Cohérent avec consumers Story 5-4. *Test E2E HTTP : `post_accept_emits_dual_audit_invoice_paid`.*
...
- [ ] T5.2 — Handler `GET /proposals` :
  ...
  3. **Architecture 2-pass (MP4-3 Pass 4 patch — clarification explicite)** :
...
| L38 | Pas de filtre currency au repo (S4-1 Pass 4 CRITICAL revert) | La table `invoices` n'a pas de colonne `currency` en v0.1 (mono-CHF, multi-devise reportée Story 11). Le filtre currency au repo est garanti implicitement par l'absence de factures non-CHF. AC #67 marqué `#[ignore]` — à activer post-Story 11 quand la colonne sera ajoutée. **Risque résiduel** : si Story 11 ajoute la colonne sans propager le filtre au repo `find_unpaid_invoices_for_window`, les tx EUR/USD pourraient matcher invoices CHF. À traquer comme dépendance Story 11. |
