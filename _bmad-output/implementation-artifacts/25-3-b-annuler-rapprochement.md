...
4. **`reconciliation.cancelled`**, **littéral** au site d'insertion, écrit par
   ...
   assumée** : les deux dernières sont écrites par `NewAuditLogEntry::user` (sœur
   `invoice_settlements_write.rs:466`, socle `journal_entries.rs:1659`) et ne portent **pas** la clé API ; seule
...
9. **Propres au rapprochement, écrits une fois.** Famille **`reconciliation-cancel-*`** dans
   ...
   **repli serveur** dépend, lui, de qui refuse : rangs 0 et 2 → `ReconciliationNotCancellable`
   ...
   `PERIOD_LOCKED` (`kesh-api/src/errors.rs:2698`), `OPTIMISTIC_LOCK_CONFLICT` (`:2262`),
   `RECONCILIATION_ACCOUNT_LOCKED` (`:1913`), `RECONCILIATION_LOCK_RELEASE_FAILED` (`:1927`),
   `NOT_FOUND` (`:2257`) — liste **illustrative**, la branche de repli couvrant tout code hors de
   l'union — : le dialogue affiche le **`message` du serveur**, déjà traduit et propre à
...
| Date | Étape | Note |
|---|---|---|
| 2026-09-25 | review P2 | **Une lentille Haiku 4.5** en contexte frais, prompt versionné `25-3-b-review-prompt-p2.md`, sur la remédiation de P1 (diff aplati) **et** l'axe laissé partiel par P1 (ordre des verrous contre huit gestes), axes déclarés geste par geste. **0 finding.** L'orchestrateur a **repris** les deux cycles qui comptaient : encaissement, avoir et dévalidation verrouillent la facture avant l'exercice sans jamais verrouiller une ligne de règlement existante — pas de cycle ; l'acceptation d'une proposition « facture » sur un **autre** compte bancaire (exercice du jour puis facture) forme bien un cycle possible avec le dé-rapprochement (facture puis exercice du jour) — c'est le piège 6-bis, tranché en validation : rejoué côté dé-rapprochement, échec par proposition (`FailedProposal`) côté acceptation, jamais un 500. ⇒ **Boucle close.** **Trend** : P1 Sonnet 1 MED retenu (+1 réfuté, 1 reclassé LOW hors périmètre) → P2 Haiku 0. **Gates finaux** (tête `10776334`) : backend `scripts/test-fast.sh` **2463/2463**, base remise à zéro ; frontend inchangé depuis son gate vert (`check` 0 erreur, lint PASS, **777/777**, build OK) ; E2E, `kesh_e2e` **reconstruite**, run à 18:41 UTC : **220 passés / 9 échoués / 19 ignorés**, zéro régression — 7 KF-029, 1 KF-046 (`sidebar-navigation:75`), 1 pollution (`products.spec.ts:152`, vert rejoué seul deux fois, fichier hors du périmètre de la branche). |
| 2026-09-25 | review P1 | **Trois lentilles Sonnet** en contexte frais, prompt versionné `25-3-b-review-prompt-p1.md`, axes déclarés. *Auditor* : **0 au-dessus de LOW** — les décomptes du Dev Agent Record tous exacts, recomptés depuis la source ; les trois écarts réels et documentés ; textes 6/6 mot pour mot ; manuel vérifié jusqu'au PDF aplati ; 1 LOW (le manuel ne cite pas les deux motifs que l'écran ne produit pas — voulu). *Blind* : ① MEDIUM **retenu** — `cancel_blocker` était du **code mort**, que rien n'appelait, et son doc-comment affirmait servir au geste ⇒ la fonction des motifs est désormais UNE, `cancel_blocker(conn, company_id, &bt)`, appelée par `get_view` et par `cancel_in_tx` (l'écart de signature avec l'AC 3, qui prévoyait un identifiant, est voulu : chaque appelant choisit sa lecture) ; ② MEDIUM **réfuté** — le sélecteur E2E `data-tx-id` préexistait sur la ligne (lu avant modification ; la spec passe et sa mutation est tuée) ; ③ interblocage croisé : connu, documenté, #463. *Edge* : 1 MEDIUM **reclassé LOW, hors périmètre** — les rangs 3-5 délégués au socle lisent un instantané pris à l'étape 4 : le rang 5 est pourtant lu en verrouillant par le socle (`find_open_covering_date … FOR UPDATE`), le rang 3 n'existe que sur état forgé, et le rang 4 (compte archivé) est non verrouillant **dans toute création d'écriture** (`validate_accounts`, `archived_accounts_in_tx`) — fenêtre générale au journal, antérieure, sur un geste rare d'administrateur ; **signalé à Guy**. ⚠️ Axe laissé **partiel** par l'Edge : l'ordre des verrous contre huit gestes — repris en P2. Gate : base remise à zéro, **2463/2463** ; frontend non touché. |
