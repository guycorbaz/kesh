# Prompt — revue de code P2, Story 25-1c-b2 (textes du journal d'audit)

*Versionné le 2026-09-26. **Une lentille** (Haiku), contexte frais. Story de documentation : le
...
`git diff ac0a2fc9 c493cdcc` — **diff unique aplati** : dev de la b2 + remédiation P1 (c493cdcc, à relire en priorité : un patch introduit souvent le défaut suivant). Fiche :
...
## Contexte P1 (ne pas re-signaler, mais vérifier la correction)

P1 a trouvé quatre résidus hors inventaire (`+page.svelte:5`, `api-external.md:81`,
`user-guide/fr/getting-started.md:12`, `website/roadmap.html:259`) et l'accord « elle part » de
`admin-manual.tex:1946`. Vérifie que les corrections sont justes (sens, typographie, accords) et
qu'aucune autre copie du symptôme ne reste. P1 n'a **pas** lu phrase par phrase la sous-section
« Consulter le journal d'audit » ni la note de la numérotation du manuel utilisateur, ni vérifié la
réserve « hors restauration » contre `crates/kesh-api/src/routes/` (import de sauvegarde) : **ce sont
tes priorités.**
