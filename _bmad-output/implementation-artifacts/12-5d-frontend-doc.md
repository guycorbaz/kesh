...
4. **Formulaire de complétion** (**Dialog**, pattern création de `supplier-invoices/+page.svelte` — DC L6) :
   ...
   - Soumission → `completeImport(id, body)` → succès `notifySuccess` + retrait de la liste + **redirection vers `/supplier-invoices/{newId}`** (UX standard, **à implémenter** ; l'id vient de la `SupplierInvoiceResponse` retournée par `/complete`).
