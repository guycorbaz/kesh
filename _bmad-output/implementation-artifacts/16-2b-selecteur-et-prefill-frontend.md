...
`bmad-code-review` **passe 2** — 2026-08-05, **Sonnet** (modèle ≠ passe 1, contexte frais), diff aplati `main...HEAD`. **Un seul finding côté frontend**, classé `defer`. Les findings backend sont dans le story file de 16-2a.

- [x] [Review][Defer] **Un compte `Revenue` non imputable, que D3 accepte délibérément, est INATTEIGNABLE depuis le formulaire article** — `edge`, **MEDIUM reclassé**, **différé vers l'[issue #286](https://github.com/guycorbaz/kesh/issues/286)**. Le filtre des suggestions d'`AccountAutocomplete` exige `a.active && a.postable` (`:200-207`) ; `postableExemptAccountId` n'intervient que dans `isInvalid`, **jamais** dans ce filtre, et la fiche article ne passe pas cette prop. Un compte `Revenue` non imputable — valide selon D3 — ne peut donc **jamais** être choisi depuis l'interface, y compris sur un article vierge.

  **Pourquoi ce n'est PAS un patch** : le filtre est **intentionnel et documenté**, ajouté en passe 2 de revue de **16-1b** parce que, sans lui, « le sélecteur de compte de produit proposait *4000 — Achats*, et le sélectionner marquait aussitôt la ligne invalide en désactivant l'enregistrement — l'interface conduisait l'utilisateur dans l'état bloquant qu'elle venait de créer ». Le comportement de l'UI est **le bon** ; ce que le finding met au jour, c'est que **D3 est plus permissive que l'interface**, écart qui ne se manifeste que par appel direct à l'API.

  **Ce qui est retenu et traité ailleurs** : l'écart lui-même est le **second déclencheur** déjà porté par #286, et l'absence de test verrouillant D3 fait l'objet d'un **patch dans 16-2a**. Refermer l'écart côté backend changerait un comportement hors du périmètre scellé par quatre passes de `validate` — c'est exactement ce que l'arbitrage de la passe 1 a écarté en retenant la variante (c).
...
**2026-08-05 — Passe 3 de `bmad-code-review` — ✅ BOUCLE CONVERGÉE** (**Haiku 4.5**, rotation Opus → Sonnet → Haiku complète, diff aplati). **Aucun finding côté frontend** — les 2 LOW de la passe portent sur la couverture de tests backend et sont consignés dans le story file de 16-2a, avec leur motif de non-retenue.

**Trend des trois passes, les deux stories confondues : `2H/3M/3L` → `0H/4M/2L` → `0H/0M/2L`.** Critère d'arrêt atteint : plus rien au-dessus de LOW.

**Aucun fichier frontend modifié depuis la passe 1.** Le gate frontend (**510/510**, plus `check` / `lint-i18n-ownership` / `build` à 0) et la suite E2E — dont les **4 scénarios de cette story, tous verts** — restent donc valides sans rejeu. La baseline E2E rouge reste ce qu'elle était, démontrée par comparaison avec `main` et tracée (#287 pour le flake `sidebar-navigation`, #96/#97/#107/#108/#124/#282 pour le reste).

**2026-08-05 — Passe 2 de `bmad-code-review`** (**Sonnet**, contexte frais, diff aplati). **Un seul finding côté frontend, classé `defer`** — les quatre patches de cette passe sont backend et vivent dans le story file de 16-2a. **Aucun fichier frontend n'a été modifié** : le gate frontend (510/510) et la suite E2E du commit précédent restent donc valides sans rejeu.

**Le finding différé, et pourquoi il n'est pas un patch.** Un compte `Revenue` **non imputable** — que D3 accepte délibérément côté API — est **inatteignable depuis le formulaire article** : le filtre des suggestions d'`AccountAutocomplete` exige `a.active && a.postable`, et `postableExemptAccountId` n'intervient que dans `isInvalid`, jamais dans ce filtre. Mais ce filtre est **intentionnel et documenté**, ajouté en passe 2 de revue de **16-1b** parce que, sans lui, le sélecteur proposait « 4000 — Achats » et le choisir marquait aussitôt la ligne invalide : *l'interface conduisait l'utilisateur dans l'état bloquant qu'elle venait de créer*. Le comportement de l'UI est **le bon** ; ce que le finding révèle, c'est que **D3 est plus permissive que l'interface**, écart qui ne se manifeste que par appel direct à l'API. Le refermer côté backend changerait un comportement hors du périmètre scellé par quatre passes de `validate` — ce que l'arbitrage de la passe 1 a écarté en retenant la variante (c). Écart versé à l'**[issue #286](https://github.com/guycorbaz/kesh/issues/286)** comme second déclencheur, et consigné dans `deferred-work.md` ; l'absence de test verrouillant D3 est, elle, **corrigée** par un patch de 16-2a.
