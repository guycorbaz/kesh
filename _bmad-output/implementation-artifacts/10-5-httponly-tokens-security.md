...
- [ ] **T6.4** : adapter `login()` : après le `POST /login`, le body de réponse contient désormais `{ access_token, refresh_token, expires_in, user_id, username, role }` (D1 + **D6 actés Pass 2 F-E2E-LOGIN-RESPONSE-USERID-P2-9**). Set directement `_currentUser = { userId: response.user_id.toString(), role: response.role }` + `_expiresIn = response.expires_in` depuis le body — **pas de second round-trip `/me` nécessaire**. Le navigateur a déjà set les cookies HttpOnly via les headers Set-Cookie (T2.1) avant que `login()` lise le body. Si la session est restorée au boot (pas après un login), c'est `hydrate()` (T6.3) qui appelle `/me` à la place.
...
- [ ] **T7.3** : modifier `api-client.ts:doRefresh` (lignes 57-104) — modifications complètes Pass 1 F-AH-P1-2 + F-T7-P1-5 :
  ...
  - **(c)** Adapter la validation de la réponse lignes 87-96 selon **D1 Option A acté T2.1** : si `LoginResponse`/`RefreshResponse` body conserve `accessToken + refreshToken + expiresIn` pour rétro-compat tests, la validation actuelle reste valide. **Mais** `authState.login(data.accessToken, data.refreshToken, data.expiresIn)` ligne 97 ne doit plus stocker en localStorage (T6.1) — vérifier que la fonction `login()` post-T6.1 ne fait que set `_currentUser` + `_expiresIn` (et pas `_accessToken`/`_refreshToken` puisque inaccessibles JS — voir D5 acté T6.5). Choix simple : `login()` après refresh ne fait rien d'observable côté `_currentUser` (déjà set par hydrate au boot), juste update `_expiresIn` pour refresh proactif éventuel.
...
- [ ] **T12.3.a** : **Pass 1 F-AH-P1-6 + F-AH-P1-11** — refactorer le helper central `frontend/tests/e2e/helpers/test-state.ts` (modification CENTRALE impactant 27+ spec files via `clearAuthStorage()` et 7 spec files via `authedApiContext()`) :
  - **`authedApiContext(page)`** (ligne ~150) lit actuellement `localStorage.getItem('kesh:auth:accessToken')` et l'injecte en `Authorization: Bearer`. Post-Story 10-5, choix recommandé : **option (a)** utiliser directement `page` (qui a les cookies HttpOnly via Playwright browser context) au lieu d'un `APIRequestContext` séparé — passer `page.request` aux callers (Playwright `APIRequestContext` partage le storage state du browser context). Ou **option (b)** : faire un login API via `page.request.post('/api/v1/auth/login', ...)` pour récupérer les cookies de la réponse, puis injecter `storageState` dans un nouveau `APIRequestContext`. Recommandation **(a)** plus simple.
  - **`readAccessTokenFromStorage(page)`** (helper interne) : retirer ou marquer dépréqué (puisque le token n'est plus en localStorage). Les 3 spec files qui le consomment via `authedApiContext` sont impactés en cascade.
...
- **D4 — Tests E2E adaptation** : **ACTÉ Option (a) (Pass 1 F-AH-P1-6)** — refactor du helper central `frontend/tests/e2e/helpers/test-state.ts` (impact cascade 27+ spec files via `clearAuthStorage` + 7 via `authedApiContext`). Utiliser `page.request` directement (cookies HttpOnly partagés via Playwright browser context) plutôt qu'un `APIRequestContext` séparé. Voir T12.3.a.
