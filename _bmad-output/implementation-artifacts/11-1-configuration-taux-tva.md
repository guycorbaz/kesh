...
2. **Repository** (`vat_rates.rs`) — nouvelles fonctions calquées sur `repositories/bank_accounts.rs` :
   ...
   - **Advisory lock company REQUIS** (révision : la décision « pas d'advisory lock » de la passe 1 est annulée par l'ajout de `category`) : `acquire_company_sentinel_lock(&mut tx, company_id)` au début de chaque mutation, comme `bank_accounts` (qui l'utilise pour l'invariant cross-row `is_primary`). Ici l'invariant cross-row est le **non-chevauchement par catégorie** : sans le lock, deux insertions concurrentes pourraient créer deux taux « normal » qui se chevauchent.
...
4. **Changement de taux d'une catégorie au fil des années** (AC#5) — modélisé sans mutation destructive : pour passer le **taux normal** de 7.7 % à 8.1 % au 2024-01-01, l'admin **crée un nouveau taux** `category='normal'` (`valid_from = 2024-01-01`) et **clôt l'ancien** `normal` (`PUT valid_to = 2024-01-01`). `find_for_category_at_date('normal', date)` renvoie alors 7.7 % avant 2024 et 8.1 % à partir de 2024. Le champ `rate` d'un taux existant n'est **pas** modifiable (valeur figée — on crée un nouveau taux). L'UI guide ce flux par catégorie.
...
4bis. **(AC non-chevauchement par catégorie)** La création/modification d'un taux est **rejetée** (erreur de validation per-requête) si elle introduit un chevauchement de période `[valid_from, valid_to)` avec un autre taux **actif de la même catégorie** (même `company_id`). Vérifié en transaction sous advisory lock company. Garantit le déterminisme de `find_for_category_at_date`.
5. **(AC changement de taux 7.7→8.1, catégorie normal)** Scénario : taux `category='normal'` 7.70 (`valid_from = 2023-…, valid_to = NULL`) ; l'admin crée un `normal` 8.10 (`valid_from = 2024-01-01`) puis clôture le 7.70 (`PUT valid_to = 2024-01-01`). Vérification via `find_for_category_at_date('normal', date)` : au **2023-12-31** → 7.70 ; au **2024-01-01** → 8.10. Les deux coexistent dans l'historique (`?history=true`). Démontre que l'ancien taux reste applicable aux opérations antérieures, et que la **continuité par catégorie** est préservée année après année.
...
- **Advisory lock company — REQUIS** : avec l'ajout de `category` et son invariant de **non-chevauchement par catégorie** (4bis), `vat_rates` a désormais une contrainte cross-row (analogue à `is_primary` de `bank_accounts`). Chaque mutation acquiert `acquire_company_sentinel_lock(&mut tx, company_id)` en début de transaction, AVANT la vérification de chevauchement — sans le lock, deux insertions concurrentes pourraient créer deux taux de même catégorie aux périodes chevauchantes. Réutiliser le helper de `bank_accounts` (le promouvoir partagé plutôt que dépendre de `bank_accounts.rs`).
