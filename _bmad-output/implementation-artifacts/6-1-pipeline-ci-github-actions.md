...
| 2026-04-16 | Fix CI mid-review — Claude Opus 4.6 (1M context) | **Découverte cachée par l'absence de CI gate** : 84 tests `kesh-db` cassés silencieusement sur `main` depuis ≥ 3 jours (commits `acd7a60`, `e0df67f`, `feat(story-5.1)` tous mergés avec CI rouge avant l'activation de la branch protection). Cause : tests via `test_pool()` (connexion directe DB `kesh`) au lieu de `sqlx::test` éphémère — exigent migrations appliquées + 1 company + 1 user Admin seedés. Correctif : ajout dans le job `backend` de (1) `cargo-binstall` + `sqlx-cli`, (2) `cargo sqlx migrate run`, (3) inline SQL `INSERT INTO companies/users`. `docs/ci.md` mis à jour (section Migrations passée de 3 à 4 endroits + nouvelle section « Seed CI »). Décision Guy : pas de KF, fix in-scope (l'esprit de la story 6-1 est précisément de détecter ces régressions silencieuses). |
| 2026-04-16 | Style fix mid-review — Claude Opus 4.6 (1M context) | `cargo fmt --all` appliqué (7 fichiers : `crates/kesh-api/src/{errors.rs,routes/invoices.rs}`, `crates/kesh-qrbill/src/{generator.rs,pdf.rs,types.rs,validation.rs}`, `crates/kesh-qrbill/tests/golden_test.rs`). Violations préexistantes mergées sur `main` sans CI gate. Aucune modification fonctionnelle. |
