...
- `kesh_reconciliation::manual::build_journal_entry_for_counterparty` (helper public 8-5a-base — utilisé par flow `accept-with-rule` **step 11** §accept-with-rule-flow ; Pass 3 AA-F2 fix cross-reference)
...
8. **Tests** — Unit `kesh-reconciliation::rules` (≥ 6 cas matchType + priorité + skip archived account), Integration `kesh-db::reconciliation_rules` (≥ 7 sqlx multi-tenant + soft-delete + UNIQUE partiel), E2E HTTP `kesh-api` (≥ 28 tests : 13 rules CRUD + 5 application + 10 accept-with-rule, dont AcceptBodyExtractor cohérence + currency filter + audit nullable Pass 4), Vitest (≥ 4), Playwright (≥ 1 actif + 1 a11y).
...
**Currency filter (Pass 3 R1 ECH3-1)** : **les rules HÉRITENT** du filter currency 8-4 (`reconciliation.rs:338` `if tx.currency != "CHF" { continue }`) — les rules **NE s'appliquent PAS aux tx non-CHF v0.1** (cohérent L38 héritée 8-4 mono-CHF). Raison : `build_journal_entry_for_counterparty` (8-5a-base) construit une JE Decimal sans devise, et `journal_entry_lines.amount` est traité comme CHF côté plan comptable suisse. Appliquer une rule sur une tx EUR créerait une écriture comptable faussée (montant EUR stocké comme CHF, sans taux de change). **À implémenter dans le handler `get_proposals`** : le filter currency doit s'appliquer **AVANT** la branche rule fallback (placer le rule-matching APRÈS le `if tx.currency != "CHF" { tx_candidates.push((tx, Vec::new())); continue; }` ligne 338 — la même branche `continue` qui skip non-CHF pour l'invoice matching skip aussi pour les rules). Pas de candidate `type='rule'` retournée pour les tx non-CHF. Defense-in-depth dans `accept_one_rule` step 0 préflight : `if bt.currency != "CHF" → FailedProposal { error_code: 'RECONCILIATION_CURRENCY_MISMATCH', details: { txCurrency } }` per-proposal (évite la JE faussée même si un client bypass via API direct sans passer par GET /proposals). **Limitation v0.1** : multi-currency rules → reporté Story 11 (cohérent L47 8-5a-bis).
...
#### §accept-with-rule-flow — steps 1-16 détaillés (Pass 3 AA-F1)
...
3. **Bank account lookup INSIDE lock** : SELECT inline `bank_accounts.journal_account_id` (pattern `accept_one_split` Pass 1) :
   ...
   ```
   → **`failed: [{ errorCode: 'BANK_ACCOUNT_NOT_CONFIGURED', details: { bankAccountId: <id> } }]`** (per-proposal `FailedProposal` — HTTP global = `200 OK` avec failed[]). **Pattern hérité strict de `accept_one_split`** (cf. `reconciliation.rs:1108-1122` post-merge 8-5a-bis : `Err(FailedProposal { error_code: "BANK_ACCOUNT_NOT_CONFIGURED", ... })`). **Pass 4 ECH4-1 correction** : la version Pass 1 P-C1 disait `412 BANK_ACCOUNT_NOT_CONFIGURED` par analogie avec le standalone `post_manual` 8-5a-base (qui rend bien 412 en AppError global), mais l'architecture batch `accept_one_*` n'autorise PAS d'AppError global — toutes les erreurs per-proposal passent par `FailedProposal`.
...
#### §pass-3-clarifications (Pass 3 Opus R3+R4)

**R3 ECH3-3 — `valueDate` body NON-supporté pour `type='rule'` v0.1 (décision explicite)** : contrairement à `type='split'` (8-5a-bis M4''' Pass 3 Opus) qui supporte `body.value_date.or(tx.value_date).unwrap_or(tx.booking_date)` (3 couches), le variant `Rule` v0.1 **n'a PAS de body field `valueDate`**. Raison : le use-case canonique (Swisscom récurrent, salaires) n'a aucune raison d'override la date — la rule applique tx.value_date naturel. Le variant Rust :
...
Rule {
    ...
    // PAS de `value_date: Option<NaiveDate>` v0.1. Pass 3 R3 décision Guy.
}
```

Le step 8 §accept-with-rule-flow résout `entry_date = tx.value_date.unwrap_or(tx.booking_date)` (2 couches). **Si un client envoie `valueDate` dans le body** : Serde l'ignore silencieusement (champ inconnu, par défaut Serde permet — sauf si `#[serde(deny_unknown_fields)]` est ajouté sur l'enum). **Décision** : ne PAS ajouter `deny_unknown_fields` v0.1 (cohérent avec le pattern existant 8-5a-bis qui n'a pas non plus de deny_unknown). Si v0.2 livre des rules avec override date → ajouter `value_date: Option<NaiveDate>` au variant + 3-couches step 8.

**R4 ECH3-7 — Audit `reconciliation.accepted` `value_date` field nullable** : le shape JSON dans §accept-with-rule-flow step 15 montre `"value_date": "2026-05-15"` qui est l'**`entry_date` résolu** (toujours non-null après step 8). **Décision** : pour cohérence avec `reconciliation.accepted` 8-4 hérité (qui logge `tx.value_date` brut, peut être null), l'audit shape utilise **`tx.value_date` brut** (peut être `null`) — PAS `entry_date` résolu. Re-clarification du shape :
...
```json
{
  ...
  "amount": "150.00",
  "journal_entry_id": 999,
  "value_date": null,         // ← tx.value_date brut (peut être null si tx CSV sans value_date)
  "entry_date": "2026-05-15",  // ← Pass 3 R4 ajout : entry_date résolu pour traçabilité
  "was_previously_rejected": false
}
```

**Ajout du champ `entry_date`** (nouveau Pass 3 R4) — distinct de `value_date` brut. Le test E2E `accept_with_rule_emits_audit_with_null_value_date_when_tx_value_date_is_null` doit asserter `value_date: null` quand `bt.value_date IS NULL` ET `entry_date: <tx.booking_date>` (fallback step 8).

#### §pass-3-clarifications (suite — R5 dedup query block)

**R5 BH-F1 — Suppression duplicate query block dans §rule-application** : Pass 2 Q5 a ajouté `accounts_info: HashMap<i64, (String, String)>` comme query unifiée, mais le block Pass 1 P-H4 original `active_account_ids: HashSet<i64>` est resté en parallèle. **Le dev doit utiliser uniquement `accounts_info`** — `active_account_ids = accounts_info.keys().copied().collect()` si vraiment besoin d'un HashSet typé. Le code snippet ci-dessous remplace les 2 blocks :

```rust
// Pass 3 R5 — query unifiée unique :
let accounts_info: HashMap<i64, (String, String)> = sqlx::query_as::<_, (i64, String, String)>(
    "SELECT id, number, name FROM accounts WHERE company_id = ? AND active = TRUE",
)
.bind(company_id)
...
// `first_matching_rule` filtre par `accounts_info.contains_key(&r.counterparty_account_id)`.
// Si helper signature exige HashSet<i64> (rétrocompat T3.1) :
let active_account_ids: HashSet<i64> = accounts_info.keys().copied().collect();
```

(Le bloc Pass 1 P-H4 SELECT inline `query_scalar::<_, i64>` est obsolète après Pass 3 R5 — supprimer.)
...
| 10 | `bank_account.journal_account_id` NULL au flow accept-with-rule (batch) | **200 (failed[])** | **`BANK_ACCOUNT_NOT_CONFIGURED`** (Pass 4 ECH4-1 correction — per-proposal `FailedProposal`, pattern strict `accept_one_split`. Le standalone `post_manual` 8-5a-base rend 412 en AppError global, mais le batch ne peut pas) |
...
1. Détecter SQL 1062 sur la contrainte `uq_reconciliation_rules_match_active` via le variant existant `DbError::UniqueConstraintViolation(message)` qui contient le nom de la contrainte violée (cf. `kesh-db/src/errors.rs:26` + Q6 §pass-2-clarifications + Pass 4 LOW#5 — variant `DbError::DuplicateRule` PAS créé en T2, on parse le message de `UniqueConstraintViolation`).
2. Le repository `reconciliation_rules` retourne `ReconciliationRulesError::Duplicate { match_type, match_value }` (variant local au module qui wrap le parsing du `UniqueConstraintViolation`).
...
108. **(FR47 — PATCH /rules optimistic lock)** Given rule `version=3`, When PATCH avec `version=2` (stale), Then `409 OPTIMISTIC_LOCK_CONFLICT` (Pass 4 ECH4-2 correction — code canonique du codebase, cf. `kesh-api/src/errors.rs:1080` mapping `DbError::OptimisticLockConflict` → string `"OPTIMISTIC_LOCK_CONFLICT"`. La version Pass 1/2/3 disait `OPTIMISTIC_LOCK_VIOLATION` — string inexistante côté code, aurait cassé le test E2E). Given PATCH avec `version=3`, Then `200 OK` + `version=4` + audit `reconciliation_rule.updated` avec `details.before` / `details.after` champs modifiés. *Test E2E HTTP : `rule_update_uses_optimistic_lock`.*
...
117bis. **(FR47 — rule skip si tx non-CHF — Pass 3 R1 ECH3-1)** Given tx `pending` avec `currency='EUR'` ET rule active match `counterparty_contains:Swisscom`, When `GET /proposals`, Then `candidates[]` ne contient PAS de candidate `type='rule'` (rule silencieusement skip, cohérent avec invoice matching mono-CHF L38 hérité 8-4). ET Given le même contexte, When `POST /accept { type: 'rule', bankTransactionId: <tx EUR id>, ruleId, counterpartyAccountId }` (client bypass GET), Then `failed: [{ errorCode: 'RECONCILIATION_CURRENCY_MISMATCH', details: { txCurrency: 'EUR', expectedCurrency: 'CHF' } }]` per-proposal (defense-in-depth backend). *Tests E2E HTTP : `get_proposals_skips_rule_for_non_chf_transaction` + `accept_with_rule_rejects_non_chf_transaction`.*
...
118bis. **(FR47 — POST accept type=rule bank_account non configuré)** Pass 1 P-C1 ECH-12 + AA-F1 + Pass 4 ECH4-1 correction architecture batch. Given une candidate type=rule sur tx 42 ET `bank_account.journal_account_id IS NULL`, When POST accept `{ type: 'rule', bankTransactionId: 42, ruleId: 7, counterpartyAccountId: 6510 }`, Then **`200 OK`** avec `failed: [{ proposalIndex: 0, errorCode: 'BANK_ACCOUNT_NOT_CONFIGURED', details: { bankAccountId: 17 } }]` (per-proposal `FailedProposal` — pattern strict `accept_one_split` `reconciliation.rs:1108-1122`). Pas de mutation DB (`bank_transactions.status` reste `pending`, `applied_count` reste inchangé). **Note** : distinct du standalone `POST /api/v1/reconciliation/manual` 8-5a-base qui rend 412 en AppError global — le flow batch `accept_one_rule` ne peut pas escalader en AppError sans casser l'architecture (toutes les autres erreurs per-proposal du batch passent par `FailedProposal`). *Test E2E HTTP : `accept_with_rule_returns_failed_when_bank_account_not_configured`.*
...
- [ ] T2.2 — Créer `crates/kesh-db/src/repositories/reconciliation_rules.rs` :
  ...
  - `create_in_tx(tx, company_id, user_id, NewReconciliationRule) -> Result<ReconciliationRule, DbError>` (Pass 4 LOW#5 correction : mappe SQL 1062 sur `uq_reconciliation_rules_match_active` via le variant existant `DbError::UniqueConstraintViolation(String)` `kesh-db/src/errors.rs:26` — parser le nom de la contrainte dans le message. PAS de nouveau variant `DbError::DuplicateRule` à créer en T2 — Q6 §pass-2-clarifications avait acté cette résolution mais T2.2 et §rules-schema étaient restés stale).
...
- [ ] T4.2 — Étendre `crates/kesh-api/src/routes/reconciliation.rs` (du 8-4/8-5a) :
  - Modifier `get_proposals` pour appliquer les rules en fallback (cf. spec d'origine §rule-application). **Insertion explicite entre Pass 3 (contacts batch-load) et Pass 4 (scoring `.map()` sync)** (Pass 4 LOW#8) : (a) `let active_rules = reconciliation_rules::find_active_for_company(&state.pool, company_id).await?;` puis (b) `let accounts_info: HashMap<i64, (String, String)> = sqlx::query_as::<_, (i64, String, String)>("SELECT id, number, name FROM accounts WHERE company_id = ? AND active = TRUE").bind(company_id).fetch_all(&state.pool).await?.into_iter().collect();`. Pass 4 scoring loop dérive `active_account_ids = accounts_info.keys().copied().collect()` sans SELECT supplémentaire (cf. §rule-application R5).
  - Étendre `post_accept` pour gérer `type='rule'` (cf. spec d'origine §accept-with-rule-flow). Body schema validé : `{ type: 'rule', bankTransactionId, ruleId, counterpartyAccountId }`. **4 sites à modifier** dans `reconciliation.rs` (cf. §risques #8a-d) : (a) déclaration enum `AcceptProposalInput::Rule` ; (b) `impl AcceptProposalInput { fn bank_transaction_id() }` (pattern match exhaustif — compilateur catch) ; (c) `accept_batch` dispatch `match proposal { Invoice => accept_one_invoice, Split => accept_one_split, Rule => accept_one_rule }` ; **(d) Pass 4 MEDIUM ECH4-3 — `AcceptBodyExtractor` error message hardcodé `["invoice", "split"]` ligne ~248 `reconciliation.rs` → mettre à jour `["invoice", "split", "rule"]`** (sans (d) les clients reçoivent un message qui prétend `"rule"` n'est pas accepté alors que le handler le supporte — confusion). Pass 3 R2 avait amendé §risques #8d mais T4.2 ne référençait pas explicitement le site (d) avant Pass 4 ECH4-3.
...
- [ ] T4.4 — Étendre `crates/kesh-api/src/errors.rs` (4 nouvelles variants) :
  ...
  - `AppError::ReconciliationRuleMismatch { rule_id }` → mappé en `failed[]` shape (per-proposal error, pas HTTP global — cohérent T3.3 ligne 842 + §accept-with-rule-flow step 4 line 233. Pass 4 LOW#6 annotation : la variant existe pour complétude du `From<ReconciliationError>` mais n'est jamais propagée en 400 global ; `accept_one_rule` retourne `Err(FailedProposal { error_code: "RECONCILIATION_RULE_MISMATCH", ... })`).
  - `AppError::ReconciliationRuleNoLongerMatches { rule_id }` → mappé en `failed[]` shape (per-proposal error, pas HTTP global).

- [ ] T4.5 — Tests E2E HTTP `crates/kesh-api/tests/reconciliation_rules_e2e.rs` *(nouveau)* (≥ 28 tests, Pass 1 énumération + AC #109b/#115b/#118bis + Pass 3 R1/R4 + Pass 4 ECH4-3/4-4) :
  ...
  20. `accept_with_rule_returns_failed_when_bank_account_not_configured` (**AC #118bis nouveau Pass 1, renommé Pass 4 ECH4-1** — assert HTTP 200 + `failed[0].errorCode='BANK_ACCOUNT_NOT_CONFIGURED'`).
  ...
  25. `accept_with_rule_extractor_error_lists_rule` (**Pass 4 ECH4-3** — assert que `AcceptBodyExtractor` retourne un message d'erreur listant `"rule"` parmi les types acceptés quand `type` est absent/inconnu ; couvre le site (d) du §risques #8d).
  26. `get_proposals_skips_rule_for_non_chf_transaction` (**AC #117bis Pass 3 R1 ECH3-1** — currency filter cohérent 8-4 : tx avec `currency != "CHF"` ne déclenche pas de rule candidate).
  27. `accept_with_rule_rejects_non_chf_transaction` (**AC #117bis Pass 3 R1 ECH3-1** — defense-in-depth `accept_one_rule` step 0 préflight `failed[0].errorCode='RECONCILIATION_CURRENCY_MISMATCH'`).
  28. `accept_with_rule_emits_audit_with_null_value_date_when_tx_value_date_is_null` (**Pass 3 R4 ECH3-7** — audit shape : `value_date = tx.value_date` brut nullable, distinct de `entry_date` résolu non-null).

  **Total : 28 tests E2E HTTP** (13 CRUD + 5 application + 10 accept-with-rule). Compte initial spec d'origine ~14, augmenté par Pass 1 à 24 pour couvrir AC #109b/#118bis, par Pass 3 +3 (R1 currency ×2 + R4 audit null), par Pass 4 +1 (ECH4-3 AcceptBodyExtractor). Alignement 1 test = 1 AC ou 1 finding tracé.
...
- `crates/kesh-api/tests/reconciliation_rules_e2e.rs` *(nouveau, ≥ 28 tests — cf. T4.5 enum complet Pass 4)*
...
- **E2E HTTP `kesh-api`** : helper `spawn_app(pool)` (pattern 8-1b/8-2/8-3/8-4/8-5a). ≥ 28 tests T4.5 (cumul Pass 1+3+4).
...
8. **Variant `Rule` à `AcceptProposalInput` (Pass 1 P-H6 ECH-08 + Pass 3 R2 ECH3-2)** : l'enum tagged 8-5a-bis a `Invoice` + `Split`. 8-5b ajoute `Rule { bank_transaction_id, rule_id, counterparty_account_id }`. **4 sites à modifier** dans `reconciliation.rs` : (a) déclaration enum + `#[serde(rename = "rule", rename_all = "camelCase")]` ; (b) `impl AcceptProposalInput { fn bank_transaction_id(&self) -> i64 { ... } }` (pattern match exhaustif — compilateur catch) ; (c) `accept_batch` dispatch `match proposal { Invoice => accept_one_invoice, Split => accept_one_split, Rule => accept_one_rule }` ; **(d) Pass 3 R2 — `AcceptBodyExtractor` error message hardcodé `["invoice", "split"]` ligne ~248 → mettre à jour `["invoice", "split", "rule"]` pour cohérence du message d'erreur 400 quand type absent/inconnu**. Sans le patch (d), les clients reçoivent un message qui prétend `"rule"` n'est pas accepté alors que le handler le supporte — confusion garantie. Test E2E `accept_with_rule_extractor_error_lists_rule` (extension du test discriminator hérité 8-5a-bis).
...
| Date | Entrée | Auteur |
|------|--------|--------|
| **2026-05-14** | **`bmad-code-review` cycle CLAUDE.md COMPLETED — 2 passes Sonnet → Haiku, cycle review STOP** — Status `review` → `done`. Pass 1 Sonnet 4.6 (3 reviewers parallèles : Blind Hunter + Edge Case Hunter + Acceptance Auditor, contexte frais pour briser biais Opus auteur dev-story) — 32 findings bruts → 27 distincts post-dédup (0 CRITICAL + 6 HIGH + 9 MEDIUM + 12 LOW). Verdict initial Acceptance Auditor : NO-GO. **12 patches appliqués** (6 HIGH + 4 MEDIUM + 2 LOW) : [HIGH-1 BH] `unreachable!()` → `tracing::error + AppError::Internal` aux 4 sites match Rule variants (accept_batch/reject_batch/post_split/post_manual) — évite panic Tokio task crash sur futur refactor. [HIGH-2 EC] docstring `accept_one_rule` corrigée "optimistic version check AND version=? au step 13" au lieu de "SELECT FOR UPDATE bank_transaction" (mensonge — serialization repose sur advisory lock). [HIGH-3 EC] DELETE pre-flight déplacé INSIDE transaction — élimine race window concurrent reactivate violant CO Art. 958f audit trail. [HIGH-4 AA] `increment_applied_count_in_tx` SET ajoute `version=version+1` (régression vs spec step 14 + scope-locked §5 + T2.2) + signature retourne `i64` pour audit field. [HIGH-5 AA] audit `reconciliation_rule.applied` ajoute `applied_count_after` (régression vs scope-locked §5 + spec step 16). [HIGH-6 AA] audit `reconciliation.accepted` ajoute `match_type` (régression vs spec step 15). [MEDIUM-3 AA] audit `reconciliation_rule.deleted` shape complet (rule_id + before/after active). [MEDIUM-4 AA] audit `reconciliation_rule.created` shape complet (rule_id + active: true). [MEDIUM-5 AA] Test #24 triple audit assertions complètes (3 actions + applied_count_after + match_type fields asserted). [LOW-1 EC] ReferenceContains fallback filter empty strings (CAMT.053 parser peut produire `Some("")` sémantiquement absent). [LOW-2 EC] score 0.0 → 1.0 dans `AcceptedProposal` rule (cohérence proposal vs accept response). [LOW-3 AA] audit `reconciliation_rule.updated` cleanup non-spec fields + rule_id ajouté. [LOW-4 AA] `rule_match_type` field ajouté ReconciliationCandidate (§api-response-shapes) + frontend type sync. **7 deferred** (test enhancements MEDIUM-1 invoice override positive, MEDIUM-6 Vitest RulesList/RuleFormModal, MEDIUM-7 Playwright a11y page-level, LOW-5 AC #112 GET test, LOW-6 audit count test divergence, + 2 patterns transverses MEDIUM-8 list_by_company_paginated transactional + MEDIUM-9 invoice_id=0 sentinel BH-H1 v0.2 + MEDIUM-10 update_in_tx FOR UPDATE). **3 dismissed** (MEDIUM-2 PATCH self-collision faux positif MariaDB UNIQUE exclut row courant, LOW counterpartyAccountId design optimistic guard, LOW step ordering no impact). Pass 2 Haiku 4.5 fresh-context orthogonal verification : **0 findings > LOW**. Vérification ground-truth indépendante confirmée (4 sites `tracing::error`, `version = version + 1`, `applied_count_after`, `filter(|s| !s.is_empty())` triple, `rule_match_type`). 64/64 tests verts post-patches (28 unit kesh-reconciliation + 8 sqlx reconciliation_rules + 28 E2E HTTP reconciliation_rules_e2e en 56s). **Critère arrêt CLAUDE.md ATTEINT Pass 2** — 0 finding > LOW + verification ground-truth + 0 régression. Cycle review STOP. Budget consommé : 2/8 passes. Verdict final : **`GO done`**. Path-dep aval : aucune (8-5b dernière sub-story Epic 8 — merge clôt Epic). Prochaine étape : push patches + mark PR #87 ready + merge sur main + rétrospective Epic 8. | Claude (Sonnet 4.6 P1 + Haiku 4.5 P2 — cycle code-review) |
| **2026-05-13** | **`bmad-dev-story` Opus 4.7 single-pass continuous COMPLETED** — Status `ready-for-dev` → `in-progress` → `review`. Branche `story/8-5b-reconciliation-rules-engine` depuis main commit `fca3b41` post-merge PR #86 (Pass 4+5 validate cycle). **T1 migration SQL** : table `reconciliation_rules` (12 cols + colonne `active_uniq` VIRTUAL pour UNIQUE partiel Q3 workaround MariaDB ≥ 10.6), 4 CHECK constraints, 2 FK ON DELETE RESTRICT, index `(company_id, active, priority, id)`, `TABLES_TO_TRUNCATE` sync (lesson 8-1b retro). **T2 repository kesh-db** : 7 helpers Executor générique (find_active_for_company, list_by_company_paginated, find_by_id_for_company, create_in_tx avec audit log, update_in_tx avec optimistic lock + UNIQUE reactivation conflict, soft_delete_by_id_for_company idempotent, increment_applied_count_in_tx atomique sans version bump per Pass 1 ECH-10) + `is_duplicate_rule_constraint()` parser. **T2.4 8 tests sqlx** verts (>7 spec). **T3 kesh-reconciliation::rules** : helpers purs `rule_matches` (4 match_types : CounterpartyContains/Exact, ReferenceContains avec fallback `reference || end_to_end_id || transaction_id`, IbanExact normalisé canonique) + `first_matching_rule` (priority ASC + id ASC, skip inactive accounts + inactive rules). **T3.3 4 variants `ReconciliationError`** : RuleNotFound, RuleNoLongerMatches, RuleMismatch, RuleDuplicate + 4 sites match exhaustifs (accept_batch/reject_batch/post_split/post_manual closures avec unreachable branches Rule). **T3.4 8 tests unit** verts (>8 spec) couvrant AC #113, #115, #115b tiebreaker (Pass 2 Q10), #116, #117. **T4.1 routes CRUD** `/api/v1/reconciliation/rules` (~460 lignes) : POST + GET list + GET id + PATCH + DELETE soft idempotent 204, validation handler-side (label/match_value empty + length, priority range 1-1000, IBAN canonical normalize, pre-flight `accounts::find_by_id_in_company` anti-énumération), `map_create_or_update_db_err` via `is_duplicate_rule_constraint` → 409. **T4.2 extension `reconciliation.rs`** : (a) variant `AcceptProposalInput::Rule { bank_transaction_id, rule_id, counterparty_account_id }` + bank_transaction_id() match exhaustif + accept_batch dispatch + (d) Pass 4 ECH4-3 `AcceptBodyExtractor` message `["invoice","split"]` → `["invoice","split","rule"]` + (e) `accept_one_rule` (~290 lignes, 16 steps strict pattern `accept_one_split` FailedProposal per-proposal) + (f) `get_proposals` rule fallback avec refactor `ReconciliationCandidate` discriminator `candidate_type` + pre-load `active_rules` + `accounts_info` HashMap (Pass 4 LOW#8 explicit). **T4.3 mount lib.rs** comptable_routes + authenticated_routes. **T4.4 4 nouveaux AppError variants** + HTTP mapping ReconciliationRuleNotFound 404 + ReconciliationRuleDuplicate 409 + (RuleMismatch/RuleNoLongerMatches 500 fallback défensif jamais émis HTTP global per Pass 4 LOW#6). **T4.5 28 tests E2E HTTP** verts (cf. spec énumération complète Pass 4) : 13 CRUD (#101-#112), 5 get_proposals (#113-#117), 7 accept-with-rule (#118-#122 + #118bis + Pass 4 ECH4-3 AcceptBodyExtractor), 3 Pass 3 R1/R4 (currency filter ×2 + audit value_date nullable). **T5 frontend** : `lib/features/reconciliation/rules/` (rules.types.ts + rules.api.ts + RulesList.svelte + RuleFormModal.svelte + rules.api.test.ts 4 tests Vitest) + extension `ReconciliationProposals.svelte` (badge bleu rule + onAccept dispatche `type='rule'`) + refactor `ReconciliationCandidate` type avec discriminator + page `/reconciliation/rules/+page.svelte`. **T6 i18n 35 clés × 4 locales** (FR canonical + DE/IT/EN traduites basique, traducteur officiel v0.2 limitation L51) + lint-i18n-ownership PASS. **T7 Playwright** : 1 scénario actif (create rule via UI) + 1 axe a11y scan zero violations sur RuleFormModal. **T8 audit shapes** : 4 actions `reconciliation_rule.{created,updated,deleted,applied}` distinctes Q4b + extension `reconciliation.accepted` avec `details.type='rule'` + entry_date résolu + value_date nullable Pass 3 R4. **Validation Test Locally First** : `cargo fmt --all --check` clean, `cargo build --workspace --all-targets` clean, `cargo clippy --workspace --all-targets -D warnings` clean, `cargo test -p kesh-reconciliation --lib` 28/28 verts (8 rules + 20 héritées 8-4/8-5a), `cargo test -p kesh-db --test reconciliation_rules_repository` 8/8 verts, `cargo test -p kesh-api --test reconciliation_rules_e2e -- --test-threads=1` 28/28 verts (53s), `cargo test -p kesh-api --test reconciliation_e2e` 24+1ign verts (0 régression 8-4), `npm run check` 0 errors, `npm run lint-i18n-ownership` PASS, `npm run test:unit` 227/227, `npm run build` clean. **Régressions résiduelles non-bloquantes pré-existantes** documentées sprint-status : (a) `config::tests::*` 20/20 cassent en local lecture `.env` ; (b) `repositories::journal_entries::tests::*` 20/20 cassent en local `FiscalYearClosed` — non liés à 8-5b. **Stats** : 5 commits sur branche `story/8-5b-reconciliation-rules-engine` (T1+T2, T3+T4-errors, T4.1+T4.2(a-e)+T4.3, T4.2(f)+T4.5, T5+T6+T7+style fmt), ~4400 lignes nettes ajoutées (migration + repo + rules engine + routes CRUD + accept_one_rule + 28 E2E HTTP + frontend feature + i18n + Playwright). Prochaine étape : `bmad-code-review 8-5b` cycle CLAUDE.md (Opus auteur → Sonnet P1). | Claude (Opus 4.7 dev-story) |
| **2026-05-13** | **Pass 5 validate Haiku 4.5 — CONVERGENCE CONFIRMÉE** — 0 findings > LOW. 9 patches Pass 4 vérifiés ground-truth via grep direct : (1) AC #108 → `OPTIMISTIC_LOCK_CONFLICT` confirmé (errors.rs:1080+1288) ; (2) AC #118bis → FailedProposal 200+failed[] confirmé (accept_one_split:1108-1122 STABLE) ; (3) §rules-schema + T2.2 → `DbError::UniqueConstraintViolation` parsing message, PAS de variant `DuplicateRule` ; (4) T4.2 site (d) AcceptBodyExtractor:248 explicite + preload `find_active_for_company` + `accounts_info` ; (5) T4.4 RuleMismatch per-proposal annotation cohérente T3.3/step 4 ; (6) T4.5 header `≥ 28 tests` + items 25-28 (`accept_with_rule_extractor_error_lists_rule` + AC #117bis currency ×2 + R4 audit null) ; (7) §scope-locked + source tree + §standards counts alignés `≥ 28`. **Cross-check Pass 5** : pas de référence active à `OPTIMISTIC_LOCK_VIOLATION` / `412 BANK_ACCOUNT_NOT_CONFIGURED` / `DbError::DuplicateRule` / ancien nom de test — toutes occurrences restantes sont historiques (commentaires de correction Pass 4 + Change Log Pass 1/3). AC coverage analysis : 30+ ACs traités, aucun orphan. §i18n-keys (19 clés × 4 locales) + §audit-log-shapes + §api-response-shapes + §pass-N-clarifications cohérents post-patches Pass 4. **Trend cycle complet** : Pass 1 Sonnet = 17 > LOW → Pass 2 Haiku = 7 > LOW → Pass 3 Opus = 0 > LOW (déclaré convergent — FAUX-POSITIF dûment invalidé Pass 4) → Pass 4 Sonnet = 4 > LOW (invalidation Pass 3 + 9 patches) → **Pass 5 Haiku = 0 > LOW (convergence réelle ground-truth)**. **Critère arrêt CLAUDE.md ATTEINT Pass 5** — 0 finding > LOW + vérification ground-truth indépendante main coder. Budget consommé : 5/8 passes. Cycle review STOP. **Verdict Pass 5 : `GO ready-for-dev`**. Path-dep tous helpers code merged STABLE (accept_one_split + AppError::BankAccountNotConfigured + manual::build_journal_entry_for_counterparty + DbError::UniqueConstraintViolation). 8-5b débloqué pour `bmad-dev-story 8-5b` à la reprise de session. Note méthodologique : cycle 5 passes valide l'utilité de la règle CLAUDE.md « LLM différent + contexte frais + boucle jusqu'à 0 > LOW » — Pass 3 Opus avait rubber-stampé 0 > LOW sur un biais convergent (3 modèles avaient hallucinated `OPTIMISTIC_LOCK_VIOLATION` à travers Pass 1/2/3) que seul Sonnet Pass 4 a brisé en grepant les strings réels du code. | Claude (Haiku 4.5 validate — Pass 5) |
| **2026-05-13** | **Pass 4 validate Sonnet 4.6 — INVALIDATION CONVERGENCE Pass 3** — 8 findings (0 CRITICAL / 2 HIGH / 2 MEDIUM / 4 LOW). Pass 3 Opus avait déclaré convergence à 0 > LOW (post-merge PR #85 squash `b81a74b`), Pass 4 contexte frais Sonnet 4.6 + lecture ground-truth code merged refute. **9 patches appliqués** : [ECH4-1 HIGH] AC #118bis + §accept-with-rule-flow step 3 + §error-precedence-order row 10 → bascule de `412 BANK_ACCOUNT_NOT_CONFIGURED` (AppError global) vers `200 + failed[].errorCode='BANK_ACCOUNT_NOT_CONFIGURED'` (per-proposal FailedProposal). Ground-truth `reconciliation.rs:1051+1108-1122+1148` post-merge 8-5a-bis : `accept_one_split` retourne `Result<AcceptedProposal, FailedProposal>` pour TOUTES les erreurs y compris BANK_ACCOUNT_NOT_CONFIGURED. L'architecture batch interdit l'escalade en AppError. Pass 3 Opus avait confondu le standalone `post_manual` 8-5a-base (412 global) avec le context batch `accept_one_rule` (failed[] only). Test renommé `accept_with_rule_returns_failed_when_bank_account_not_configured`. [ECH4-2 HIGH] AC #108 → `OPTIMISTIC_LOCK_VIOLATION` (inexistant côté code) corrigé en `OPTIMISTIC_LOCK_CONFLICT` (canonique `kesh-api/src/errors.rs:1080` + test l.1288). Hallucination string à travers 3 passes — biais Sonnet/Haiku/Opus convergent sur la même fausse intuition. [ECH4-3 MEDIUM] T4.2 ajout explicite du site (d) `AcceptBodyExtractor:248` (Pass 3 R2 avait amendé §risques #8d mais T4.2 ne le listait pas). Test #25 `accept_with_rule_extractor_error_lists_rule` ajouté à T4.5. **Cross-check Pass 4 ground-truth** : `AcceptBodyExtractor:248` toujours `["invoice","split"]` sur main — patch code requis au dev-story 8-5b (Pass 3 R2 documenté en spec mais pas appliqué au code). [ECH4-4 MEDIUM] T4.5 enum complété : 3 tests Pass 3 (R1 currency ×2 + R4 audit null) absents de la liste numérotée — ajoutés en items 26-28. Header `≥ 17 tests` → `≥ 28 tests`. [LOW#5] §rules-schema + T2.2 : suppression mention `DbError::DuplicateRule` (variant jamais créé en T2 — Q6 §pass-2-clarifications avait acté UniqueConstraintViolation parsing mais T2.2 stale). [LOW#6] T4.4 `AppError::ReconciliationRuleMismatch` annotation per-proposal cohérente avec T3.3 ligne 842 + step 4 ligne 233. [LOW#7] §scope-locked + source tree + §standards : test counts stale `≥ 14` → `≥ 28`. [LOW#8] T4.2 placement explicite `find_active_for_company` + `accounts_info` preload entre Pass 3 contacts batch-load et Pass 4 scoring sync. **Ground-truth verifications cross-check** : `accept_one_split:1043-1148` STABLE (FailedProposal pattern strict), `OPTIMISTIC_LOCK_CONFLICT` string canonique STABLE (errors.rs:1080+1288), `AcceptBodyExtractor:248` toujours `["invoice","split"]` NON PATCHÉ (dette code-vs-spec qui doit être levée au dev-story 8-5b). **Trend cycle complet réévalué** : Pass 1 Sonnet = 17 > LOW → Pass 2 Haiku = 7 > LOW → Pass 3 Opus = 0 > LOW (déclaré convergent) → **Pass 4 Sonnet = 4 > LOW** (invalidation convergence Pass 3 par lecture code merged ground-truth). **Critère arrêt CLAUDE.md NON encore atteint** — Pass 5 obligatoire (Haiku 4.5 cycle Opus → Sonnet → Haiku → Opus reprise). Compteur budget LLM : 4/8 passes consommées, reste 4 passes. Verdict Pass 4 : **`NO-GO`** sur la convergence Pass 3 ; spec 8-5b nécessite Pass 5 pour valider que les 9 patches Pass 4 ne réintroduisent pas de régression. | Claude (Sonnet 4.6 validate — Pass 4) |
| **2026-05-12** | **Pass 3 validate Opus 4.7 — VALIDATION FINALE** — 27 findings bruts (BH 13 + ECH 7 + AA 7). Verdict Acceptance Auditor Opus : **CONDITIONAL GO** (0 CRITICAL / 0 HIGH côté Auditor / 2 MEDIUM éditoriaux / 5 LOW). ECH Hunter trouve 1 HIGH résiduel (ECH3-1 currency filter missed Pass 1+2). **7 patches appliqués** (R1-R5 + 2 fixes éditoriaux AA-F1/F2) : [R1 HIGH ECH3-1] §rule-application Currency filter ajouté — rules **HÉRITENT** du filter 8-4 `tx.currency != "CHF"` (skip non-CHF cohérent L38) + defense-in-depth accept_one_rule step 0 préflight `RECONCILIATION_CURRENCY_MISMATCH` per-proposal + AC #117bis nouveau avec 2 tests E2E HTTP. [R2 MEDIUM ECH3-2] §risques #8 ajout 4ème site (d) `AcceptBodyExtractor` message hardcodé `["invoice", "split"]` → `["invoice", "split", "rule"]` (ligne ~248 reconciliation.rs). [R3 MEDIUM ECH3-3] §pass-3-clarifications décision explicite : variant `Rule` v0.1 PAS de body field `valueDate` (cohérent use-case Swisscom récurrent, contrairement à `Split` qui supporte 3-couches override). [R4 MEDIUM ECH3-7] §pass-3-clarifications + step 15 audit shape : `value_date` = `tx.value_date` brut nullable (peut être `null`) + ajout nouveau champ `entry_date` résolu pour traçabilité (distinct des 2). Test `accept_with_rule_emits_audit_with_null_value_date_when_tx_value_date_is_null`. [R5 MEDIUM BH-F1 Opus] §rule-application : suppression duplicate query block résiduel — `accounts_info: HashMap<i64, (String, String)>` est l'unique query batch, `active_account_ids = accounts_info.keys()` dérivé sans SELECT additionnel. [AA-F1 MEDIUM] rename header §accept-with-rule-flow "steps 1-13" → "steps 1-16". [AA-F2 MEDIUM] fix cross-reference §contexte L37 "step 8 helper used" → "step 11 helper used". **Ground-truth verifications confirmées** Opus Auditor : `manual::build_journal_entry_for_counterparty` STABLE (manual.rs:67), `find_strictly_pending_by_id_for_account` STABLE (reconciliation.rs:323), `AcceptProposalInput::{Invoice, Split}` STABLE (reconciliation.rs:178-196), `AppError::BankAccountNotConfigured` STABLE (errors.rs:361), `DbError::UniqueConstraintViolation(String)` STABLE (errors.rs:26+105+152) — **Q6 pre-check pass d'office sans création variant T2**. Faux positifs dismiss : BH-F4 sentinel defer (confirmé Pass 1), BH-F11 step ordering (logique OK), BH-F17 index already correct, ECH-Opus F12 migration filename (placeholder MMDD à résoudre dev-story = OK), F13 numbering tables (cosmétique cross-ref). Trend cycle complet : Pass 1 = ~17 findings > LOW → Pass 2 = ~7 > LOW → **Pass 3 post-patches = 0 HIGH + 0 MEDIUM résiduel** (R1-R5 résolus, AA-F1/F2 résolus). **Critère arrêt CLAUDE.md ATTEINT Pass 3** — 0 finding > LOW. Cycle review STOP. Verdict final : **`GO ready-for-dev`**. Path-dep 8-5a-bis MERGED PR #83 squash `3c996fb` + sync PR #84 `f759dab` — tous helpers ground-truth STABLE. 8-5b débloqué pour `bmad-dev-story 8-5b`. | Claude (Opus 4.7 validate — VALIDATION FINALE) |
