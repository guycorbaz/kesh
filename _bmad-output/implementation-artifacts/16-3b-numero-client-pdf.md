...
**Une seule formulation, et c'est délibéré** : `assert_eq!(y_none - y_some, 4.5)` — le delta *doit* valoir 4,5 ; la mutation le met à 0.

⚠️ **Une variante « ordonnée absolue codée en dur » a été proposée puis RETIRÉE.** Elle annonçait « en `None`, ordonnée échéance exactement 265,5 » — **c'est faux** : sur le fixture par défaut (`origin_reference: None`, `pdf.rs:1038`), la chaîne réelle est `277` (titre) → `−7 =` `270` (n° facture) → `−4,5 =` **`265,5` (date)** → bloc origine sauté → `−4,5 =` **`261,0` (échéance)**. `265,5` est la position de la **date**. Un développeur suivant cette variante aurait écrit une assertion qui **échoue sur du code correct** — le mode d'échec que cet AC dénonce.

Et même corrigée à `261,0`, la variante resterait **fragile** : la valeur dépend du fixture (avec `origin_reference: Some`, elle redescend encore de 4,5). Le delta, lui, est **invariant**. D'où le retrait plutôt que la rustine.

⚠️ **C'est la TROISIÈME erreur de calcul sur ce bloc** (après `252` vs `256,5`, et le décompte des lignes de `my`), toutes du même type : un chiffre posé sans être dérivé du code. Sur ce bloc, **ne jamais écrire une ordonnée sans la recalculer depuis la chaîne de décréments de `pdf.rs:287-339`**.
