...
```sql
singleton_role VARCHAR(32) AS (CASE WHEN active AND role IN (<les 8 singletons>) THEN role ELSE NULL END) STORED
...
- **Given** la table `accounts` existante, **When** la migration s'applique, **Then** elle gagne :
  ...
  - `singleton_role VARCHAR(32) AS (CASE WHEN active AND role IN (<les 8 singletons>) THEN role ELSE NULL END) STORED` — le `active AND` est **obligatoire** (§Décision unicité) ;
  - `CONSTRAINT uq_accounts_company_singleton_role UNIQUE (company_id, singleton_role)`.
...
- **Cohérence des deux `AccountRole`** (`kesh-core` vs `kesh-db`) : mêmes variantes, même `as_str()`, `FromStr` réciproque — le garde-fou de la duplication imposée par l'orphan rule.
...
`NewAccount` gagne deux champs ; un littéral de struct Rust ne tolère pas de champ manquant (`#[serde(default)]` n'agit que sur la désérialisation JSON, jamais sur un littéral). **29 occurrences réparties sur 16 fichiers** cassent `cargo build --workspace --all-targets` — le **premier** des 4 checks obligatoires. Inventaire (`grep -rn "NewAccount {" crates/ --include=*.rs`) :
...
- [ ] **T5** `repositories/accounts.rs` : `COLUMNS:17` **et** `FIND_BY_ID_SQL:19`, `account_snapshot_json:27-38`, `is_no_op_change:164-166`, `create:41`, `update:170` (UPDATE SQL + binds), `bulk_create:354`, `bulk_create_from_chart:421` (rôles du chart + calcul de `postable`) — AC-C/F.
...
- [ ] **T13** Mettre à jour les **~29 littéraux `NewAccount { … }`** des 13 fichiers de test listés en AC-G (`role: None, postable: true`, ou constructeur dédié). À faire **tôt**, pas au moment du gate — AC-G.
...
3. **La liste des rôles singleton existe à deux endroits** : le `CASE WHEN active AND role IN (…)` de la colonne générée (SQL, figé par la migration) et `AccountRole::is_singleton()` (Rust). Ils **doivent** rester synchrones. Commentaire croisé obligatoire dans les deux fichiers. Une divergence donne soit un 409 inexpliqué, soit une unicité non appliquée.
...
6. **La migration ne peut pas être `ALGORITHM=INSTANT`** à cause de la colonne générée `STORED` (`ERROR 1845` vérifié). Ne pas copier-coller le pattern de `20260531000001_bank_accounts_archived.sql:15-17` sans réfléchir : il échouerait.
