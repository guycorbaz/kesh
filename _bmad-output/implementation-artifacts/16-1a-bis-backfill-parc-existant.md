...
- **AC-B5** — `docs/migrations-idempotence-audit.md` : ligne ajoutée au tableau détaillé avec verdict **`tracked-by-sqlx`** (justifié par l'absence d'`IF NOT EXISTS`, **pas** par le backfill, qui est idempotent — D-B6), **ET** récapitulatif agrégé de bas de fichier mis à jour en cohérence (`Total` et `Idempotence tracked-by-sqlx` chacun +1). L'invariant « Idempotence `no` : 0 » est **préservé**. Garde-fou **P5** de `CLAUDE.md`.
  - **Le nombre de migrations apparaît à TROIS endroits de ce fichier**, pas deux : l'**en-tête de section** `## Table d'audit (N migrations)`, la ligne `Total` des « Statistiques », et le compteur `tracked-by-sqlx`. Les trois se **recomptent depuis le tableau**. *(Ajouté en passe 1 de `bmad-code-review` : l'en-tête était resté à 56 pour 57 réelles, seul site oublié des trois — la dérive exacte que P5 existe pour empêcher.)*
