...
- **Renderers PDF/CSV** : `render_vat_report_csv` (`csv.rs`, écrit déjà une ligne « TVA récupérable » l.40 +
  « Solde ») et `render_vat_report_pdf` (`pdf.rs`, libellés `vat_recoverable`/solde) **rendent déjà ces
  champs** depuis la struct. **Aucun changement** : ils s'allument automatiquement quand `generate()` remplit
  la valeur. (Vérifier en test que le CSV/PDF affichent la valeur réelle.)
...
- **AC7** — **PDF/CSV** : les exports TVA (`render_vat_report_pdf`, `render_vat_report_csv`) affichent la
  **valeur réelle** du récupérable et du solde (aucun code à changer — vérifié par test que la valeur n'est
  plus `0` quand le compte a un solde).
- **AC8** — **Tests** (intégration `#[sqlx::test]`, niveau `vat_report`) :
  ...
  - (c) écriture sur `1171` **hors** période (entry_date avant start / après end) → **exclue** ;
  - (d) `default_vat_recoverable_account_id == NULL` → `total_vat_recoverable == 0` ;
...
- **T-D2** — Brancher dans `generate` (remplacer l.109-110) : lire
  ...
  Conserver le scoping anti-IDOR `company_id`.
- **T-D3** — Front : retirer la note « à venir » de `VatReportView.svelte` (bloc `reports-vat-recoverable-note`
  l.71-74). Vérifier que le libellé du solde est explicite (« Solde net dû à l'AFC » ou équivalent i18n). Ne
  PAS toucher le tableau des taux ni les autres champs.
- **T-D4** — Tests d'intégration `vat_report` (AC8 a-g). Réutiliser le pattern de fixtures `vat_report_e2e.rs`
  / `invoices_validate_vat.rs` (seed company + compte `1171` via fixture 18-1c). Pour créer les écritures
  `1171`, utiliser `journal_entries::create` (écriture manuelle) ou l'helper de validate. **Le compte
  `default_vat_recoverable_account_id` est déjà configuré dans la fixture (= compte `1000`, Story 18-1c)** —
  ⚠️ donc en test le « compte préalable » est `1000` (réutilisé) : poster les écritures de test sur
  `seeded.accounts["1000"]` et vérifier le solde. Pour le cas (d) NULL, mettre le champ à NULL via UPDATE.
