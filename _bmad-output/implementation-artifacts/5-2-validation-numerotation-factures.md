...
- [x] T6.6 Tests Vitest `invoice-number-format.test.ts` : mirror des tests Rust côté frontend pour le helper `previewInvoiceNumber`.
...
| 2026-04-14 | 0.8     | Code review pass 2 (Haiku × 3 — Blind / Edge / Auditor). 20 findings bruts → 14 rejects (faux positifs vérifiés factuellement : `journal_entries.description = VARCHAR(500)` pas 128, `FOR UPDATE` résout déjà TOCTOU, `[...str]` TS ≡ Rust `chars()`, etc.) + 1 defer (TD-5.2-4) + **5 LOW patches** polish. Acceptance Auditor : **PASS 0 findings**. **Convergence atteinte** (règle CLAUDE.md : findings ≤ LOW). Patches polish : Q1 suppression code mort TS, Q2 guard `fy.name` contre `{`/`}` dans render(), Q3 migration CHECK `status='validated' ⇒ journal_entry_id NOT NULL` (defense-in-depth), Q4 variant dédié `FormatError::InvalidSeq(i64)`, Q5 commentaire explicatif sur double-check status. fmt+clippy+tests verts. Status → **done**. | Claude Haiku 4.5 (review) + Claude Opus 4.6 (patches) |
| 2026-04-14 | 0.7     | Code review pass 1 (Sonnet × 3 — Blind Hunter / Edge Case Hunter / Acceptance Auditor). 34 findings bruts → 14 patches + 11 defer + 4 rejects. Patches appliqués : **P1** FOR UPDATE sur `get_or_create_default_in_tx` (TOCTOU config, HIGH). **P2** worst_case_len `{SEQ}` i64 19 chiffres + `{SEQ:NN}` max(n,19) Rust+TS (HIGH). **P3** `validate_invoice` retourne `lines`, handler skip re-fetch post-commit (MEDIUM). **P4** validation description_template frontend + feedback inline (MEDIUM). **P5** redirection post-save InvoiceForm → vue détail (expose bouton Valider, MEDIUM). **P6** défaut `due_date = invoice.date` dans create/update (MEDIUM). **P7** clé i18n `invoice-validate-success-body`. **P8** cocher T6.6. **P9** previewInvoiceNumber TS short-circuit SEQ:NN hors borne. **P10** contact introuvable → NotFound (404) au lieu de Invariant (500). **P11** whitelist control chars dans `validate_description_template` Rust+TS. **P13** dialog close sur FISCAL_YEAR_INVALID / CONFIGURATION_REQUIRED / OPTIMISTIC_LOCK. **P14** `render()` guard seq < 1. **Edge#13** fallback toast si invoiceNumber null. **Edge#17** `validate_description_template` chars().count() vs bytes.len(). fmt + clippy 0 warning, 17 tests core + 13 Vitest passent, svelte-check 0 erreur. | Claude Sonnet 4.6 (review) + Claude Opus 4.6 (patches) |
