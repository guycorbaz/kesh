...
- **DC5 — Extension `audit_log` non-breaking.** Ajout `actor_type ENUM('user','api_key') NOT NULL DEFAULT 'user'` + `actor_api_key_id BIGINT NULL`. `DEFAULT 'user'` rend la migration non-breaking pour les lignes existantes ET pour les call-sites Rust si on ajoute les champs avec `#[serde(default)]` / valeurs par défaut. ⚠️ **Impact ripple** : `NewAuditLogEntry` (kesh-db) est consommé par ~tous les handlers qui auditent — l'ajout de champs casse la compilation de chaque call-site sauf si on fournit un constructeur rétro-compatible (`NewAuditLogEntry::user(...)` conservant la sémantique actuelle + `NewAuditLogEntry::api_key(...)`). Voir T-A4.
...
4. **AC4 — `require_auth` accepte JWT cookie OU Bearer PAT** : le middleware `crates/kesh-api/src/middleware/auth.rs::require_auth` est étendu pour, en l'absence de JWT valide (cookie `kesh_access_token` ou `Authorization: Bearer <jwt>`), tenter de valider un **PAT** si `Authorization: Bearer kesh_pat_…` est présent. Un PAT valide produit le **même** `CurrentUser` injecté dans les extensions (aucun changement aux handlers ni au router). Discrimination JWT vs PAT par le **préfixe `kesh_pat_`** du bearer. Token absent/invalide/révoqué/expiré → `401 UNAUTHENTICATED`.
...
7. **AC7 — Routes `/api/v1/settings/api-keys`** (guard `require_comptable_role`, DC4) :
   - `GET /api/v1/settings/api-keys` → liste des clés de la company (`{ id, name, scope, createdAt, lastUsedAt, revokedAt, expiresAt, version }`, **jamais** le hash ni le secret).
...
8. **AC8 — Audit-trail PAT (DC5, issue §6, OLICo Art. 9)** : (a) la **création** et la **révocation** d'une clé sont auditées (`actor_type='user'`, action `api_key.created` / `api_key.revoked`, `details_json` avec `name` + `scope`, **jamais** le secret) ; (b) toute action **mutante via PAT** sur les routes métier logge `actor_type='api_key'`, `actor_api_key_id=<id clé>`, `user_id=<créateur>` (responsabilité conservée). Extension `audit_log` : colonnes `actor_type` + `actor_api_key_id` ajoutées (migration non-breaking, DEFAULT `'user'`), `NewAuditLogEntry` étendu avec constructeurs rétro-compatibles (`::user(...)` = comportement actuel, `::api_key(...)`). Aucun call-site d'audit existant ne change de comportement (tous restent `actor_type='user'`).
...
- [ ] **T-A3 — Repo `api_keys`** (AC: #2, #3)
  ...
  - [ ] Génération token : helper `generate_pat()` (RNG crypto ≥ 160 bits → base62, préfixe `kesh_pat_`) + `sha256_hex(token)` — emplacement `crates/kesh-api/src/auth/` (nouveau `api_key.rs` ou `pat.rs`). Vérifier crate `sha2` (ajouter au Cargo.toml si absent) et `rand`/`base62`. **DRY** : ne pas réinventer le RNG (réutiliser `OsRng` déjà importé dans `auth/password.rs`).
...
- [ ] **T-A4 — Extension `audit_log` (actor_type / actor_api_key_id)** (AC: #8) ⚠️ **ripple — voir DC5**
  ...
  - [ ] Adapter `repositories/audit_log.rs::insert_in_tx` (INSERT des 2 nouvelles colonnes) **sans** changer la signature publique si possible (le `NewAuditLogEntry` porte déjà les nouveaux champs). Mettre à jour **tous** les call-sites existants vers `NewAuditLogEntry::user(...)` (refactor mécanique — `grep` les usages de `NewAuditLogEntry {`).
...
- [ ] **T-A5 — Middleware auth factorisé (JWT OU PAT)** (AC: #4, #5, #6)
  ...
  - [ ] Étendre `middleware/auth.rs::require_auth` (L66-140) : si pas de JWT valide ET bearer commence par `kesh_pat_` → `validate_pat`. Injecter `CurrentUser`. Appliquer le **gate de scope** (DC3) : si PAT `scope=Read` et `req.method()` mutante → `403 API_KEY_READ_ONLY`. JWT cookie → comportement inchangé.
...
- [ ] **T-A6 — Routes CRUD `/settings/api-keys` + audit création/révocation** (AC: #7, #8)
  ...
  - [ ] Enregistrer les routes dans `crates/kesh-api/src/lib.rs` sous `comptable_routes` (avant le `.route_layer(require_comptable_role)`, calque enregistrement `bank_accounts` L253-262).
...
- [ ] **T-A7 — Tests backend** (AC: #13)
  ...
  - [ ] Integration (`#[sqlx::test]`) : CRUD scopé company (isolation cross-company → NotFound), `find_active_by_key_hash` (révoqué/expiré exclus), middleware PAT (Bearer valide → 200 ; read + POST → 403 ; créateur désactivé → 401 ; token inconnu → 401), audit création/révocation + audit mutation via PAT (`actor_type=api_key`).
...
**Auth & middleware** :
- `crates/kesh-api/src/middleware/auth.rs:66-140` — `require_auth` : extrait JWT du cookie `kesh_access_token` ou header `Authorization: Bearer`, décode (HS256, ~1-2 ms), injecte `CurrentUser { user_id, role, company_id, exp }` (struct L32-38) dans les extensions. **Point d'injection PAT = ici.**
- `crates/kesh-api/src/middleware/rbac.rs:19-41` — `check_role` + `require_admin_role` / `require_comptable_role` (guards réutilisables, appliqués en `route_layer` APRÈS `require_auth`).
...
- `entities/bank_account.rs` (struct + New), `entities/account.rs:43-66` (enum sqlx mapping), `repositories/bank_accounts.rs` (CRUD scopé company + optimistic lock + soft-delete `archived`/`revoked_at` + audit dans tx), `routes/bank_accounts.rs` (handlers, payloads, extraction `Extension<CurrentUser>`), `lib.rs:253-262` (enregistrement).
