# Prompt — passe 2 de `bmad-create-story validate`, Story 25-3-c

*Versionné le 2026-09-26. Deux lentilles en contexte frais (Haiku 4.5) — cycle Sonnet → Haiku → Opus.*

⛔ **La passe 1 a corrigé la fiche** (entrée « validate P1 » de son Change Log, commit `3dc2e617`) :
nommage des deux champs de motif de la vue (`settlement_cancel_blocker` / `invoice_cancel_blocker`),
réécriture exigée des doc-comments de la queue, place des variantes dans l'enum, un seul bloc de
marquage pour le bouton, clés de texte nommées, phrase `:1157-1160` du manuel à réécrire. **Ces
corrections sont le premier suspect** : la sévérité se déplace vers le dernier patch. Pour voir
exactement ce qu'elles ont changé : `git diff 33fbe9ec 3dc2e617 -- _bmad-output/implementation-artifacts/25-3-c-annuler-facture-fournisseur.md`.
