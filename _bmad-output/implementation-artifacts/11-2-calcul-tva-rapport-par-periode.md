...
**Status final** : `review` — implémentation + revue complètes, prêt pour merge. **T6.1 Issue [#180](https://github.com/guycorbaz/kesh/issues/180) créée** (comptes TVA + récupérable, `v0.2-milestone`). Décision Guy : push/PR 11-2 **après merge de 11-1** (#179) puis rebase sur main. Reste : T6.4 Playwright E2E (déféré, gap LOW) ; `cargo test --workspace` complet rejoué par la CI.
