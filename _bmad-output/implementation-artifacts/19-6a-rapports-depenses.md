...
- [ ] **T1 — Fondation kesh-report** (AC: 1-2) : `project_report.rs` (`ProjectReportScope` + `resolve_scope`, `ProjectPeriodMode` + `period_label` + `where_clause_and_binds`), `ReportError::ProjectNotFound`, re-exports lib.rs.
- [ ] **T2 — Agrégation Dépenses + drill-down** (AC: 3-4) : `generate_project_expenses` + DTOs + requête détail drill-down + tests agrégation (13 a-f).
- [ ] **T3 — Export PDF + CSV** (AC: 5-6) : `render_project_expenses_pdf` (labels dédiés) + `render_project_expenses_csv` + re-exports.
- [ ] **T4 — API** (AC: 7-9) : `ProjectReportQuery`, 2 endpoints, mapping `ProjectNotFound`→404, i18n slug 5 locales, audit, enregistrement lib.rs avant `;`.
