...
- **Code-review Pass 1 (2026-07-04, Sonnet ×3 — Blind Hunter / Edge Case Hunter / Acceptance Auditor, diff aplati main...HEAD)** : 1 HIGH + 2 MEDIUM + 3 LOW, verdict Acceptance Auditor GO 22/23 ACs. Patches :
  ...
  - **MEDIUM (AA2) — AC22 tests composants** : ajouté `RuleFormModal.test.ts` (nouveau) + tests sélecteur projet dans `ManualMatchModal.test.ts` et `TransactionSplitModal.test.ts` (visible si projets, masqué sinon, tag existant force l'affichage). 15/15 verts.
  - **LOW (AA3/BH/ECH)** — `FailedProposal.details` : ajout de `{ projectId }` (AC11) via helper paramétré ; et **mislabel** d'un `NotFound` non-projet en `PROJECT_NOT_FOUND` : le chemin `create_in_tx` d'`accept_one_rule` (projet déjà validé en amont) repasse en `DATABASE_ERROR` générique ; le split (validation per-ligne DANS create_in_tx) garde le mapping projet. Assertion `details.projectId` ajoutée au test archivé.
  - **NOTE (AA4)** — `Option<Option<i64>>` vs texte AC3 : choix DC4-autorisé (effacement supporté), documenté. Non-bloquant.
  - Gate post-patch : reconciliation_rules_e2e 31/31 (+1), manual 15/15, split 11/11, **E2E Playwright 9/9 réels**, frontend reconciliation unit 41/41 (+9 composants), check 0 err / lint / build ; fmt + clippy 0. Modèles : dev Fable 5→Opus 4.8 → review Sonnet. **Trend > LOW : Pass 1 = 1 HIGH + 2 MEDIUM → patchés. Prochaine passe : Haiku (contexte frais).**
