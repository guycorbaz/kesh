...
| Date | Étape | Note |
|---|---|---|
| 2026-09-24 | review P1 | **Passe 1, deux lentilles Sonnet** en contexte frais, prompt versionné `25-3-zero-review-prompt-p1.md`, **tous les axes déclarés exercés**. **0 CRITICAL, 0 HIGH, 0 MEDIUM, 2 LOW.** ⛔ **LOW-1 corrige une prétention du test** : mes deux assertions étaient **négatives** — zéro écriture inverse après le rollback —, donc satisfaites aussi bien par un `reverse_in_tx` qui **ne ferait rien du tout**. *Un test de rollback qui ne prouve pas l'écriture ne prouve pas le rollback.* ⇒ lecture ajoutée **à l'intérieur de la transaction**, avant le drop, et **éprouvée** : une mutation qui annule l'écriture avant le retour fait désormais rougir le test, ce qui n'était pas le cas. LOW-2 : la troisième assertion est **redondante** avec la deuxième (même prédicat) — gardée pour documenter le chemin de production, mais le commentaire dit désormais qu'elle **ne compte pas comme une garde de plus**. ✅ **Vérifié et à ne pas refaire** : la lentille A a comparé les neuf étapes **ligne à ligne** contre `main` — séquence identique —, et apporté une preuve plus forte que la mienne sur les sept déréférencements : **quatre des sept sites appellent des fonctions typées CONCRÈTEMENT** `&mut Transaction`, donc un `&mut **tx` mal placé y serait un *type mismatch*, non une compilation silencieuse. *Mon « ça compile » était un argument faible.* La lentille B a vérifié **dans le code** (et dans le source vendored de `sqlx`) les affirmations des doc-comments — `cancel` ne pose jamais `reverses_entry_id`, ne consulte jamais `reversal_blocker`, et `Drop` déclenche bien le rollback —, recompté `27` tests (⚠️ un `grep` naïf en rend **28** : la ligne 5 est un commentaire de module qui *mentionne* l'attribut) et les `3 + 4` déréférencements. Axe déclaré **non exercé** : le gate `2419/2419`, non recomptable sans exécution — repris par l'orchestrateur. |
