...
| Date | Action | Auteur |
|------|--------|--------|
| 2026-05-04 | Code review Pass 3 — 3 reviewers parallèles **Haiku 4.5** (cycle Opus → Sonnet → Haiku par règle CLAUDE.md). 10 + 0 + 0 findings → triage : **0 HIGH** + **0 MEDIUM** post-triage (8/10 BH-Haiku findings REJECT — nitpicks, faux positifs, duplicates des défers Pass 1/2 ; 1 LOW retenu) + **9 REJECT**. Acceptance Auditor Haiku verdict **GO** sans condition (8/8 ACs PASS, 21/21 patches APPLIED, 0 régression). Edge Case Hunter Haiku : `[]` (analyse mécanique 25 zones, aucun unhandled path). 1 patch Pass 3 appliqué : nouveau fixture `v04_ntry_sign_fallback.xml` (Ntry sans CdtDbtInd, 2 TxDtls avec leurs propres signes CRDT/DBIT) + test `parse_ntry_without_sign_falls_back_to_txdtls_signs` qui verrouille la régression Pass 2 F13. Trend findings > LOW : Pass 1 = 12 → Pass 2 = 4 → **Pass 3 = 0**. **Critère d'arrêt CLAUDE.md atteint** : 0 finding > LOW post-triage. Story 8-1a prête à merger. | Claude (Opus 4.7, code-review Pass 3) |
| 2026-05-04 | Code review Pass 2 — 3 reviewers parallèles **Sonnet 4.6** (cycle Opus → Sonnet par règle CLAUDE.md). 13 + 7 + 1 verdict findings → triage : **0 HIGH** (BH#2 « régression overdraft » REJECT — scénario physiquement impossible ; BH#1 / BH#3 reclassés MEDIUM/DEFER) + **4 MEDIUM** (F13 ntry sign optionnel quand TxDtls porte CdtDbtInd, F14 anchored Ntry Dt/DtTm, F15 Bal assertions au prefix-test, F16 fixture orpheline `v04_balance_mismatch.xml`) + **5 LOW** (F17 within_tolerance vacuous documenté, F18 Display SourceFormatTag, F19 signed-mix test, F20 Change Log count, F21 RltdPties ancestor guard) + **11 defer** + **1 reject**. Acceptance Auditor Sonnet : **NO-GO conditionnel** sur F16, 11/12 patches Pass 1 APPLIED OK, F11 PARTIAL (rectifié par F20). 9 patches appliqués Pass 2 : ntry_sign devient `Option<CdtDbtSign>` avec fallback Ntry → erreur seulement si TxDtls vide ou si TxDtls et Ntry tous deux sans signe ; matchers `BookgDt|ValDt > Dt|DtTm` anchorés ; opening/closing assertions ajoutées au prefix-test ; nouveau test `validate_balance_detects_balance_mismatch_fixture` qui charge la fixture via `kesh-import` ; nouveau test `validate_balance_handles_signed_mix_correctly` ; `Display for SourceFormatTag` délégué à `as_db_str()` ; `RltdPties` ancestor check sur counterparty Nm. Trend findings > LOW : Pass 1 = 12 → Pass 2 = 4 → relance Pass 3 Haiku (cycle Opus → Sonnet → **Haiku** → Opus). | Claude (Opus 4.7, code-review Pass 2) |
| 2026-05-04 | Code review Pass 1 — 3 reviewers parallèles (Blind Hunter / Edge Case Hunter / Acceptance Auditor, tous Opus 4.7 — biais d'auteur connu, à compenser en Pass 2 avec un LLM différent). 54 findings bruts → triage : **2 HIGH** (F1 sign parsing strict, F2 partial Bal block) + **8 MEDIUM** (F3 CI `--allow-dirty`, F4 `cargo metadata --no-deps`, F5 `set -euo pipefail`+jq check, F6 truncated fixture EOF guard isolation, F7 anchored matchers, F8 doc comment validate_balance, F9 explicit `check_end_names`, F10 boundary tests 0.01) + **2 LOW** (F11 doc cosmétique, F12 error context dot-path) + **9 defer** (8-1b/8-2) + **12 reject** (faux positifs). Acceptance Auditor verdict initial : **GO** (8/8 ACs PASS). 12 patches appliqués Pass 1 : `apply_balance` retourne `Result` et erre sur Bal partiel, `parse_sign` strict CRDT/DBIT, `MissingRequiredField(String)` avec dot-path indexé `stmt[i].ntry[j].field`, anchored matchers `ends_with(...)` partout dans `handle_stmt_text`, fixture `v04_truncated_after_stmt_close.xml` + test isolant le guard EOF, 2 tests boundary 0.01 dans `validate_balance`. Trend findings > LOW : 12 → relance Pass 2. | Claude (Opus 4.7, code-review Pass 1) |
| 2026-05-04 | Implémentation Story 8-1a terminée. T1 (10 fixtures + README), T2 (parseur `kesh-import::camt053` v04/v08, dispatch namespace, 10 tests d'intégration), T3 (extensions `kesh-core::bank_imports` : `BankImportDraft` / `SourceFormatTag` / `from_imported` / `validate_balance` / `validate_currency_supported_v0_1`, 3 nouvelles variantes `CoreError::BankImport*`, 9 tests unitaires + 4 tests spike pré-existants = 13 tests `bank_imports::tests` total), T8 (invariants CI : `cargo publish --dry-run -p kesh-import` AC #7 + invariant zéro path dep AC #6). Validation locale : fmt + build + clippy `-D warnings` + tests `kesh-import` (10 + 7) et `kesh-core` (147) verts ; `cargo publish --dry-run -p kesh-import` OK ; `cargo metadata` invariant vide. ACs #2/#5/#6/#7/#8/#9/#14a/#15a satisfaits. Status story : `ready-for-dev` → `review`. | Claude (Opus 4.7, dev-story exécution) |
