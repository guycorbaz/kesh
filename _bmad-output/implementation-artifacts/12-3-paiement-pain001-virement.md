...
- [ ] **T3 — Repo lot (kesh-db) + extraction pay_in_tx** (AC: 1,2,3,6,7,8,9 ; DC1,DC6) :
  ...
  - [ ] `create_batch` : pour chaque facture, `SELECT … FOR UPDATE` [H1] + guards (existe/`open`/a IBAN ou QR-IBAN/pas dans lot `generated`/company-scoped) + **validation `kesh-qrbill` IBAN/QR-IBAN/QRR** [M3/AC5], erreurs per-facture en `PaymentBatchFailedItem { supplier_invoice_id, error_code, details }` (pattern `FailedProposal` `reconciliation.rs:152`, struct DÉDIÉE pas import) [H3]. Calcule `end_to_end_id` (ex. `PAY-{batch_id:08}-{invoice_id:08}`, ≤35) persisté [H2].
  - [ ] `confirm_batch` : UNE tx, `pay_in_tx` per-facture (atomique), lot→`confirmed` ; rollback total sur échec.
  - [ ] `cancel_batch` : lot→`cancelled`, déverrouille (DELETE items ou statut).
