...
le code livré ; la passe 4, braquée sur lui et sur les catalogues à l'exclusion de toute prose,
rapporte **0 CRITICAL · 2 HIGH · 10 MEDIUM · 8 LOW**. La sévérité **remonte**, et l'explication n'est
pas que la remédiation ne mord pas : c'est que **le code n'avait jamais été regardé d'aussi près**.

**Décisions attendues (3)**

- [ ] [Review][Decision] **Le français `imported-supplier-invoices-save = « Valider la facture »` est FAUX, et il a été laissé faux** — le code **crée** la facture. Les trois cibles disent l'acte réel (`übernehmen`/`Registra`/`Record`), si bien que **le francophone est le seul à lire le mot trompeur**, et « valider » a dans Kesh un sens comptable réservé (rendre une pièce immuable). La règle « le français est verbatim » suppose un français **correct** ; ici il ne l'est pas. Corriger en « Enregistrer la facture », ou maintenir le verbatim ?
- [ ] [Review][Decision] **`-col-qty` et `-col-vat` sont mal nommées** — elles titrent des colonnes de **lignes**, encadrées par trois clés `-line-*`, alors que toute la famille `-col-*` sert la **liste des factures**. Le jour où la liste gagne une colonne TVA, le nom est pris avec une valeur posée pour un autre niveau d'agrégation — le mécanisme même que cette story vient de défaire. Renommer en `-line-qty` / `-line-vat` tant qu'aucune installation n'a chargé ces clés ?
- [ ] [Review][Decision] **`-err-currency` : la réserve de version n'existe plus qu'en français** (« CHF uniquement **en v0.4** ») — la passe 2 l'a retirée des trois cibles. Le francophone lit une limitation datée, les autres une limitation définitive : **ce n'est pas le même message**. Retirer aussi du français, ou reporter dans les trois cibles ?

**Patches (12)**

- [x] [Review][Patch] **HIGH — le statut de facture s'affiche en français dans les quatre langues** [`supplier-invoice-helpers.ts:20-30`] — `supplierInvoiceStatusLabel()` retourne `'Ouverte'`/`'Payée'`/`'Annulée'` **en dur**, sans jamais appeler `i18nMsg` ; aucune clé `supplier-invoices-status-*` n'existe. ⚠️ **Et cette story a traduit l'en-tête de la colonne** : un germanophone lit « Status » au-dessus de « Payée », sur **chaque ligne** de la liste et en badge sur chaque fiche. Invisible du moissonneur, de l'allowlist et des deux gardes — angle mort #255. Pire, `supplier-invoice-helpers.test.ts:20-22` **verrouille le français** par `toBe('Ouverte')`. ✅ Aucun arbitrage neuf : `ouverte` et `payée` sont **déjà en partie A**, et `annulée` est attesté (`invoice-status-cancelled`). *(convergé ECH + AA)*
- [x] [Review][Patch] **HIGH — `placeholder="Qté"` en dur à DEUX sites sur trois** [`+page.svelte:378`, `import/+page.svelte:558`] — la clé `supplier-invoices-col-qty` existe pourtant dans les quatre locales. Le docstring de la garde déclare avoir couvert « un `Qté` » ; il y en avait **trois**. C'est « greper le symptôme, pas le site », commis dans le fichier même qui documente cette règle. *(convergé ECH + AA)*
- [x] [Review][Patch] **L'en-tête `de-CH` justifie une valeur qui n'est plus livrée** [`de-CH:1489`] — il dit « Rendu par « **erfassen** » et non « validieren » » alors que la passe 3 a livré `Rechnung **übernehmen**`. ⚠️ **Régression de mon propre patch** : valeur changée, justification laissée en arrière. `it` et `en` n'ont pas cet écart.
- [x] [Review][Patch] **Les quatre en-têtes annoncent « 113 clés » ; il y en a 119** [`{fr,de,it,en}-CH`] — et la ventilation « 103 statiques » est fausse du **même écart de 2**, ce qui montre qu'aucun des deux n'a été recompté après l'ajout de `-col-qty` et `-col-vat`. Cette fois le compte faux est **dans le code livré**, pas dans un rapport.
- [x] [Review][Patch] **`compte de charge` est employé et figé sans figurer nulle part au glossaire** — 3 sites attestants hors story, 2 ajoutés ici, **absent de la partie A comme de la partie B**. ⚠️ **C'est le seul terme du domaine dont on ait la PREUVE qu'il dérive** : la passe 1 (H2) a dû corriger `Aufwandkonto` → `Aufwandskonto` et `conto costi` → `conto di costo`. Rien n'empêche 23-4 à 23-6 de recommencer. Et **l'anglais s'en tire par coïncidence** (`expense account`, monolithique) — un contrôle mené sur l'anglais ne le verrait pas, pour la troisième fois sur cet epic. → monter en partie A.
- [x] [Review][Patch] **`TTC` n'est protégé nulle part alors que son pendant exact `montant HT` est en partie A** [`glossaire:89`] — cette story fige la forme cible dans **trois clés neuves** (`Inkl. MWST` / `IVA incl.` / `Incl. VAT`). Le risque est **daté et localisable** : `credit-notes-col-total` et `-col-line-total`, encore à l'allowlist, portent le repli « Total HT » — le rollout qui les traduira devra retrouver la forme sans que rien ne la lui impose. → monter `montant TTC` en partie A.
- [x] [Review][Patch] **`depenses-par-projet` n'est pas une clé — c'est une VALEUR** [`glossaire:135,137,151`] — le slug de nom de fichier de `reports-filename-project-expenses`. Il est cité comme « précédent » sur **deux lignes de partie A**, et la prose de **l'arbitrage de Guy sur « analytique »** s'en réclame. ⚠️ **Classe exacte du précédent `city` / `Town/city`** : le glossaire est réfuté par ce qu'il donne comme preuve. La traduction reste juste ; c'est l'attestation qui est fausse. → citer `supplier-invoices-detail-project`, désormais une vraie clé de libellé.
- [x] [Review][Patch] **La carte de codes d'erreur peut GRANDIR sans qu'aucune garde ne rougisse** [`import/+page.svelte:55-72`] — mutation exécutée par la lentille : 11ᵉ entrée ajoutée avec un suffixe absent de tout catalogue → **111 tests verts**. `CARDINALITES` compare deux valeurs **toutes deux internes au test** ; rien ne lit la carte de production. Le docstring affirme pourtant qu'« elle rougit dès que la carte évolue » — **faux dans le sens de la croissance**, qui est le cas réel (Epic 12 ajoutera des codes). ✅ *Le sens décroissant, lui, est bien couvert, et un code inconnu tombe sur `-error-unknown` : jamais de clé brute à l'écran.*
- [x] [Review][Patch] **La garde neuve applique la doctrine inverse de sa voisine, écrite dans le même commit** [`i18n-un-repli-par-cle.test.ts:33,84`] — borne molle `>= 90` pour un relevé réel de ~106 (une régression effaçant douze sites passerait), là où `i18n-keys.test.ts` pose `sitesTotal: 1497` **exact** et écrit « un écart se recompte, il ne s'ajuste pas ».
- [x] [Review][Patch] **`de-CH` : `Nr. Lieferantenrechnung` est un calque agrammatical, livré deux fois** — deux substantifs juxtaposés sans composition ni génitif, transposition mot à mot de « N° facture fournisseur ». L'anglais du même diff met l'ordre juste (`Supplier invoice no.`). → `Lieferantenrechnungs-Nr.`
- [x] [Review][Patch] **`it-CH` : la modalité d'échec disparaît des deux messages de rechargement** — « non è stato ricaricato » énonce un **fait**, pas un **échec**. Le français dit « n'a pas **pu** être rechargée », l'allemand « **konnte** nicht », l'anglais « **could not** » : l'italien est le seul à perdre le modal, dans un message d'erreur. → « Non è stato possibile ricaricare l'elenco ».
- [x] [Review][Patch] **Sept nits de catalogue et de rangement** — commentaire orphelin en fin d'allowlist (le bloc qu'il décrivait a été retiré) ; « 1405 clés » dans la § AC7, recompté et daté ; `Buchungssatz`/`journal entry`/`scrittura contabile` déclarés « RELEVÉS » en en-tête mais employés dans **aucune** des 115 valeurs ; `-target` rendu `valore QR` en italien, ce qui écrase la distinction que le français tient entre « cible QR » et « montant du QR » ; `Quell-Bankkonto` au lieu du terme bancaire suisse `Belastungskonto` ; ⚠️ **`-doc-failed` ≡ `-source-doc-failed` et `-doc-gone` ≡ `-source-doc-gone` : NON corrigés, avec motif** — le **français** est lui aussi identique sur ces paires, et il est verbatim ; différencier les seules cibles ferait dire aux trois langues une chose que le français ne dit pas. À traiter par une story qui touche légitimement le français ; et `fr-CH` range ses 10 clés `-error-*` ailleurs que les trois autres, ce qui rend le `diff` côte à côte illisible sur ce bloc.

**Reportés — réels, hors périmètre d'un rollout de traduction (4)**

- [x] [Review][Defer] **Un `catch` sans branche `else` rend un état MENTEUR sur les trois écrans** [`+page.svelte:134`, `[id]:69,95,107`, `import:107`] — une erreur qui n'est pas une `ApiError` (backend injoignable, coupure réseau) est avalée : `errorMsg` reste vide et la liste affiche **« Aucune facture fournisseur enregistrée »**. Un comptable conclut que ses pièces ont disparu. Sur `pay()`, le bouton se réactive **sans aucun message** — sur un règlement, c'est le pire retour possible. ⚠️ **Bug applicatif préexistant**, pas un défaut i18n.
- [x] [Review][Defer] **`report.warnings` : trois messages français fabriqués au `format!` côté Rust** [`inbox_import.rs:250,322,585`] et rendus tels quels. Le rapport d'import a trois branches ; cette story en traduit deux.
- [x] [Review][Defer] **Le QR lisible mais non-SPC affiche le message français du backend** [`+page.svelte:103`] — `(isApiError(err) && err.message) || i18nMsg(…)` donne la priorité au backend, rendant la clé morte sur ce chemin, pourtant le plus fréquent (on scanne le mauvais QR d'une page). L'écran d'import traite la même condition avec une clé traduite.
- [x] [Review][Defer] **Trois précédents de partie A sont ambigus** — `Carnet d'adresses`, `Client` et `TVA due` désignent chacun plusieurs clés dont l'une **contredit** le glossaire (`contacts-page-title = Adressbuch`, `due-dates-column-contact = Customer`, `reports-vat-column-vat-due = VAT due`). Hors périmètre, mais même classe que `depenses-par-projet`.

**Réfutés au grep — 3 dismiss** : « `imported-supplier-invoices-reload-failed` est orpheline » — elle est appelée à `import/+page.svelte:86` *(la lentille avait posé la réserve elle-même)* ; « `common-loading` n'est attesté nulle part » — présente dans les **quatre** catalogues ; « les trois cibles suppriment « analytique » de *Projet analytique* » — c'est l'**arbitrage de Guy** du 2026-08-19, `Projekt`/`progetto`/`project`, inscrit au kickoff de l'epic.
...
clés du domaine **entre elles**, ce qui ne peut par construction jamais voir une collision avec
les **1292 autres entrées** du catalogue. *(État actuel, recompté : **1412** clés par locale, dont
**120** au domaine — le contrôle ci-dessous a été mené à l'état de la passe 3, où le domaine en
comptait 116 ; les 1292 hors domaine, eux, n'ont pas bougé.)* Le contrôle est donc mené à **deux portées**, et c'est la
...
**Portée 2 — chaque libellé du domaine contre le catalogue ENTIER de sa locale.** Neuf
...
| date | passe | résultat |
|---|---|---|
| 2026-08-20 | **passe 4** de `bmad-code-review` | **0 CRITICAL · 2 HIGH · 10 MEDIUM · 8 LOW** (Opus ×3, braquée sur le CODE, prose exclue du diff). Trend : 3H/10M/6L → 2H/2M/2L → 2H/3M/1L → **2H/10M/8L**. ⚠️ **La sévérité remonte, et c'est le bon résultat** : les passes 2 et 3 auditaient les comptes rendus ; celle-ci, la première à regarder le code d'aussi près, rapporte plus que les deux précédentes réunies. **HIGH-1, convergé par deux lentilles : le statut de facture s'affichait en FRANÇAIS dans les quatre langues** — `supplierInvoiceStatusLabel()` retournait `'Ouverte'`/`'Payée'`/`'Annulée'` en dur, sous un en-tête que cette story venait de traduire. Angle mort #255 : invisible du moissonneur, de l'allowlist et des deux gardes — et un test vert **verrouillait** le français. **HIGH-2 : `placeholder="Qté"` en dur à DEUX sites sur trois**, la clé existant pourtant en 4/4. **Douze patches appliqués**, dont la promotion de `compte de charge` et `montant TTC` en partie A (le premier est le seul terme du domaine dont on ait la **preuve** qu'il dérive), la correction de `depenses-par-projet` — cité trois fois comme clé attestante alors que c'est une **valeur**, classe exacte du précédent `city`/`Town/city` —, et l'extraction de la carte de codes d'erreur, qui pouvait **grandir sans qu'aucune garde ne rougisse** (mutation mesurée : 111 tests verts ; la même mutation rougit désormais). **Quatre findings reportés**, tous des bugs applicatifs sans rapport avec la traduction. **3 dismiss** au grep. Compteurs recomptés depuis la source : domaine **120** clés, **119 écrites** (109 statiques + 10 dynamiques), catalogue **1412**, glossaire partie A **65**, `sitesTotal` **1502**. |
