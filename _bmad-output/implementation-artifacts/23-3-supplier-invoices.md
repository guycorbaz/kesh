...
- [x] [Review][Patch] **Les quatre en-têtes annoncent « 113 clés » ; il y en a 115** [`{fr,de,it,en}-CH`] — et la ventilation « 103 statiques » est fausse du **même écart de 2**, ce qui montre qu'aucun des deux n'a été recompté après l'ajout de `-col-qty` et `-col-vat`. Cette fois le compte faux est **dans le code livré**, pas dans un rapport.
...
les **1292 autres entrées** du catalogue. Le contrôle est donc mené à **deux portées**, et c'est la
...
**Portée 2 — chaque libellé du domaine contre le catalogue ENTIER de sa locale** (1412 clés). Neuf
