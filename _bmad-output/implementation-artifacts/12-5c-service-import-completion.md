...
3. **Boucle de traitement par fichier** : `read_dir(KESH_INBOX_DIR)` **non récursif** (pas de descente dans les sous-dossiers). Pour chaque entrée (jusqu'à `MAX_FILES_PER_RUN`) :
   ...
   9. **Succès** → suppression du fichier inbox (tolérer `ENOENT`, idempotent). **Échec** → déplacement vers `failed/`. Le dossier `failed/` est créé (`create_dir_all`) avant la boucle (ou au 1ᵉʳ déplacement). **Nommage anti-collision** : la cible dans `failed/` est nommée **`{stem}_{8 premiers hex du sha256}.{ext}`** (ou `{sha256hex}.{ext}` si le hash est déjà calculé) — `std::fs::rename` écrase atomiquement une cible homonyme sur POSIX, donc deux fichiers distincts de même nom écraseraient l'un l'autre sans le suffixe discriminant. Anti-traversal sur le nom (jamais de composant `/`/`..`).
...
6. **Refactor `supplier_invoices::create_in_tx`** : extraire le cœur de `create` (`repositories/supplier_invoices.rs:212-406`, qui `pool.begin()` l.239 / `tx.commit()` l.406) en **`create_in_tx(tx: &mut sqlx::Transaction<'_, sqlx::MySql>, company_id, user_id, new: NewSupplierInvoice) -> Result<SupplierInvoiceWithLines, DbError>`** — **exactement** le pattern `pay`/`pay_in_tx` (l.457/490, qui retourne `SupplierInvoiceWithLines`). **Type retour = `SupplierInvoiceWithLines` (kesh-db), PAS `SupplierInvoiceResponse`** : `SupplierInvoiceResponse` vit dans `kesh-api` (`routes/supplier_invoices.rs:61`) et `kesh-db` **ne peut pas en dépendre** (cycle Cargo). Le handler HTTP construit la réponse via `SupplierInvoiceResponse::from_parts(inv.invoice, inv.lines)` (pattern `supplier_invoices.rs:303`). `create` (pool-owning) **conserve son comportement** (appelle `create_in_tx` puis `commit`, comme `pay` appelle `pay_in_tx`). Aucune régression des tests 12-2.
...
7. **`POST /api/v1/imported-supplier-invoices/{id}/complete`** (Comptable+, company-scopé) — corps JSON camelCase :
   ...
   **Complétion atomique (DC6)** dans **une seule transaction** :
   ...
   6. **Réconciliation montant (F2)** : si `staging.amount` présent → exiger `round2(Σ lignes TTC, HALF_UP) == round2(staging.amount, HALF_UP)` sinon bloquer « Le total des lignes (X) ne correspond pas au montant du QR (Y) ». **Le `Σ lignes TTC` DOIT être calculé exactement comme `create_in_tx`/`generate_purchase_journal_lines`** — c.-à-d. via `kesh_core::accounting::vat::line_vat_amount(line_total, vat_rate)` (TVA **arrondie par ligne**, HALF_UP/centime, FR55 AFC), PAS une formule alternative `Σ(qty*pu*(1+taux/100))` arrondie globalement (drift centime possible : la réconciliation passerait alors que `create_in_tx` inscrirait un `total_amount` différent, et pain.001 paierait ≠ du QR/QRR). Idéalement réutiliser le même helper que `create_in_tx` pour garantir l'égalité par construction. Renseigner `NewSupplierInvoice.expected_payment_amount = staging.amount`. Si `staging.amount` absent → pas de contrôle.
...
- `crates/kesh-db/src/repositories/imported_supplier_invoices.rs` : `create`, `find_by_id_scoped(company_id, id)`, `find_by_company_hash(company_id, file_hash)`, `list_by_status(company_id, status)` — tous scopés `company_id`. **`list_by_status` ne valide pas `status`** (dette D1 à traiter à la frontière HTTP). **Fonctions UPDATE à AJOUTER** (le repo 12-5b n'expose aucun UPDATE) : `reactivate_to_complete(&mut tx, company_id, id)` (réactivation `discarded`→`to_complete`, AC3 step 5) et `mark_completed(&mut tx, company_id, id, supplier_invoice_id)` (complétion, AC7 step 7) — toutes deux `tx`-aware (appelées dans la transaction de complétion/import) et scopées `company_id`.
- **Cross-module** : le handler `/complete` (`routes/imported_supplier_invoices.rs`) importe `SupplierInvoiceResponse` depuis `crate::routes::supplier_invoices` (`use crate::routes::supplier_invoices::SupplierInvoiceResponse;`) — ne pas redéfinir (DRY).
