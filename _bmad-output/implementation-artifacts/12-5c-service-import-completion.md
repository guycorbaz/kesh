...
- [x] **T4.1 — Config inbox** : `KESH_INBOX_DIR` (`/data/inbox`) + `KESH_INBOX_MAX_FILE_BYTES`/`MAX_FILES_PER_RUN`/`MAX_PDF_PAGES` dans `Config` + `from_env` (pattern `documents_dir` 12-5b). Câbler `MAX_PDF_PAGES` → `DecodeConfig.max_pages`.
- [x] **T4.2 — Refactor DC6** : extraire `supplier_invoices::create_in_tx(&mut tx, …)` (miroir `pay_in_tx`), `create` délègue + commit. 0 régression tests 12-2.
...
Trend > LOW : **5** (4 MEDIUM + 1 cross-confirmé). Acceptance Auditor : implémentation jugée fidèle (AC1-AC12, DC6, F-OPUS-1/2, F9, D1/D2, FailedProposal tous conformes). Patches :
- **EC1/BH2/AA4 (MEDIUM ×3 consensus)** — code `SOURCE_DOCUMENT_NOT_FOUND` trompeur pour staging absent en `/complete` + `/discard` (un même code sur 2 sémantiques différentes piégeait le `catch` 12-5d). Patch : nouveau variant `AppError::ImportedInvoiceNotFound` → 404 `IMPORTED_INVOICE_NOT_FOUND` ; downloads gardent `SOURCE_DOCUMENT_NOT_FOUND`.
- **BH1 (MEDIUM)** — fichier disparu mid-loop (upload qui supprime la source après copie) → `move_to_failed` `rename` ENOENT → `AppError::Internal` propagé → **500 abat tout le batch**. Patch : `move_to_failed` retourne `Result<bool>` (false = ENOENT/race) + helper `dispose_failed` qui mappe la disparition en `Skipped` (comme la race `read_dir`), pas un 500.
- **EC2/BH4 (MEDIUM)** — `Content-Disposition` omis pour noms accentués (`Reçu_Müller.pdf`, courant en Suisse romande) car `HeaderValue::from_str` rejette le non-ASCII → header silencieusement absent. Patch : helper `content_disposition` avec repli ASCII `filename=` **ET** `filename*=UTF-8''<pct>` (RFC 5987).
- **BH3/EC3 (MEDIUM/LOW)** — `mark_completed`/`mark_discarded` ignoraient `rows_affected` → no-op silencieux possible (facture orpheline sans lien staging). Patch : `rows_affected == 0` → `DbError::Invariant` (rollback la tx).
- **AA1 (MEDIUM)** — chemin `magic_matches()` jamais atteint par un test (le test `notes.txt` rejette sur l'extension). Patch : fixture `fake.pdf` (extension valide + contenu PNG) → `UNSUPPORTED_FILE_TYPE` ; + assertion id inexistant → 404 `IMPORTED_INVOICE_NOT_FOUND`.
- **LOW dismiss/documentés** : BH5 (mtime exotique FS sans timestamps), BH6/EC5 (TOCTOU/panic lock = L6 accepté v0.4), EC4 (0-byte → diagnostic UNSUPPORTED, fichier correctement rejeté), AA2 (signature `reactivate_to_complete` pool vs tx = choix intentionnel service-level), AA3 (`ImportCompletionRejected` vs `Validation` = résolution correcte de la contradiction spec, documentée), AA5 (test traversal = couvert en esprit par le test symlink, le garde canonicalize étant inatteignable pour une entrée directe non-symlink).

Build + clippy `-D warnings` + fmt verts ; 16 tests inbox_import_e2e verts. Pass 2 : Haiku 4.5 (diff aplati HEAD vs baseline).
