...
5. **Entités + repositories** — `crates/kesh-db/src/entities/bank_import.rs` + `bank_transaction.rs` + `crates/kesh-db/src/repositories/bank_imports.rs` + `bank_transactions.rs` :
   - `entities::BankImport` (struct miroir DB), `entities::NewBankImport` (struct insert), `entities::BankImportSourceFormat` (enum String-mappé `Camt053V04` / `Camt053V08` ; `Csv` ajouté Story 8-2 ; `as_str()` + `try_from_str(&str)` cohérents avec le pattern `Role::as_str` de `users.rs`).
     - **Dérive obligatoire `BankImport`** : `#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]` (pattern aligné `BankAccount`, `Account`, etc. ; sans `sqlx::FromRow`, `sqlx::query_as::<_, BankImport>(...)` ne compile pas — pattern utilisé par tous les `find_*` du repo).
     - `NewBankImport` : `#[derive(Debug, Clone)]` suffit (pas lu via FromRow, seulement passé en argument).
   - `entities::BankTransaction`, `entities::NewBankTransaction`, `entities::BankTransactionStatus` (enum `Pending` / `Reconciled` avec `as_str` / `try_from_str`).
     - **Dérive obligatoire `BankTransaction`** : `#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]`.
     - **Note `BankImportSourceFormat` et `BankTransactionStatus`** : ces enums sont stockés en VARCHAR côté DB ; le mapping FromRow se fait au niveau de la struct parente via `#[sqlx(try_from = "String")]` (ou méthode équivalente — cf. `Role` dans `entities/user.rs` pour le pattern exact).
...
6. **Route API `kesh-api/routes/bank_imports.rs`** — endpoints :
   ...
   **Détails implémentation :**
   ...
   - **Hash SHA-256** : ajouter `sha2 = "0.10"` à `kesh-api/Cargo.toml` (même version que `kesh-qrbill/Cargo.toml` qui l'utilise déjà — version uniforme dans le workspace pour éviter les duplications cargo). Pattern de calcul streaming :
     ```rust
     use sha2::{Digest, Sha256};
     let file_bytes = field.bytes().await?;  // Vec<u8> du multipart
     let file_hash = format!("{:x}", Sha256::digest(&file_bytes));
     // file_hash : String hex lower-case 64 chars
     ```
     **Ne pas utiliser** `ring` (heavier dep, déjà transitivement présent via rustls mais pas exposé pour usage applicatif), ni `md5` (cryptographiquement cassé, non adapté à la déduplication forte).
...
- [ ] T6.1 — Activer feature multipart : changer `axum = "0.8"` (actuel) en `axum = { version = "0.8", features = ["multipart"] }` dans `crates/kesh-api/Cargo.toml`. La feature `multipart` n'est pas dans les default features Axum 0.8. **Ajouter aussi** `sha2 = "0.10"` (même version que `kesh-qrbill/Cargo.toml` — uniformité workspace) pour le hash SHA-256 du fichier importé.
...
- [ ] T6.10 — Limite multipart : `axum::extract::DefaultBodyLimit::max(max_bytes)` appliqué via `Router::layer()` sur le sub-router bank-import uniquement (10 MiB + 4 KiB overhead par défaut). Configurable via `KESH_BANK_IMPORT_MAX_MB` (`kesh-api/src/config.rs`). **Pas besoin** d'ajouter la feature `"limit"` à `tower-http` — DefaultBodyLimit est natif Axum. **Documenter dans `.env.example`** : ajouter le bloc commenté `# Story 8.1 — Taille max du fichier d'import bancaire en Mo (default 10)` + `KESH_BANK_IMPORT_MAX_MB=10` pour que les opérateurs voient l'option.
...
| 2026-05-03 | **Pass 1 spec validate (Sonnet 4.6, contexte frais)** — 16 findings (2C / 5H / 6M / 3L), tous appliqués (`all`). C1 i18n keys camelCase→kebab-case strict matchant nom du dossier (`bank-import-*`), C2 `Role::Accountant`→`Role::Comptable` + RBAC via sub-router `comptable_routes` (pas helper inline), H1 `quick-xml 0.36`→`0.39` (latest, `Reader::config_mut` requiert ≥0.37), H2 `axum features=["multipart"]` + `DefaultBodyLimit::max(...)` (pas `tower-http "limit"`), H3 audit_log signature `insert_in_tx(tx, NewAuditLogEntry { entity_type, entity_id, ... })`, H4 `quick_xml::NsReader` + fixture `v04_prefixed_namespace.xml`, H5 `find_by_id_for_company` tâche explicite + 2 tests, M1 bras `error_code()` exhaustifs, M3 `NaiveDateTime` (convention codebase), M4 7 bras `IntoResponse` listés, M5 `from_imported(... imported_at: NaiveDateTime)` paramètre (pureté `kesh-core`), M6 catch-remap `DbError::UniqueConstraintViolation` avant `?` (pattern `invoices::create`), L1 `routes/mod.rs` ajouté source tree, L2 tolérance balance reformulée, L3 test preview balance mismatch ajouté. | Claude (Validator Sonnet 4.6 + Author Opus 4.7) |
| 2026-05-03 | **Pass 2 spec validate (Haiku 4.5, contexte frais)** — 7 findings (0C / 3H / 2M / 2L), 3 appliqués + 4 rejetés (acceptation 43%, taux faux positifs Haiku ~57% conforme observations Stories 7-3/7-4). **Appliqués** : H1 `sha2 = "0.10"` ajouté à `kesh-api/Cargo.toml` (déjà présent dans `kesh-qrbill` — uniformité workspace), H3 `#[derive(sqlx::FromRow)]` explicité sur `BankImport` + `BankTransaction` (sans dérive `query_as` casse), L2 `.env.example` doit déclarer `KESH_BANK_IMPORT_MAX_MB=10`. **Rejetés** : H2 (FormData example — `FormData.append(name, true)` coerce string en navigateur, déjà adressé par C1 patch chain), M1 (suggestion `Vec<NewBankTransaction>` direct violerait l'invariant spike `kesh-core` ne connaît pas types DB), M2 (lint déjà vérifié et documenté C1), L1 (cosmétique). **Trend** : Pass 1 16 findings → Pass 2 7 raw → 3 actionables → 0 LOW résiduels post-triage. **Critère d'arrêt CLAUDE.md atteint après Pass 2** : 0 findings >LOW non adressés. Cycle Opus(auteur) → Sonnet(P1) → Haiku(P2) complété. Spec considérée converge — prête pour `/bmad-dev-story 8-1`. | Claude (Validator Haiku 4.5 + Author Opus 4.7) |
