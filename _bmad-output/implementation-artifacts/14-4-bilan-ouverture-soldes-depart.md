...
### Pass 1 (Sonnet ×3 — Blind Hunter / Edge Case Hunter / Acceptance Auditor, contexte frais, diff aplati `1b410d4e..1f2aa395`)

**Brut : 2 MEDIUM + 1 LOW (AA : 0 > LOW, matrice AC A→I 100 % ✓).** Triage orchestrateur avec vérification ground-truth :

- **BH-1 (MED → reclassé LOW, patché)** — le doc-comment de `create_opening_entry` sur-promettait (« sérialise avec toute création d'écriture concurrente ») alors que le `FOR UPDATE` porte sur la ligne du premier exercice : une écriture normale concurrente dans un **autre** exercice ne se sérialise pas avec la génération. **Analyse** : l'entrelacement est bénin — état final (ouverture + écriture) identique à l'ordre séquentiel légal « génération puis écriture » ; toute écriture commitée avant le lock est vue par `count_by_company` sous le lock ; l'invariant anti-double-ouverture (deux générations = même ligne) tient. Défaut = précision documentaire. **Patch** : doc-comment « Portée exacte de la sérialisation » (fn + site du lock). Doc-only, pas de test requis.
- **ECH-1 (MED → defer, issue #274)** — TOCTOU garde D4 : le type-check `find_types_by_ids_in_tx` s'exécute dans une tx courte séparée, avant le lock de `create_opening_entry` ; un `PUT /accounts/{id}` retypant Asset→Revenue dans la fenêtre passe. **Ground-truth vérifié** : `accounts::update` autorise le retypage d'un compte **avec écritures** à tout moment (verrou optimiste seul) → l'état final de la course est atteignable **légalement en séquentiel** (générer puis retyper 1 s après) ; la fenêtre n'ajoute aucun état nouveau, et déplacer le check sous le lock ne fermerait pas le retypage post-commit. Placement de la garde conforme AC-B/T3 (contrat scellé 15 revues). Cause racine pré-existante (hors story) → **defer** + **issue GitHub #274** (`[KF]` retypage d'un compte à historique reclassifie silencieusement bilan/P&L).
- **ECH-2 (MED → dismiss)** — « premier exercice non re-dérivé sous verrou » : redécouverte mot pour mot du **résidu L4 documenté** de la spec (catégorie B tracée, mauvais millésime pas doublon, remédiation = marqueur L3). ECH confirme lui-même « pas de corruption ni doublon ».
- **ECH-3 (LOW, patché + test)** — statut `READY` mais 0 compte de bilan éligible → grille en-tête nu + bouton désactivé sans explication. **Patch** : état `opening-balances-empty-grid` explicite (message + clé i18n ×4, table/bandeau/bouton non rendus) + test Vitest (16/16).

**Bilan Pass 1 : 0 finding > LOW après triage** (1 LOW-doc patché, 1 LOW patché+test, 1 defer #274, 1 dismiss L4). Vérifs post-patch : Vitest 16/16, `svelte-check` 0 err, lint-i18n PASS, `cargo build` OK. **Pass 2 de confirmation (Haiku ×3, orthogonal)** lancée par convention projet (les patches doivent être revus par un modèle différent).

### Pass 2 (Haiku ×3 — confirmation orthogonale, contexte frais, diff aplati `1b410d4e..7181703b`)

**Brut : BH 0 > LOW (patches P1 confirmés visibles) ; AA 0 > LOW (matrice AC 100 % re-vérifiée, patches P1 jugés corrects et complets) ; ECH 3 MED + 3 LOW.** Garde Haiku grep ground-truth appliquée aux 3 MED :

- **ECH2-1 (MED) « DECIMAL(19,2), arrondi silencieux au-delà de 2 décimales » → RÉFUTÉ, hallucination Haiku** : `journal_entry_lines.debit/credit` = **`DECIMAL(19,4)`** (`20260412000001_journal_entries.sql:42-43`, grep ground-truth) — exactement la borne d'`AMOUNT_RE` (4 décimales). Le « grep » de Haiku sur `types.rs` (Money « 2 décimales ») ne correspond à rien. Dismiss.
- **ECH2-3 (MED) « `accounting_language` NULL → panic » → RÉFUTÉ par le type** : `accounting_language: Language` (enum non-Option, colonne NOT NULL) — état impossible, Haiku le concédait lui-même (« OK en pratique »). Dismiss.
- **ECH2-2 (MED → reclassé LOW réel, patché + test)** — une ligne dont l'utilisateur tape « 0 » explicitement (`"0" !== ''`) entrait dans `nonEmptyRows` → envoyée au POST → 400 `EntryLineDebitCreditExclusive` (XOR strict vérifié `balance.rs:169-171`) avec message confus, là où `JournalEntryForm` bloque ce cas côté client (classification « partial »). Pas de corruption (le serveur rejette), incohérence de miroir + UX. **Patch** : `nonEmptyRows` filtre les lignes dont les deux côtés parsent à 0 (vide OU « 0 » explicite — traitées comme vides, non envoyées) ; montants invalides conservés par prudence (déjà bloqués par `hasInvalidAmount`). **Test Vitest** : ligne « 0 » explicite filtrée du POST (17/17).
- **LOW** : apostrophe U+2019 sur terminal non-UTF8 (dismiss — cosmétique non-applicatif) ; absence de fallback si clé `-entry-description` manquante (dismiss — clés présentes ×4, pattern i18n projet standard) ; test 0/0 manquant (couvert par le patch ci-dessus).

**Bilan Pass 2 : 1 finding réel (LOW après reclassement, patché+testé), 2 hallucinations Haiku réfutées par grep.** Vérifs post-patch : Vitest 17/17, `svelte-check` 0 err. → **Pass 3 de confirmation (Opus ×3, orthogonal)** requise (patch appliqué en P2).
