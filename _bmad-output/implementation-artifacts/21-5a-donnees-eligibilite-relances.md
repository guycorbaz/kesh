...
1. **Migration `invoice_reminders`** (nouveau fichier `crates/kesh-db/migrations/AAAAMMJJ000001_invoice_reminders.sql`) crée la table append-only avec, au minimum :
   ...
   - `subject TEXT NOT NULL`, `body TEXT NOT NULL` (**snapshots** du texte réclamé — preuve pré-contentieuse ; pour un rappel manuel, texte descriptif ou vide selon décision validate).
...
3. **Migration colonnes suspension** (peut être dans le même fichier ou un fichier séparé `AAAAMMJJ000002_invoices_dunning_paused.sql`) : `ALTER TABLE invoices ADD COLUMN dunning_paused_at DATETIME(6) NULL, ADD COLUMN dunning_paused_note VARCHAR(500) NULL;` (pattern `emailed_at`/`emailed_to` `20260709000002_invoices_emailed.sql:10-12`). Nullable → non-breaking. **PIÈGE OBLIGATOIRE (ground-truth)** : ces 2 colonnes DOIVENT être ajoutées au struct FromRow `entities/invoice.rs` (`pub struct Invoice`, après `emailed_to:38`) **ET** à **TOUTES les listes SELECT explicites** du repo `repositories/invoices.rs` — en particulier `FIND_INVOICE_SCOPED_SQL` (`invoices.rs:44-48`) et les autres `SELECT <colonnes> FROM invoices`. Une colonne au struct sans le SELECT → **échec sqlx au runtime** (`ColumnNotFound`). C'est la cause n°1 d'échec de ce type de story.

### Entité & repository `invoice_reminders`

4. **Entité** `crates/kesh-db/src/entities/invoice_reminder.rs` : struct `InvoiceReminder` (champs miroir DB, `fee_amount: rust_decimal::Decimal` avec `#[serde(with = "rust_decimal::serde::str")]` — Decimal (dé)sérialisé en **STRING**, jamais number), `channel` typé (enum `ReminderChannel { Email, Manual }` avec `as_str`/`FromStr`, ou VARCHAR validé). Déclaré dans `entities/mod.rs`.
...
8. **État terminal (fin de cycle visible — HIGH produit, item 9)** : si **aucun niveau supérieur n'existe** dans la config (niveau courant = MAX config), la facture **reste dans la liste** en état terminal `« Dernier niveau atteint — poursuite à envisager »` avec la date du dernier rappel — **jamais de sortie silencieuse**. Le DTO expose un champ discriminant (ex. `terminal: bool` + `last_reminder_at`).
...
10. **Liste à rappeler** `GET /api/v1/dunning/reminders` (ou `/invoices/to-remind` — nom à figer validate) — **RBAC Comptable+** : retourne les factures éligibles **groupées par contact**, chaque facture avec : `invoice_id`, `invoice_number`, `contact_id`, `contact_name`, `due_date`, `current_level`, `next_level`, `terminal`, `last_reminder_at`, **badge `has_email`** (false si `contacts.email` vide/NULL → non-cochable pour l'envoi lot en 21-6). Scopé company. DTO Response **sans `company_id`**.
11. **Toggle suspension** `PUT /api/v1/invoices/{id}/dunning-pause` (pause) + `.../dunning-resume` (reprise), ou une route toggle avec body — **RBAC Comptable+** : pose/lève `dunning_paused_at` (+ `dunning_paused_note` optionnelle à la pause). **Calquer exactement `mark_as_paid`** (`repositories/invoices.rs:1430-1556`) : `SELECT … FOR UPDATE` scopé company (`FIND_INVOICE_SCOPED_SQL`), verrou optimiste (`version = ?` dans le WHERE de l'UPDATE + `rows==0 → OptimisticLockConflict`), audit **dans la même tx**. Le DTO request porte `version` (validé `>= 0` → 400 explicite, cf. `MarkPaidRequest`/`validate_version` `invoices.rs:756-780`). **Garde anti-transition** (transposer `alreadyUnpaid` `invoices.rs:1468`) : reprise d'une facture **non suspendue** → `InvalidInput` 4xx (évite un faux 409 + audit spurieux). **Pause ET reprise auditées** (`invoice.dunning_paused` / `invoice.dunning_resumed`, acteur + facture, via `NewAuditLogEntry::from_current_user` ou `::user`). **Invariant anti-dissimulation (sécurité, item 10)** : une facture suspendue **reste dans la balance âgée et l'échéancier** — elle ne sort **que** de la liste « à rappeler » (l'AC 7 filtre `dunning_paused_at IS NULL` ; l'échéancier `list_due_dates` ne filtre PAS dessus). Scopé company (cross-tenant → 404, jamais 403).
12. **Rappel manuel** `POST /api/v1/invoices/{id}/reminders/manual` — **RBAC Comptable+** : enregistre une ligne `invoice_reminders` `channel='manual'`, `sent_to = NULL`, niveau + date + note fournis, `fee_amount` = snapshot du frais du niveau visé (config courante). Le cycle avance sans e-mail Kesh. **Gardes d'éligibilité** (facture validée, non payée ; niveau cohérent) → 4xx explicites. Audit `invoice.reminder_sent` (channel manual).
...
14. **Historique** `GET /api/v1/invoices/{id}/reminders` — **RBAC tous rôles authentifiés (lecture)** : liste des rappels d'une facture (annulés distinguables). Alimente la fiche facture (21-6).
...
17. **Compteurs de tests figés bumpés** — valeurs EXACTES (ground-truth 2026-07-15, **à revalider au dev** avant commit) :
    - `crates/kesh-api/src/exports/global.rs:278` `18 → 19` ; `:279` `18 → 19` ; `:296` `csv_count: 18 → 19` ; `:193` capacité `19 → 20` ; doc-comments en-tête `:3-15` (« 18 tables » → 19).
    ...
    - `crates/kesh-db/tests/migrations_fresh_install.rs:30-56` ajouter `invoice_reminders` à la liste `expected` (sémantique subset, non bloquant mais bonne pratique — `dunning_levels`/`cds` y figurent `:40,:43`).
...
19. **Tests éligibilité SQL** : facture non-échue → absente ; échue niveau 1 dû → présente niveau 1 ; après rappel niveau 1, niveau 2 dû à `sent_at + delay` ; suspendue → absente de la liste mais **présente échéancier/balance âgée** (invariant item 10) ; payée → absente ; dernier niveau atteint → **présente en terminal** (item 9) ; sans `due_date` → absente ; seed lazy déclenché à la 1re évaluation.
20. **Tests E2E routes** (`crates/kesh-api/tests/`) : liste groupée par contact + badge `has_email` ; toggle suspension (pause+reprise audités, facture suspendue absente de la liste) ; rappel manuel (ligne créée, cycle avance) ; annulation Admin (soft, exclue du MAX) ; **RBAC** : Comptable OK / Consultation 403 sur les mutations, Admin requis pour l'annulation ; **anti-IDOR** cross-tenant → 404 ; gardes d'éligibilité (payée/suspendue → 4xx).
...
**Éligibilité SQL — somme cumulée des délais** : `delay_days` est « depuis l'étape précédente » (entité `dunning_level.rs`). L'échéance du niveau N nécessite `Σ delay_days(1..N)` → **fenêtre `SUM(delay_days) OVER (ORDER BY level_number)`** sur `dunning_levels` (MariaDB 10.2+ supporte les window functions) OU self-join. Niveau 1 dû : `CURDATE() >= due_date + INTERVAL grace_period_days DAY + INTERVAL delay_days(1) DAY`. Niveau N>1 dû : `CURDATE() >= DATE(sent_at du dernier rappel non-annulé) + INTERVAL delay_days(N) DAY`. `grace_period_days` de `company_dunning_settings` (get-or-create/seed). Niveau courant = `COALESCE(MAX(r.level_number) WHERE r.cancelled_at IS NULL, 0)`. Prochain = plus petit `dunning_levels.level_number > courant`. Terminal si aucun `> courant`.
...
- **Backup auto-vérifié** : `backup_inventory_matches_schema` (`backup.rs:579`) — filet contre l'oubli de table.
