...
I want fixer deux incohérences code Epic 9.5 catégorie A : (1) régression locale `kesh-api::config::tests::*` 20/24 fail liée à la collision avec `.env` projet, (2) `audit_log.details_json` JSON keys incohérence entre 3 fonctions `emit_*_audit` (2 en camelCase Story 9-1/9-2a + 1 en snake_case Story 9-2b),
so that `cargo test -p kesh-api --lib config::tests` passe **24/24** localement même avec `.env` projet présent, et que les JSON keys `audit_log.details_json` soient uniformément **snake_case** dans les 3 fonctions audit (cohérent SQL JSON path `details_json->>'$.field_name'` future-proof, AC #23 spec 9-2b explicite + retro Epic 9 challenge C3).
...
1. **Given** un workspace Kesh avec `.env` projet contenant `KESH_HOST=0.0.0.0` + `KESH_TEST_MODE=true` (état réel observé en dev), **When** `cargo test -p kesh-api --lib config::tests` est exécuté (depuis racine repo, parallel ou serial), **Then** **24/24 tests passent** (zéro fail dû à pollution `.env`). Solution = isolation env explicite côté tests, pas dépendance ordre d'exécution.

2. **Given** la solution choisie pour AC #1 (e.g. extension `reset_env()` + nouvelles env vars purgées, OU `serial_test` crate + `#[serial(env)]`, OU `temp-env` scoped, OU `#[cfg(not(test))]` autour de `dotenvy::dotenv()` dans `from_env`), **When** documentée dans Dev Agent Record, **Then** justification écrite incluant : (a) cause root précise observée par diagnostic (e.g. `KESH_HOST` non purgée par `reset_env()`), (b) approche choisie + raison, (c) trade-offs vs alternatives.

3. **And** la solution **ne casse pas** les tests qui dépendent intentionnellement de `dotenvy::dotenv()` chargement `.env` (si des tests valident le comportement « lit `.env` correctement »). Si conflit, documenter explicitement la mitigation.
...
15. **Given** les commits, **When** lus, **Then** chaque commit a un scope clair (e.g. `fix(kesh-api): config::tests env isolation (24/24 local)` puis `refactor(kesh-api): audit_log JSON keys snake_case (emit_report_*_audit)`). Pas de commit mélangé Item 1 + Item 2 (les 2 items sont indépendants — facilite revert).
...
- [ ] **T3** Implémenter fix Item 1 (AC: #1, #2, #3, #4)
  - [ ] T3.1 Choisir approche parmi (a) extension `reset_env()` avec env vars manquantes, (b) `serial_test = "3"` crate + annotations `#[serial(env)]` sur les tests env-touching, (c) `temp-env = "0.3"` crate scoped, (d) `#[cfg(not(test))]` autour de `dotenvy::dotenv()` dans `from_env()`.
  - [ ] T3.2 Si nouvelle dep : ajouter à `[dev-dependencies]` de `crates/kesh-api/Cargo.toml`. Si refactor reset_env : modifier directement le fn ligne 711-740.
  - [ ] T3.3 Appliquer l'approche choisie. `cargo build --workspace -p kesh-api --tests` clean.
  - [ ] T3.4 Re-lancer `cargo test -p kesh-api --lib config::tests -- --test-threads=1` → vérifier **24/24** pass.
  - [ ] T3.5 Re-lancer avec parallel (`cargo test -p kesh-api --lib config::tests`) → vérifier **24/24** pass aussi (idéal mais pas obligatoire si `--test-threads=1` est imposé par convention CI).
...
- [ ] **T8** Tests + validation finale (AC: #8, #9, #10, #11, #13, #14)
  ...
  - [ ] T8.4 `cargo test -p kesh-api --lib config::tests` → 24/24 pass (re-vérif T3.4).
...
- [ ] **T9** Commits séparés (AC: #15)
  - [ ] T9.1 Commit 1 : `fix(kesh-api): config::tests env isolation — 24/24 pass local avec .env présent` (T2-T3, message body avec cause root + approche choisie).
...
### Cause root probable Item 1 — état au moment de la spec

Le fichier `config.rs:687-1278` a déjà :
- Un `env_lock()` `Mutex` global pour sérialiser les tests qui touchent env vars (ligne 700).
- Une fonction `reset_env()` qui `env::remove_var` pour `DATABASE_URL`, `KESH_PORT`, `KESH_HOST`, `KESH_ADMIN_USERNAME`, `KESH_ADMIN_PASSWORD`, `KESH_JWT_SECRET`, `KESH_JWT_EXPIRY_MINUTES`, `KESH_REFRESH_TOKEN_MAX_LIFETIME_DAYS`, `KESH_REFRESH_INACTIVITY_MINUTES` (lignes 711-740).

**Hypothèse cause root** : `reset_env()` purge `KESH_HOST` mais **PAS** `KESH_TEST_MODE` (vérifié par grep — variable `KESH_TEST_MODE` est setée ligne 1082+ mais pas listée dans `reset_env`). Si `.env` projet contient `KESH_TEST_MODE=true`, après `reset_env()` la var reste, et les tests qui assument absence de `KESH_TEST_MODE` au démarrage échouent.

**Solution la plus chirurgicale (T3.1 option a)** : étendre `reset_env()` pour purger AUSSI les env vars potentiellement polluées par `.env` projet :
- `KESH_TEST_MODE`
- `KESH_BACKEND_URL` (potentiellement set par dev local)
- Tout autre `KESH_*` que peut contenir `.env`

Diff trivial (~5 lignes ajoutées à `reset_env()`).

**Alternative T3.1 option b — `serial_test` crate** : ajouter `serial_test = "3"` à `[dev-dependencies]` + annoter chaque test env-touching avec `#[serial(env)]`. Plus invasif mais plus robuste long-terme. Trade-off : ~30 annotations à ajouter.

**Alternative T3.1 option c — `temp-env` crate** : `temp_env::with_var("KESH_TEST_MODE", None, || { Config::from_env() })`. Scoped, cleanup automatique. ~5-10 tests à refactoriser.

**Alternative T3.1 option d — `#[cfg(not(test))]` autour de `dotenvy::dotenv()`** : skip le chargement `.env` en mode test. Risque : casse les tests qui voulaient tester le comportement « lit `.env` ».

**Recommandation pré-spec** : option (a) — la moins invasive, fix le problème observé sans introduire de dépendance. Le dev choisit après diagnostic T2 si une autre option est nécessaire.
...
- **Tests existants à valider** :
  - `cargo test -p kesh-api --lib config::tests` — cible 24/24 pass.
...
(Vide à la création — sera renseigné post-dev et post-review avec résumé des cycles spec validate + code review.)
