...
7. Pour chaque fichier valide, une **facture importée « à compléter »** (`imported_supplier_invoices`, statut `to_complete`) est créée à partir des coordonnées QR (créancier nom/IBAN/QR-IBAN, référence, montant TTC, devise) + lien fichier archivé. **DC3 figé.** L'utilisateur la **complète** dans l'UI :
   ...
   - **Lignes** : chaque ligne = `description` (**obligatoire** — `NewSupplierInvoiceLine.description: String` NOT NULL + CHECK non-vide, `supplier_invoices.sql:83,100`) + `quantity` + `unit_price` (HT) + `vat_rate` + `expense_account_id` — **saisis à la complétion** (le QR ne fournit ni compte ni TVA). `description` pré-remplissable avec `staging.unstructured_message` si non-vide. Le pré-remplissage QR couvre IBAN/référence/montant TTC/devise (partie error-prone).
...
### Pass 4 — validate (Sonnet 4.6, 2026-06-29)
Passe de convergence, axe cohérence des patches Pass 3 (DC6/AC10/réconciliation) + mapping staging→`NewSupplierInvoice` champ par champ. **6 findings** : 0 CRITICAL, 0 HIGH, 4 MEDIUM, 2 LOW. Trend > LOW : **4**. Aucune régression sur les DC figés ; 11 ground-truth re-vérifiés OK (table dans le rapport). Tous patches étroits (1-3 lignes).
- **P4-M1** (MEDIUM) résolu : champ `warnings: Vec<String>` ajouté au schéma réponse (AC3) pour troncature/run concurrent.
- **P4-M2** (MEDIUM) résolu : corps des lignes complété (`description` NOT NULL + quantity/unit_price/vat_rate/expense_account_id, AC7/AC10) — ground-truth `NewSupplierInvoiceLine`.
- **P4-M3** (MEDIUM) résolu : méthode de réconciliation précisée (arrondi 2 déc. HALF_UP des deux membres, AC7).
