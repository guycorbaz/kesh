...
- **2026-09-26** — **revue de code P2** (Haiku, diff aplati `ac0a2fc9..c493cdcc`, prompt `25-1c-b2-review-prompt-p2.md`) — **0 au-dessus de LOW**, un LOW laissé tel quel : `i18n-keys.test.ts:193` nomme « la piste de contrôle » dans une généalogie datée (Story 25-1a), référence historique légitime. ⚠️ **Ce « 0 » a été vérifié par l'orchestrateur**, sur les axes que la lentille avait exercés sans preuve : elle cite des lignes fausses (`fiscal-years/+page.svelte:163,171` pour le champ numéro), n'a pas aplati les PDF et a « recompté » les gates en relisant le Change Log. Repris : la sous-section « Consulter le journal d'audit » contrôlée phrase par phrase contre `routes/audit_log.rs` (plafond `MAX_EXPORT_ROWS`, langue de l'installation, libellés de l'export, `actor_label` figé à l'écriture, « (clé API) »), `query-helpers.ts` (numéro entier > 0 exigeant un type) et la page (« — » pour l'identifiant 0) ; les trois PDF aplatis, césures recollées : 0 `??`, les sept phrases de contrôle présentes, 0 occurrence de « piste de contrôle » / « piste d'audit ». **Boucle close** : P1 1 HIGH / 1 MED → P2 0 > LOW ; la remédiation P1 ne touchait en code qu'un commentaire d'en-tête Svelte. Gate après remédiation : `check` 0 erreur, `lint-i18n-ownership` PASS, `audit-log-page.test.ts` 18/18 ; gate complet (817 / 2463 / E2E 224-7-19) antérieur à la remédiation, qui ne change aucun comportement.
