...
| Date | Étape | Note |
|---|---|---|
| 2026-09-26 | review P3 ciblée | **Une lentille Opus**, contexte frais, braquée sur le **seul commit** de la remédiation P2 (`b0399adf`), prompt versionné `25-5-a-review-prompt-p3-ciblee.md`, axes déclarés (non exercés : exécution des tests, `cargo check`, `test_schema_guard.rs`, PDF, requêtes des dépôts). **1 MEDIUM, 4 LOW.** ① MEDIUM : la garde de colonnes comptait sa propre liste (`30` littéral) et non `TABLES_EXPORTEES` — une table ajoutée demain au registre échappait à tout contrôle de colonnes → égalité d'**ensembles** avec le registre ; **mutation tuée** (`projects` retirée de la garde ⇒ « la garde de colonnes doit couvrir exactement `TABLES_EXPORTEES` »). LOW : CHANGELOG — e-mail, téléphone et site web de la société ajoutés à l'énumération ; motif de `client_number_uniq` rendu exact (dérivée de `client_number_canonical`, non exportée) ; une exemption dont la colonne entrerait dans l'en-tête rougit désormais ; la règle `_json` réduite au seul `bank_profiles.column_mapping`. Vérifié sans défaut par la lentille : ordre en-tête / valeurs des quatre tables élargies (22, 26, 11, 14 colonnes), formatage, aucun consommateur par position, découpage du squash, les six motifs. ⇒ **La remédiation ne touche aucune ligne de code de production** (`mod tests` et CHANGELOG) : **boucle close** au critère du CLAUDE.md. **Trend** : P1 Sonnet 4 HIGH / 5 MED → P2 Haiku 0 > LOW des lentilles, **1 HIGH de l'orchestrateur** sur l'axe qu'une lentille déclarait non exercé → P3 Opus ciblée 1 MED (test seul). **Gate ciblé** : `fmt` + `clippy` verts, `binary(exports_global_e2e) \| test(/csv_tables/)` **37/37**. **Gate backend complet** exécuté ensuite (base remise à zéro, `scripts/test-fast.sh`, tête `9ef1811c`) : **2466 / 2466** (4 ignorés) — 2463 de `main` + 3 (deux unitaires de P1, la garde de colonnes de P2). **E2E** ensuite (binaire et frontend buildés depuis le worktree, `kesh_e2e` reconstruite, montage complet, `smtpConfigured:true`) : **221 passés / 8 échoués / 19 ignorés, zéro régression** — les 7 de KF-029 et `sidebar-navigation:75` (pollution déjà relevée) ; `export-global.spec.ts` vert. Frontend non touché par la remédiation. **Revue close, gates complets : story `done`.** |
