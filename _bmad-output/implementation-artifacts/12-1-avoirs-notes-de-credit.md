...
- [ ] **12-1f — Tests d'intégration + E2E + doc** (AC: 15)
  ...
  - [ ] **Mettre à jour `crates/kesh-db/tests/migrations_upgrade_path.rs`** : l'assert fail-loud `assert_eq!(total, 35, …)` (~ligne 67) doit devenir `35 + N` (N = nombre de fichiers `.sql` créés en 12-1a) et `n_before_upgrade_window = total - (12 + N)` ajusté (~ligne 91). Sinon test ROUGE dès l'ajout des migrations.
