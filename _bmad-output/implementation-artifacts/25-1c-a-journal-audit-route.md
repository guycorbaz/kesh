...
`main` (`git rebase --onto origin/main story/25-1c-zero-audit-company-id`) **après** le squash-merge
...
3. **Affichage sobre** (pour la 25-1c-b) : la route renvoie les **codes bruts** — `action`,
   `entity_type` — et le détail JSON tel quel. Aucune traduction côté serveur.
...
- **UTC** : MariaDB tourne en UTC (`@@system_time_zone = UTC`, `NOW() = UTC_TIMESTAMP()`, vérifié le
  ...
  UTC**. À écrire dans le doc-comment.
...
de consultation (story 3.5 ou post-MVP) ») devient faux et est corrigé : la consultation passe par
...
- **Dates** reçues en `String` et parsées dans le handler (`journal_entries.rs:291-304`) : format
  invalide ⇒ **400 `VALIDATION_ERROR`** nommant le paramètre ; `dateFrom > dateTo` ⇒ 400.
...
|---|---|---|
| `id` | `id` | — |
| `createdAt` | `created_at`, **rendu en UTC explicite** : `2026-09-15T14:26:33.123Z` | `NaiveDateTime` sérialise **sans** fuseau ; l'écran doit savoir que c'est de l'UTC |
...
- **neuf colonnes**, en-têtes traduits dans la `accounting_language` de la société
...
`entity_id = AUDIT_ENTITY_ID_NONE`, via `NewAuditLogEntry::for_actor(user_id, api_key_id, …)` ; détails
...
**15. Dix clés, dans les quatre catalogues** (`crates/kesh-i18n/locales/{fr,de,it,en}-CH/messages.ftl`),
en un bloc commenté `# --- Journal d'audit — export CSV (Story 25-1c-a) ---` : les **neuf** en-têtes
de l'AC 12 et `audit-log-export-error-too-large` (9 + 1). ⚠️ Le compte se **recompte depuis la
source** à l'implémentation — `grep -c '^audit-log-' crates/kesh-i18n/locales/*/messages.ftl` doit
rendre 10 dans chacune des quatre locales. Vocabulaire de l'arbitrage 4 ;
...
c'est la consultation du journal d'audit **depuis l'application** ») et `README.md:218` (« consultable
...
- Comptable ⇒ 200, Admin ⇒ 200, **Consultation ⇒ 403**, sans jeton ⇒ 401 ;
...
- 400 `VALIDATION_ERROR` : `dateFrom` invalide, `dateFrom > dateTo`, `entityId` sans `entityType`,
  `entityId <= 0`, `action` de 65 caractères ;
...
- ⛔ **injection** : un `actor_label` posé à `=HYPERLINK("x")` sort préfixé d'une apostrophe ;
...
- **audit de l'export** : après un export, **une** entrée `audit_log.exported` dont `row_count` est
  exact ; **aucune** entrée après une consultation.
...
- [ ] **T0 — Rebase** sur `main` si la PR #437 est mergée (cf. en-tête), puis remise à zéro
...
- [ ] **T9 — Propagation** : `grep -rnE "aucune route|post-MVP|story 3\.5" crates docs/manual/fr/*.tex`
...
| `crates/kesh-i18n/locales/{fr,de,it,en}-CH/messages.ftl` | dix clés |
...
- **Fuseau** : MariaDB en UTC — `@@system_time_zone = UTC`, `NOW()` = `UTC_TIMESTAMP()`.
...
- `user-manual.tex:498-503` et `README.md:218`, qui restent vrais.
...
- `repositories/journal_entries.rs:737-864` — **patron de la lecture paginée**
