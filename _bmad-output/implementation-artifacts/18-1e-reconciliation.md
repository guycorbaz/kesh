...
- **T-E1 — Helper solde 2200 ventes** (`vat_report.rs`, jumeau de `recoverable_balance` l.156-178) :
  ...
  > **L-1 (Pass 3 Opus — limitation à documenter)** : le helper suppose le compte
  > `default_vat_payable_account_id` **stable sur la période**. Si l'admin reconfigure ce compte en cours de
  > période, les factures déjà validées portent leur ligne TVA sur l'**ancien** compte tandis que le solde de
  > référence ne somme que le compte **courant** → delta parasite. C'est un signal d'investigation
  > arguablement **légitime** (livres réellement scindés sur 2 comptes), pas un faux positif nuisible, et une
  > action admin rare. Le mentionner dans le doc-comment `///` du helper.
...
| 3 | Opus 4.8 | 0 ✅ | **Catch-architectural : CONVERGENCE.** Design prouvé sain, ground-truth 7/7 : (1) **delta==0 nominal par construction** — `generate_invoice_journal_lines` (`invoices.rs:933-995`) utilise le MÊME `line_vat_amount` agrégé par taux en `BTreeMap` que `generate` (`vat_report.rs:100-105`) → due dérivée == crédit 2200 par taux, y compris multi-factures (associativité) ; (2) **factures annulées sûres** — AUCUNE API de cancel en prod (`invoices.rs:2790` SQL direct), pas de contre-passation orpheline, `status='validated'` exclut symétriquement des 2 côtés ; (3) **aucun autre flux n'écrit 2200** — `mark_as_paid` (`invoices.rs:1256`) ne touche ni `journal_entry_id` ni ligne 2200, seul `validate_invoice` poste → DC5-iso étanche ; (4) **`entry_date = invoice_before.date`** (`invoices.rs:1141`) → filtre `i.date` cohérent avec `je.entry_date`, pas d'asymétrie période ; (5) **pas de CHECK DB balance** (`20260412000001:12`, seul `chk_jel_debit_credit_exclusive` par ligne) → tests (b)/(f) fabricables par UPDATE SQL direct (réduire crédit 81.00→80.995 valide, `credit>0` OK) ; (6) seuil (f) 0.005→ok / 0.01→delta atteignable ; (7) mapping AC↔test complet, aucun test orphelin. **2 LOW** : L-1 reconfiguration compte mid-période → delta légitime (doc-comment helper, intégré) ; L-2 test (h) CSV seul (PDF binaire, cohérent 18-1d, acceptable v0.2). **0 > LOW.** |

**Trend findings > LOW** : Pass 1 (Sonnet) 5 (2H+3M) → Pass 2 (Haiku) 3 (1H+2M) → Pass 3 (Opus) **0 ✅**. Rotation Sonnet→Haiku→Opus, contexte frais, grep ground-truth. **Catch décisif Pass 1 Sonnet** (F1 4 struct-literals compilation + F2 `dec!` dev-only) + **Pass 2 Haiku** (F-HIGH-1 i18n exports orphelins, 0 hallucination) + **Pass 3 Opus** (preuve delta==0 par construction + étanchéité DC5-iso : aucun cancel/mark_as_paid ne pollue 2200). **Cycle validate CONVERGÉ Pass 3, 0 > LOW. Prochaine : `bmad-dev-story 18-1e` (Opus 4.8).**

### `bmad-code-review 18-1e` — cycle adversarial post-implémentation

#### Review Findings — Pass 1 (Sonnet 4.6)

3 axes (Acceptance Auditor, Blind Hunter, Edge Case Hunter), diff `04162dc`, grep ground-truth.
**Acceptance Auditor : AC1-AC12 tous SATISFIED** (chaque AC mappé à la ligne de code + test). **Blind Hunter : 0 > LOW** (signe `credit−debit` Liability correct ; pas de double-comptage multi-taux — symétrie SUM ; DC5-null par `match None` vérifié l.177 ; `fetch_optional`/`unwrap_or` robuste l.146 ; seuil `Decimal::new(1,2)`==0.01 exact, bornes OK ; delta==0 nominal par construction confirmé). **Edge Case Hunter : 0 > LOW**. **3 LOW** :

- [x] **[Review][Patch]** LOW-1 — pas de test pour delta **négatif** (crédit 2200 gonflé > due). **APPLIQUÉ** : test (i) `reconciliation_negative_delta_detected` (91 > 81 → delta −10, status "delta", symétrie `abs()`). 9 tests intégration.
- LOW-2 — pas d'assertion contenu PDF (binaire difficile à asserter ; AC11(h) ne requiert que le CSV ; cohérent 18-1d). **dismiss** (acceptable v0.2).
