...
9. **AC7 — aucun test ne verrouille le français.** Les **deux** greps de contrôle de la 23-3b, plus
   ...
   trouve HUIT verrous réels sur des clés de ce périmètre** *(le texte disait « NEUF » : 7 + 1 = 8,
   corrigé en revue de code — le Dev Agent Record et `sprint-status.yaml` disaient bien huit)* : `toHaveText('Défaut')` et
...
#### ⚠️ Les décomptes RÉVISÉS en passe 2 de revue — et pourquoi le diff textuel ment

| grandeur | valeur | méthode |
|---|---|---|
| clés **ajoutées** au catalogue | **91** | différence d'ENSEMBLES entre `main` et la branche |
| clés supprimées | **0** | idem |
| parité par locale | **1520** *(1429 + 91)* | `grep -c` sur les quatre |
| décrément d'allowlist | **93** — `166 → 73` | inchangé |

⚠️ **Le diff textuel compte 96 ajouts, et il a tort.** Il voit les 96 clés écrites en T4-T8 sans
voir que **cinq ont été retirées ensuite**, dans un commit de revue — un ajout puis un retrait dans
deux commits d'une même branche laissent une trace `+` que `grep` sur le diff additionne. **Seule
la différence d'ensembles entre les deux états dit la vérité.** C'est la version chiffrée de la
règle du dépôt : *recompter depuis la source, jamais depuis un compte rendu intermédiaire.*

⚠️ **91 et non 96, parce que cinq clés étaient des DOUBLONS** — cf. le finding de la passe 2
ci-dessous.

#### AC5-zéro — la relecture des 92 replis français, consignée

*(⚠️ **92 est le nombre de replis MOISSONNÉS relus**, distinct des 91 clés finalement écrites et des
93 entrées d'allowlist retirées : le moissonneur relève les sites, l'allowlist les clés, et les trois
scissions ajoutent des clés sans ajouter de site. Les trois grandeurs ne coïncident pas, et une
lentille de revue a eu raison de demander laquelle était laquelle.)*

⚠️ **Cette section manquait, et son absence était un HIGH de la revue de code.** La relecture a bien
eu lieu avant l'écriture — mais un AC qui exige qu'elle soit *consignée* n'est pas honoré par une
relecture silencieuse. C'est le mode d'échec que cet AC a été ajouté pour fermer, et il rouvrait.

**Méthode** : les 92 replis moissonnés extraits par domaine, lus un par un contre le glossaire et
les conventions du dépôt. **Ce qu'elle a trouvé** :

| relevé | traitement |
|---|---|
| « Générer un lot », « Générer le lot », « Lot généré » | ⚠️ contredisaient l'arbitrage de la 23-3b — passés à « Créer » (AC4) |
| « Référence message » | terme ISO 20022 → `MsgId`, arbitrage de Guy |
| apostrophes : 4 replis en `’` typographique, le reste en `'` ASCII | ⚠️ **entrés VERBATIM** — la convention du dépôt (23-1b) l'impose, et le catalogue mélange déjà les deux (187 ASCII / 31 typo). Relevé, non corrigé |
| `payment-batches-lot` et `-col-id` portent tous deux « Lot » | **légitime** : un titre de fiche (« Lot #12 ») et un en-tête de colonne peuvent diverger en traduction |
| « Clôture… » (`closing`) contre « Fermer » (`-form-close`) | ⚠️ **le couple de KF-041** — tranchés séparément, `Abschluss…` contre `Schliessen` |
| minuscules initiales (`archivé`, `ouvrir et télécharger…`) | **verbatim** — 29 clés du catalogue commencent déjà par une minuscule (décision relue de la 23-1b) |

⚠️ **Ce que la relecture N'A PAS vu, et que la revue de code a rattrapé** : les trois **replis du
code source** disaient encore « Générer » alors que les valeurs du catalogue disaient « Créer ». La
relecture portait sur les valeurs à écrire, pas sur les littéraux qu'elles remplacent — **le repli
s'affiche pourtant dès que le dictionnaire est vide.** Corrigé en revue.
...
| date | passe | résultat |
|---|---|---|
| 2026-08-22 | **passe 3** de `bmad-code-review` | ⚠️ **0 CRITICAL · 0 HIGH · 0 MEDIUM · 0 LOW retenus — les DEUX findings sont réfutés au sol.** **Critère d'arrêt de la Review Iteration Rule ATTEINT** (trend `0C/1H/3M/1L` → `0C/1H/8M/5L` → **0**), après trois passes sur trois modèles distincts (Sonnet, Opus, Haiku). ⚠️ **Le contrôle décisif est la MUTATION, et elle prouve la garde** : refusionner `payment-batches-line-amount` dans `payment-batches-col-total` — c'est-à-dire annuler l'une des trois scissions — fait rougir **trois tests** (`aucune clé ne porte deux replis`, `les trois paires scindées restent SIX clés`, et la borne du relevé). La garde protège réellement ce qu'elle prétend protéger ; ce n'est pas une garde de façade. **Premier finding réfuté** : *« `closing` retirée de l'allowlist mais toujours appelée »*. ⚠️ **C'est le sens de l'allowlist qui est inversé** — elle liste les clés demandées **et ABSENTES** des catalogues ; `closing` est traduite dans les quatre locales, donc elle doit en sortir. Le correctif proposé — l'y réinscrire — **casserait** la garde, et `i18n-keys.test.ts`, qui contrôle exactement cette relation, est vert. **Second finding réfuté** : *« parité 1519, pas 1520 »*. Mon motif de comptage (`^[a-zA-Z][a-zA-Z0-9_-]* = `) capte les deux clés à **underscore** (`email-templates-type-invoice_send`, `…_reminder`) qu'un motif plus étroit rate. Contrôle croisé : **1429 sur `main` + 91 ajoutées par différence d'ensembles = 1520**, égal à la mesure directe. *(La même famille d'erreur avait produit le « 1427 » corrigé en 23-3b — un motif trop étroit rend un compte faux qui a l'air juste.)* **Gates rejoués par la lentille d'exécution** : gardes 161/161, suite complète **713/713** (72 fichiers), `check` 0 erreur, lint PASS, `kesh-i18n` **29/29** dont `parity_between_locales`, dépôt rendu propre. |
| 2026-08-22 | **passe 2** de `bmad-code-review` | **0 CRITICAL · 1 HIGH · 8 MEDIUM · 5 LOW** (Opus ×2, 151 vérifications au sol). ⚠️ **Le HIGH est le défaut que cette story COMBAT, commis par elle** : j'avais écrit cinq clés génériques (`save`, `saving`, `cancel`, `create`, `creating`) au catalogue alors que des jumelles `common-*` existaient, **déjà traduites** — et trois valeurs cibles divergeaient. Un germanophone aurait lu « Wird gespeichert… » sur un écran et « Speicherung… » sur un autre, pour la même action. ⚠️ **J'avais pourtant posé le bon geste DEUX FOIS** dans la même story (`common-loading`, `bank-accounts-labels-qr-iban` « réutilisée plutôt que dupliquée ») sans le généraliser. Les 12 sites sont routés vers `common-*`, les 5 doublons retirés : **91 clés au lieu de 96**. ⚠️ **Et la garde a attrapé mon correctif INCOMPLET** — `vat-rates` appelait `i18nMsg` là où les autres appelaient le relais `msg`, mon script ne cherchait qu'une forme ; `i18n-keys.test.ts` a rougi sur les deux clés orphelines. **MEDIUM** : `loading` restait sur quatre sites dont trois du domaine, avec un repli divergent de `common-loading` — la divergence avait été *déplacée du code vers l'écran*, entre pages sœurs ; le registre italien de 12 messages contredisait le précédent direct de la 23-3 (`Seleziona` contre `Selezionate`, même écran, une story d'écart) ; `Abschluss` nu portait deux concepts, la famille exacte de KF-041 ; **trois entrées du glossaire citaient comme attestantes des clés que la story CRÉE** — une citation circulaire, et `projet racine` n'est attesté nulle part ; un sélecteur E2E visait « Continuer » sur un bouton qui dit désormais « Enregistrer » — **une mine**, inoffensive aujourd'hui parce que le test échoue plus tôt sur un défaut préexistant. **LOW** : le repli de `payment-batches-msg-id` disait encore « Référence message » quand le catalogue dit `MsgId` — deux poids deux mesures avec les replis « Créer » ; un docstring annonçait 187 au-dessus d'une table totalisant 186. ⚠️ **Une lentille a corrigé MA consigne** : « parité 1525 » confondait deux grandeurs — 1529 est `sitesTotal`, le catalogue en compte un autre. Recompté par différence d'ensembles : **91 ajoutées, 0 supprimée, 1520 par locale**. |
| 2026-08-22 | **passe 1** de `bmad-code-review` | **0 CRITICAL · 1 HIGH · 3 MEDIUM · 1 LOW** (Sonnet ×3, dont **une lentille défaillante** — l'Edge Case Hunter a rendu un rapport vide après 64 appels d'outils ; comptée comme non exécutée, pas comme « rien trouvé »). ⚠️ **Le HIGH le plus utile est celui que la relecture avait manqué** : les trois **replis du code source** disaient encore « Générer » alors que le catalogue disait « Créer ». Sans impact en marche normale — le catalogue gagne —, mais **le repli s'affiche dès que le dictionnaire est vide**, et AC4 exige que le verbe disparaisse du domaine. La relecture d'AC5-zéro portait sur les valeurs à écrire, pas sur les littéraux qu'elles remplacent. ⚠️ **Second HIGH : AC5-zéro n'était pas CONSIGNÉ** — la relecture avait eu lieu, mais un AC qui exige qu'elle soit consignée n'est pas honoré par une relecture silencieuse. Section ajoutée, avec ce qu'elle a réellement trouvé. **MEDIUM** : `loading` porte **deux replis divergents** (« Chargement… » / « Chargement... ») et j'y avais ajouté deux sites au lieu d'utiliser `common-loading`, que ma propre table AC6-bis annonçait — c'est le défaut que cette story existe pour corriger, commis sur une clé que sa section « hors périmètre » signalait déjà ; AC7 se contredisait (« NEUF » verrous pour 7 + 1 = 8) ; et l'assertion affaiblie de `projects.spec.ts` ne discriminait plus la page. ⚠️ **Un HIGH RÉFUTÉ au sol, et sa réfutation est mesurée** : « le registre italien viole sa convention de courtoisie ». **Le glossaire documente l'inverse** — `it-CH` en 2ᵉ personne du **singulier** —, et `main` rend **91 impératifs singuliers contre 1 pluriel**. Les formes de courtoisie y sont réservées aux **phrases avec possessif** : mon `projects-empty` (« Create il vostro primo progetto ») est le jumeau mot pour mot de `homepage-invoices-empty-guided` (« Create la vostra prima fattura »). Un seul point du finding tenait — `Archiviare` détonnait à côté de `Modifica` —, corrigé en `Archivia` d'après l'écran de référence. ⚠️ **La dérogation au splitting n'a PAS à être rouverte** : la sévérité (0C/1H/3M) est inférieure au seuil écrit (1C/7H/7M) sur les trois catégories. |
