...
\begin{tabularx}{\linewidth}{>{\bfseries\ttfamily}lXl}
    ...
    KESH\_DOCUMENTS\_DIR & Dossier d'archivage des justificatifs importés (hors DB). & \texttt{/data/documents} \\
    KESH\_INBOX\_DIR & Dossier inbox scruté à l'import de factures. & \texttt{/data/inbox} \\
    KESH\_INBOX\_MAX\_FILE\_BYTES & Taille max d'un fichier inbox (octets). & \texttt{26214400} (25 Mo) \\
    KESH\_INBOX\_MAX\_FILES\_PER\_RUN & Nombre max de fichiers traités par import. & \texttt{200} \\
    KESH\_INBOX\_MAX\_PDF\_PAGES & Pages PDF max rendues par fichier (anti-DoS). & \texttt{20} \\
    \bottomrule
\end{tabularx}
