...
\subsection{Création du compte administrateur initial}

Au tout premier démarrage de Kesh sur une base de données vide, le \emph{bootstrap} crée automatiquement le compte administrateur défini par les variables \keshpath{KESH\_ADMIN\_USERNAME} et \keshpath{KESH\_ADMIN\_PASSWORD} du fichier \keshpath{.env}, ainsi qu'une \textbf{company provisoire} (placeholder) à laquelle ce compte est rattaché.

Cette company provisoire débloque le premier accès : sans elle, l'administrateur ne pourrait pas être créé (contrainte d'intégrité \keshcommand{users.company\_id NOT NULL}), et le wizard d'onboarding — protégé par l'authentification — serait inatteignable. La company provisoire est renommée et complétée lors de l'onboarding.

\subsubsection{Procédure}

\begin{enumerate}
    \item Renseigner \keshpath{KESH\_ADMIN\_USERNAME} et \keshpath{KESH\_ADMIN\_PASSWORD} (mot de passe fort, ≥ 12 caractères) dans \keshpath{.env} \emph{avant} le premier démarrage.
    \item Démarrer la stack (\keshcommand{docker compose up -d}).
    \item Ouvrir \keshpath{https://compta.exemple.ch} dans un navigateur et se connecter avec ces identifiants.
    \item Le wizard d'onboarding démarre automatiquement.
    \item \textbf{Changer immédiatement le mot de passe administrateur} après la première connexion (les logs affichent un rappel « CHANGEZ LE MOT DE PASSE » au premier démarrage).
\end{enumerate}

\begin{keshwarning}
Tant que le mot de passe administrateur du \keshpath{.env} n'a pas été changé, toute personne ayant accès au fichier peut se connecter. Restreindre les permissions du fichier \keshpath{.env} (\keshcommand{chmod 600}) et changer le mot de passe dès la première connexion. La récupération en cas de mot de passe perdu (\emph{break-glass}) est traitée par une procédure dédiée — voir les notes de version v0.1.1.
\end{keshwarning}
