...
% Cette définition est nécessaire car le SVG est dans shared/, pas dans fr/
...
\begin{itemize}
    ...
    \item Les boîtes colorées indiquent : \keshnote{} (information), \keshtip{} (astuce), \keshwarning{} (attention), \keshdanger{} (danger).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Déploiement} : Docker Compose, image officielle \keshpath{guycorbaz/kesh:latest} sur Docker Hub.
...
\begin{keshwarning}
\textbf{Ne jamais exposer le port 3000 (API Axum) ni le port 3306 (MariaDB) directement sur Internet}. Toujours passer par un reverse proxy HTTPS qui termine TLS et fait suivre vers \texttt{localhost:3000}.
\end{keshwarning}
...
\begin{tabularx}{\linewidth}{>{\bfseries\ttfamily}lXl}
    ...
    KESH\_PORT & Port d'écoute de l'API. & \texttt{3000} \\
...
\subsection{Création du super-administrateur initial}

Au tout premier démarrage de Kesh sur une instance vide, le système expose un endpoint d'\textbf{onboarding initial} qui permet de créer le premier compte super-administrateur \emph{sans authentification}.

Cet endpoint est désactivé automatiquement dès qu'un utilisateur existe dans la base. Il est donc critique de \textbf{créer le super-admin immédiatement après le premier démarrage}.

\subsubsection{Procédure via l'interface Web}

\begin{enumerate}
    \item Ouvrir \keshpath{https://compta.exemple.ch} dans un navigateur.
    \item L'écran d'onboarding initial apparaît automatiquement (titre « Bienvenue dans Kesh »).
    \item Saisir l'email, le mot de passe (fort, ≥ 12 caractères) et la confirmation.
    \item Choisir la langue par défaut de l'interface.
    \item Cliquer sur « Créer le compte administrateur ».
    \item Se connecter avec les identifiants créés.
\end{enumerate}

\subsubsection{Procédure via la CLI (sans navigateur disponible)}
...
\begin{keshwarning}
Si vous oubliez l'étape d'onboarding initial et qu'un attaquant arrive sur votre instance avant vous, il peut créer le super-admin à votre place. \textbf{Bloquez l'accès réseau jusqu'à création du super-admin}, ou désactivez l'endpoint via la variable \texttt{KESH\_DISABLE\_INITIAL\_ONBOARDING=true} et utilisez la CLI.
\end{keshwarning}

\subsection{Création de la première company}

Une fois connecté en tant que super-administrateur, créer la première company :

\begin{enumerate}
    \item Aller dans \emph{Paramètres} → \emph{Sociétés}.
    \item Cliquer sur « Nouvelle société ».
    \item Renseigner les informations légales (raison sociale, IDE, adresse, langue, timezone, devise).
    \item Choisir le plan comptable de référence (Sterchi, KMU, ou import personnalisé CSV).
...
\begin{itemize}
    \item Une nouvelle image Docker \texttt{guycorbaz/kesh:X.Y.Z} et \texttt{guycorbaz/kesh:latest} sont poussées sur Docker Hub.
