...
\begin{itemize}
    ...
    \item Champs : \texttt{id}, \texttt{user\_id}, \texttt{actor\_label} (le nom de l'auteur \emph{au moment} de l'écriture), \texttt{actor\_type} et \texttt{actor\_api\_key\_id}, \texttt{company\_id} (la société de l'auteur au moment de l'écriture, vide quand elle est indéterminable), \texttt{action}, \texttt{entity\_type}, \texttt{entity\_id}, \texttt{details\_json}, \texttt{created\_at}. ⚠️ \emph{Chaque entrée porte la société de son auteur, mais le journal ne se \textbf{consulte} pas encore~: aucune route ni aucun écran ne permet de le lire depuis l'application --- c'est le sujet de l'issue \#378.}
...
comptes utilisateurs. La \textbf{réinitialisation des données de démonstration} efface encore
