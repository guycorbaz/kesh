...
\textbf{Cette fonction sert à restaurer \emph{votre} installation} --- la remettre dans un état antérieur, ou la déplacer telle quelle vers un autre serveur. Ce n'est \textbf{pas} un outil pour verser les données d'une \emph{autre} installation dans la vôtre~: c'est techniquement possible, et les conséquences sur la piste de contrôle sont décrites plus bas (section \emph{Conformité OLICo}).
...
\begin{itemize}
    ...
    \item \textbf{Audit} : les mutations effectuées via une clé sont tracées dans \texttt{audit\_log}. ⚠️ \emph{L'attribution n'est pas encore complète~: une quinzaine de dépôts de données inscrivent \texttt{actor\_type = 'user'} au lieu de \texttt{'api\_key'}, si bien qu'une action faite par une intégration peut être journalisée comme venant d'une personne. L'imputabilité humaine est préservée --- c'est le créateur de la clé qui figure --- mais l'information «~c'était une intégration~» se perd. Correction suivie à l'issue \#431.}
\end{itemize}
...
\begin{itemize}
    ...
    \item Champs : \texttt{id}, \texttt{user\_id}, \texttt{actor\_label} (le nom de l'auteur \emph{au moment} de l'écriture), \texttt{actor\_type} et \texttt{actor\_api\_key\_id}, \texttt{company\_id} (la société de l'auteur au moment de l'écriture, vide quand elle est indéterminable), \texttt{action}, \texttt{entity\_type}, \texttt{entity\_id}, \texttt{details\_json}, \texttt{created\_at}. ⚠️ \emph{Chaque entrée porte la société de son auteur. Le journal se \textbf{consulte} désormais depuis l'API~: une liste paginée et filtrable, un vocabulaire des codes traduits, et un export CSV --- réservés aux rôles \textbf{Comptable} et \textbf{Administrateur}, et refusés aux clés API. L'\textbf{écran}, lui, reste à venir (issue \#378). La consultation ne filtre que votre société, et ne s'inscrit pas elle-même au journal.}
...
\begin{itemize}
    ...
    ⚠️ \textbf{Une seule voie fait encore disparaître une écriture, et elle est nommée}~: la \textbf{dévalidation} d'une facture (depuis la v0.12.1), qui la repasse en brouillon et supprime l'écriture que sa validation avait créée. Ce n'est pas une brèche dans l'immutabilité mais une \textbf{décision de facturation}~: elle est journalisée sous son propre nom, refusée sous huit motifs --- dont l'exercice clos, le verrou de période, la contre-passation et l'envoi au client ---, et elle ne touche jamais une écriture saisie à la main.
    \item \texttt{audit\_log} alimenté à chaque création d'écriture, avec instantané (condition 1). \emph{Il conserve aussi les modifications et suppressions antérieures au gel, ainsi que celles des autres entités.}
...
possible, et les entrées d'audit \textbf{déjà présentes chez vous} se retrouveraient alors
rattachées à la société importée~: la piste de contrôle mêlerait deux comptabilités sans
...
\begin{itemize}
    ...
    \item al. 3 : support libre + lien garanti + lisibilité possible → garanti par \texttt{audit\_log} + SHA-256 + format CSV/PDF ouverts du ZIP d'export. ⚠️ \emph{Réserve~: le ZIP d'export ne contient pas encore \texttt{audit\_log} (19 tables sur 38). La piste existe et est conservée, mais elle ne voyage pas avec l'export --- issue \#386.}
\end{itemize}
...
\begin{itemize}
    ...
    \item Art. 3 : intégrité (modification apparente) → les corrections passent par \textbf{contre-passation}, qui laisse l'écriture d'origine en place, et \texttt{audit\_log} en conserve la trace. ⚠️ \emph{La couverture du journal n'est pas encore complète, mais la réserve a changé d'objet~: la gestion des utilisateurs, la modification de la société et les pièces importées y sont désormais inscrites. Ce qui manque est la séquence d'installation (issue \#434) et les gestes de session (issue \#435).}
...
\begin{description}
    \item[Audit-trail] Trace des actions métier (table \texttt{audit\_log}), inaltérable sur une installation en service. ⚠️ \emph{La couverture n'est pas totale~: voir les réserves de la section «~Conformité~».}
