...
\medskip
\emph{Note de version.} Jusqu'à la v0.9.0, une clé \texttt{read-write} créée par un Administrateur atteignait bien les routes d'administration --- elle pouvait donc créer un compte administrateur, ce qui rendait la révocation inopérante. Ce défaut est corrigé ; si une intégration existante appelait de telles routes, elle reçoit désormais \texttt{403 API\_KEY\_ADMIN\_FORBIDDEN}.
\end{keshwarning}
