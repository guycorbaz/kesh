...
\paragraph{Recovery de mot de passe par email (SMTP).}
Optionnelles tant que \texttt{KESH\_FEATURE\_FORGOT\_PASSWORD=false}. Si activé,
la config SMTP complète est \textbf{obligatoire} (démarrage refusé sinon).
\begin{tabularx}{\linewidth}{>{\bfseries\ttfamily}lXl}
    \toprule
    \keshtableheader{Variable} & \keshtableheader{Description} & \keshtableheader{Défaut} \\
    \midrule
