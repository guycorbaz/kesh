...
La quasi-totalité des actions métier mutantes est enregistrée dans la table \texttt{audit\_log} --- 87 des 105 routes qui écrivent en base au moment de la rédaction. ⚠️ \emph{Les exceptions sont connues et suivies~: la séquence d'installation (issue \#434) et les gestes de session --- connexion, déconnexion, changement de son propre mot de passe (issue \#435). Trois autres routes n'écrivent rien parce qu'elles ne modifient rien.} La liste complète, route par route, est tenue dans le dépôt et contrôlée par un test~:
...
\begin{itemize}
    ...
    \item Champs : \texttt{id}, \texttt{user\_id}, \texttt{actor\_label} (le nom de l'auteur \emph{au moment} de l'écriture), \texttt{actor\_type} et \texttt{actor\_api\_key\_id}, \texttt{action}, \texttt{entity\_type}, \texttt{entity\_id}, \texttt{details\_json}, \texttt{created\_at}. ⚠️ \emph{Il n'y a pas de colonne \texttt{company\_id}~: la table est globale. Une consultation par société l'exige, et c'est le sujet de l'issue \#378.}
