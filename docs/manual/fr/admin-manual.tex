...
\begin{itemize}
    ...
    \item Art. 3 : intégrité (modification apparente) → les corrections passent par \textbf{contre-passation}, qui laisse l'écriture d'origine en place, et \texttt{audit\_log} en conserve la trace. ⚠️ \emph{La couverture du journal n'est pas complète~: la gestion des utilisateurs et quelques autres opérations n'y sont pas encore inscrites.}
...
\begin{description}
    \item[Audit-trail] Trace de toutes les actions métier (table \texttt{audit\_log}), inaltérable sur une installation en service --- voir les réserves de la section «~Conformité~».
