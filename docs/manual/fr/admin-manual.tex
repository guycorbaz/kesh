...
\begin{itemize}
    \item \textbf{Un verrou de période} arrête les écritures à une date choisie, indépendamment de la clôture annuelle~: aucune écriture ne peut plus être datée d'une période déjà déclarée (typiquement un trimestre TVA déposé), par aucun chemin --- ni la saisie manuelle, ni les flux automatiques. Le \textbf{déverrouillage} est réservé à un administrateur, exige un \textbf{motif} et s'inscrit au journal d'audit ; une restauration de sauvegarde qui ferait reculer la borne y laisse également une trace (\texttt{books.restored}).
