...
\begin{itemize}
    \item \textbf{Un verrou de période} arrête les écritures à une date choisie, indépendamment de la clôture annuelle~: aucune écriture ne peut plus être datée d'une période déjà déclarée (typiquement un trimestre TVA déposé), par aucun chemin --- ni la saisie manuelle, ni les flux automatiques --- et aucune écriture de la période ne peut plus en être \textbf{supprimée}, pas même avec la facture validée qui la porte. Le \textbf{déverrouillage} est réservé à un administrateur, exige un \textbf{motif} et s'inscrit au journal d'audit ; une restauration de sauvegarde qui ferait reculer la borne y laisse également une trace (\texttt{books.restored}).
    \item \textbf{Une écriture enregistrée n'est plus ni modifiable ni supprimable} --- l'application refuse les deux, sans exception et quel que soit le rôle ; la correction passe par la \textbf{contre-passation}, qui laisse l'origine et sa correction dans les livres (CO Art.~958f : l'exigence n'est pas qu'on ne se trompe jamais, c'est que la correction soit apparente). L'immutabilité n'est donc plus seulement \emph{tracée}, elle est \textbf{imposée}.
