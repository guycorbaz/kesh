...
\begin{itemize}
    ...
    \item Les \textbf{entreprises individuelles} et \textbf{sociétés de personnes} avec un chiffre d'affaires inférieur à CHF 500'000, qui peuvent légalement se limiter au régime « recettes-dépenses + patrimoine » (CO Art. 957 al. 2). ⚠️ \emph{Kesh ne propose pas ce régime simplifié : il tient une comptabilité en partie double, ce qui est admis mais plus exigeant que ce que la loi leur impose.}
    \item Les \textbf{personnes morales} (SA, Sàrl, coopératives, associations, fondations).
    \item Les \textbf{fiduciaires} gérant plusieurs dossiers clients. ⚠️ \emph{Un dossier = une instance ou un compte dédié : l'interface ne permet pas encore de basculer entre sociétés.}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Arrêt du serveur pendant l'envoi} : l'email part en tâche de fond après la réponse ; un redémarrage du conteneur à cet instant précis peut perdre l'envoi. La demande reste tracée dans le journal d'audit (\texttt{auth.password\_reset\_requested} sans \texttt{auth.password\_reset\_completed} correspondant), mais l'échec d'envoi lui-même n'apparaît pas dans les logs (le processus s'est arrêté avant de pouvoir l'écrire — un échec SMTP \emph{ordinaire}, lui, est toujours loggé). Il suffit de refaire une demande.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Tra\c{c}abilit\'e} : chaque envoi est journalis\'e dans
          le journal d'audit (\texttt{invoice.emailed}, avec destinataire et objet).
    \item \textbf{Renvoi} : autoris\'e (chaque renvoi \'ecrase la date
          d'envoi affich\'ee et laisse sa propre entrée au journal d'audit).
\end{itemize}
...
\subsubsection{Importer (restaurer la même installation)}
...
\textbf{Cette fonction sert à restaurer \emph{votre} installation} --- la remettre dans un état antérieur, ou la déplacer telle quelle vers un autre serveur. Ce n'est \textbf{pas} un outil pour verser les données d'une \emph{autre} installation dans la vôtre~: c'est techniquement possible, et les conséquences sur le journal d'audit sont décrites plus bas (section \emph{Conformité OLICo}).

\begin{keshwarning}
\textbf{Opération destructrice et irréversible.} L'import \textbf{remplace l'intégralité des données} de l'installation actuelle par celles du fichier. À la fin, vous êtes \textbf{déconnecté} et devez vous reconnecter avec les \textbf{identifiants tels qu'ils étaient au moment de la sauvegarde} --- tout compte créé, renommé ou dont le mot de passe a changé \emph{depuis} cette sauvegarde revient à son état d'alors, et les comptes créés depuis n'existent plus. Avant toute suppression, Kesh crée \textbf{automatiquement une sauvegarde de l'état courant} côté serveur (répertoire \keshcommand{KESH\_ADMIN\_BACKUP\_DIR}, défaut \keshpath{/tmp}) en filet de sécurité ; conservez ce répertoire à l'abri et purgez-le selon votre politique.
\end{keshwarning}
...
\begin{keshwarning}
Si vous avez restauré une sauvegarde \textbf{avant} la version qui introduit ce rejeu, réimporter \textbf{cette même sauvegarde} appliquera cette fois la reprise --- mais \textbf{pesez-en le coût} : comme tout import, l'opération \textbf{remplace l'intégralité de l'installation} par le contenu du fichier. \textbf{Tout ce que vous avez saisi depuis ce restore serait perdu} (factures, écritures, contacts, paiements\ldots). Ne relancez cet import que si \textbf{rien n'a été saisi entre-temps}. Dans le cas contraire, corrigez directement ce qui manque : les \textbf{rôles} depuis le plan comptable, le \textbf{compte de produit} d'une ligne depuis la facture concernée.

Il n'existe pas de commande qui rejouerait les reprises \textbf{sans} restaurer : le rejeu est une étape de l'import, pas une opération autonome.
\end{keshwarning}
...
\begin{itemize}
    ...
    \item \textbf{Administration} : plus largement, \textbf{aucune route réservée au rôle Administrateur n'est atteignable avec une clé API}, quel que soit le rôle de son créateur --- gestion des utilisateurs et des mots de passe, paramètres de facturation et de relance, taux de TVA, niveaux de rappel, modèles d'e-mail, coordonnées de l'entreprise, suppression de facture, export et import d'installation, réouverture d'exercice. La réponse est \texttt{403 API\_KEY\_ADMIN\_FORBIDDEN}. Ces opérations se font dans l'interface web. \textbf{Attention à la version} : cette fermeture n'est pas dans la v0.9.0 --- voir l'encadré \emph{« Dans quelle version ? »} plus bas.
    \item \textbf{Journal d'audit} : la \textbf{consultation du journal} --- la liste, le vocabulaire et l'export CSV (\texttt{/api/v1/audit-log*}) --- est elle aussi \textbf{fermée aux clés API}, y compris en lecture seule et y compris pour une clé créée par un Administrateur. ⚠️ \emph{La réponse porte ici un \textbf{autre} code~: \texttt{403 API\_KEY\_MANAGEMENT\_FORBIDDEN}, et non \texttt{API\_KEY\_ADMIN\_FORBIDDEN} --- le journal n'est pas une route d'administration. Le libellé parle de «~gestion de clés~» pour des raisons historiques~; sur ces trois routes, il signifie simplement qu'une clé n'y a pas accès. Ne cherchez pas le défaut dans la configuration de la clé.} Le journal d'audit se lit dans l'interface web, par un Comptable ou un Administrateur.
    \item ⚠️ \textbf{Ce qu'une clé \texttt{read-write} peut faire depuis la v0.12.1, et qu'elle ne pouvait pas avant} : \textbf{dévalider une facture}, donc \textbf{supprimer l'écriture comptable} que sa validation avait créée. La route n'est pas réservée à l'administrateur --- elle est ouverte au Comptable, et donc aux clés --- par un choix délibéré, aligné sur les API de facturation comparables. Les huit refus de la dévalidation s'y appliquent intégralement (exercice clos, verrou de période, facture réglée, créditée, relancée, envoyée, rapprochée, contre-passée), et chaque dévalidation est journalisée sous son propre nom. Si cette capacité vous paraît trop large pour une intégration, donnez-lui une clé en \emph{lecture seule}.
    \item \textbf{Portée} : chaque clé est en \emph{lecture seule} (\texttt{read} --- méthodes GET/HEAD/OPTIONS) ou \emph{lecture-écriture} (\texttt{read-write} --- toutes les méthodes). La permission effective est l'intersection de la portée de la clé et du rôle de son créateur --- \textbf{et l'administration en est exclue dans tous les cas} (point \emph{Administration} ci-dessus), \textbf{de même que le journal d'audit} (point précédent).
    ...
    \item \textbf{Journal d'audit} : les mutations effectuées via une clé sont tracées dans \texttt{audit\_log}. ⚠️ \emph{L'attribution n'est pas encore complète~: une quinzaine de dépôts de données inscrivent \texttt{actor\_type = 'user'} au lieu de \texttt{'api\_key'}, si bien qu'une action faite par une intégration peut être journalisée comme venant d'une personne. L'imputabilité humaine est préservée --- c'est le créateur de la clé qui figure --- mais l'information «~c'était une intégration~» se perd. Correction suivie à l'issue \#431.}
\end{itemize}

\begin{keshwarning}
\textbf{Moindre privilège.} Une clé créée par un \textbf{Administrateur} \textbf{n'hérite pas} de ses pouvoirs d'administration : celle-ci est fermée aux clés API sans exception, de sorte que \textbf{révoquer une clé compromise suffit à arrêter l'incident}. Appliquez néanmoins le moindre privilège à ce que la clé peut faire : privilégiez la portée \texttt{read} lorsque la lecture suffit, et un créateur Comptable quand l'intégration n'a pas besoin d'écrire davantage. La v0.2 n'applique \textbf{pas} de limitation de débit (\emph{rate-limiting}) par clé : utilisez l'expiration et la révocation pour limiter l'exposition.

\medskip
\begin{keshwarning}
\textbf{Dans quelle version ? Pas dans celle que décrit ce manuel.} Jusqu'à la \textbf{v0.9.0 incluse}, une clé \texttt{read-write} créée par un Administrateur atteint bien les routes d'administration --- elle peut donc créer un compte administrateur, ce qui rend la révocation de la clé \textbf{inopérante}. Le correctif décrit au point \textbf{Administration} ci-dessus est livré à la \textbf{prochaine version publiée} (section \texttt{[Unreleased]} du \keshcommand{CHANGELOG.md}). \textbf{Si vous exploitez la v0.9.0, traitez une clé d'origine Administrateur comme un secret d'administrateur} : ne l'utilisez que si l'intégration en a réellement besoin, et préférez un créateur Comptable. Suivi GitHub \texttt{KF-036 / \#167}.
\end{keshwarning}
\end{keshwarning}
...
Le contrôle d'accès basé sur les rôles est appliqué \textbf{au niveau de chaque endpoint API} via un middleware Tower (Axum). Les trois rôles standards (cf. section Premier démarrage) sont définis dans le code (pas de rôles custom dynamiques v0.1).

\subsection{Journal d'audit (audit\_log)}

La quasi-totalité des actions métier mutantes est enregistrée dans la table \texttt{audit\_log} --- 87 des 105 routes à verbe mutant au moment de la rédaction. ⚠️ \emph{Les exceptions sont connues et suivies~: la séquence d'installation (issue \#434) et les gestes de session --- connexion, déconnexion, changement de son propre mot de passe (issue \#435). Trois autres routes n'écrivent rien parce qu'elles ne modifient rien.} La liste complète, route par route, est tenue dans le dépôt et contrôlée par un test~:

\begin{itemize}
    \item Contrainte \textbf{insert-only} : aucune route API ne permet de modifier ou supprimer une entrée \texttt{audit\_log}, et \textbf{aucune ne permet d'effacer la table} sur une installation finalisée. \emph{La formulation antérieure ne parlait que d'«~une entrée~»~: elle était exacte au mot près et trompeuse en substance, une route effaçant alors la table entière.}
    \item Champs : \texttt{id}, \texttt{user\_id}, \texttt{actor\_label} (le nom de l'auteur \emph{au moment} de l'écriture), \texttt{actor\_type} et \texttt{actor\_api\_key\_id}, \texttt{company\_id} (la société de l'auteur au moment de l'écriture, vide quand elle est indéterminable), \texttt{action}, \texttt{entity\_type}, \texttt{entity\_id}, \texttt{details\_json}, \texttt{created\_at}. ⚠️ \emph{Chaque entrée porte la société de son auteur. Le journal se \textbf{consulte} désormais depuis l'API~: une liste paginée et filtrable, un vocabulaire des codes traduits, et un export CSV --- réservés aux rôles \textbf{Comptable} et \textbf{Administrateur}, et refusés aux clés API. L'\textbf{écran} existe aussi~: \emph{Administration} → \emph{Journal d'audit}, pour les mêmes rôles. La consultation ne filtre que votre société, et ne s'inscrit pas elle-même au journal.}
...
\begin{itemize}
    \item \textbf{Un verrou de période} arrête les écritures à une date choisie, indépendamment de la clôture annuelle~: aucune écriture ne peut plus être datée d'une période déjà déclarée (typiquement un trimestre TVA déposé), par aucun chemin --- ni la saisie manuelle, ni les flux automatiques --- et aucune écriture de la période ne peut plus en être \textbf{supprimée}, pas même en \textbf{dévalidant} la facture qui la porte. Le \textbf{déverrouillage} est réservé à un administrateur, exige un \textbf{motif} et s'inscrit au journal d'audit ; une restauration de sauvegarde qui ferait reculer la borne y laisse également une trace (\texttt{books.restored}).
    \item \textbf{Une écriture enregistrée n'est plus ni modifiable ni supprimable} --- l'application refuse les deux quel que soit le rôle, sur la route des écritures ; la correction passe par la \textbf{contre-passation}, qui laisse l'origine et sa correction dans les livres (CO Art.~958f : l'exigence n'est pas qu'on ne se trompe jamais, c'est que la correction soit apparente). L'immutabilité n'est donc plus seulement \emph{tracée}, elle est \textbf{imposée}.

    ⚠️ \textbf{En usage courant, une seule voie fait encore disparaître une écriture, et elle est nommée}~: la \textbf{dévalidation} d'une facture (depuis la v0.12.1), qui la repasse en brouillon et supprime l'écriture que sa validation avait créée. Ce n'est pas une brèche dans l'immutabilité mais une \textbf{décision de facturation}~: elle est journalisée sous son propre nom, refusée sous huit motifs --- dont l'exercice clos, le verrou de période, la contre-passation et l'envoi au client ---, et elle ne touche jamais une écriture saisie à la main. \emph{Hors usage courant, la restauration d'une sauvegarde remet les livres dans l'état de l'archive (\S\ref{sec:backup-ui-keshbackup}).}
    \item \texttt{audit\_log} alimenté à chaque création d'écriture, avec instantané (condition 1). \emph{Il conserve aussi les modifications et suppressions antérieures au gel, les dévalidations de factures, postérieures (\texttt{invoice.unvalidated}), ainsi que les opérations des autres entités.}
    \item \texttt{SHA-256} par fichier dans le \texttt{metadata.json} du ZIP d'export (condition 1 renforcée).
    \item Horodatage \texttt{created\_at} pour le moment d'enregistrement (condition 2).
    \item Documentation des procédures dans ce manuel et dans le journal de bord (conditions 3 et 4).
\end{itemize}

\begin{keshwarning}[title=\faExclamationTriangle\ Deux réserves à connaître avant d'invoquer cette conformité]
\textbf{Le journal d'audit est inaltérable sur une installation en service}, et il faut
savoir à quoi cela tient. L'\textbf{import d'une sauvegarde} ne remplace plus la table~: les
entrées de l'archive sont \textbf{fusionnées} avec celles de l'installation, et chaque entrée
porte le nom de son auteur au moment de l'écriture --- il survit donc au remplacement des
comptes utilisateurs. \faExclamationTriangle\ \textbf{Cette fusion suppose que la sauvegarde
soit celle de votre propre installation.} Importer celle d'une \emph{autre} installation est techniquement
possible, et les entrées du journal d'audit \textbf{déjà présentes chez vous} se retrouveraient alors
rattachées à la société importée~: le journal d'audit mêlerait deux comptabilités sans
qu'aucun message ne le signale. N'importez que vos propres sauvegardes. La
\textbf{réinitialisation des données de démonstration} efface encore
la table, mais elle exige le rôle d'administrateur \emph{et} est refusée dès que
l'installation est finalisée. \emph{Aucun verrou de base de données ne s'y oppose~: la
garantie est portée par l'application, non par le SGBD.} Reste, en mode test uniquement
(\texttt{KESH\_TEST\_MODE}, jamais actif en déploiement réel), les points d'entrée de
réinitialisation de la suite de tests automatisés.

\medskip
\noindent \textbf{L'empreinte SHA-256 est calculée et conservée par vous.} Sans horodatage ni
tiers indépendant, qui modifie les données recalcule l'empreinte. C'est une \textbf{bonne
pratique d'archivage} --- réelle, et supérieure à beaucoup de pratiques courantes --- mais
elle ne constitue pas à elle seule la garantie d'intégrité qu'un contrôle attend d'un support
modifiable. Pour des livres devant résister à une contestation, complétez par un archivage
tiers ou un horodatage qualifié.
\end{keshwarning}
...
\begin{itemize}
    \item al. 1 : enregistrement intégral, fidèle et systématique → la saisie en partie double est contrôlée à trois niveaux (validation métier, contrainte de base, recalcul après écriture) ; une écriture enregistrée est \textbf{définitive} et se corrige par contre-passation ; et l'\texttt{audit\_log} conserve l'historique — sous les deux réserves de la section \emph{Conformité OLICo Art. 9}.
...
\begin{itemize}
    ...
    \item al. 3 : support libre + lien garanti + lisibilité possible → garanti par \texttt{audit\_log} + SHA-256 + format CSV ouvert du ZIP d'export. \textbf{Depuis la v0.12.1, le journal d'audit VOYAGE avec l'export} (\texttt{audit\_log.csv}, issue \#386)~: il n'est plus seulement conservé dans l'instance, elle part avec les livres.
\end{itemize}
...
\begin{keshwarning}
\textbf{L'historique des rappels est protégé par l'application, et non plus seulement par votre vigilance.} Une facture qui a fait l'objet de relances ne peut plus être \textbf{dévalidée} --- la dévalidation supprimerait son écriture, et la suppression du brouillon qui s'ensuivrait effacerait le texte réclamé, les frais et le destinataire. Kesh refuse, en nommant ce motif (garde livrée par l'issue \#260, puis reprise par la dévalidation).

Pour annuler une facture déjà relancée, la voie reste l'\textbf{avoir}~: l'historique demeure complet, et c'est ce que la valeur probatoire exige.
\end{keshwarning}

\subsection{Ordonnance OLICo (RS 221.431)}

\begin{itemize}
    \item Art. 1 : journal chronologique → implémenté (rapport \emph{Journaux}). ⚠️ \textbf{Le grand livre --- l'extrait d'un compte donné --- n'existe pas encore} : aucun rapport ne le produit, et la liste des écritures ne se filtre pas par compte. C'est la première pièce que demande une révision ; elle doit aujourd'hui être reconstituée depuis l'export CSV.
    \item Art. 3 : intégrité (modification apparente) → les corrections passent par \textbf{contre-passation}, qui laisse l'écriture d'origine en place, et le journal d'audit en conserve la trace. ⚠️ \emph{La couverture du journal n'est pas encore complète, mais la réserve a changé d'objet~: la gestion des utilisateurs, la modification de la société et les pièces importées y sont désormais inscrites. Ce qui manque est la séquence d'installation (issue \#434) et les gestes de session (issue \#435).}
...
\begin{description}
    ...
    \item[Journal d'audit (angl. audit log)] Trace des actions métier (table \texttt{audit\_log}), inaltérable sur une installation en service. ⚠️ \emph{La couverture n'est pas totale~: voir les réserves de la section «~Conformité~».}
