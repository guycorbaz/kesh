...
Le \emph{journal d'audit} enregistre les modifications comptables --- qui, quand, quelle valeur --- et aucune entrée n'y est modifiable~; le \emph{hash SHA-256} accompagne tous les exports. Une recherche réglementaire dédiée (605 lignes documentées) accompagne le logiciel.
\end{tcolorbox}
...
Export ZIP en format ouvert (CSV + PDF + manifeste SHA-256) téléchargeable à tout moment, et sauvegarde complète de l'installation au format \texttt{.keshbackup}. \emph{L'export CSV couvre aujourd'hui les ventes, la banque et le grand livre ; les achats, les avoirs et les projets analytiques y seront ajoutés.}
\end{tcolorbox}
...
\sffamily
Le modèle de données est \textbf{multi-société} : chaque enregistrement porte son \texttt{company\_id}, et l'isolation est vérifiée en profondeur (défense contre les accès croisés). \emph{L'interface ne propose pas encore de créer ni de basculer entre plusieurs sociétés : un dossier = une instance ou un compte dédié.}

Une piste pour les \textbf{fiduciaires} qui souhaitent offrir Kesh à leurs clients. \emph{La vue d'ensemble sur un portefeuille de dossiers reste à construire ; aujourd'hui, chaque dossier se gère séparément.}
\end{tcolorbox}
...
\begin{itemize}
    \item Comptabilité en \textbf{partie double}, avec un plan comptable adapté à votre forme juridique.
...
Vous dirigez une PME suisse ? Kesh vous accompagne dans :
...
\begin{itemize}
    ...
    \item Bilan et compte de résultat, exportables en PDF et CSV.
    \item Multi-utilisateurs avec RBAC : Administrateur (toutes opérations), Comptable (saisie et consultation), Consultation (lecture seule).
\end{itemize}

\medskip
\noindent\textbf{Ce que Kesh ne fait pas, et qui se tient ailleurs.} La \textbf{gestion des
salaires} --- décomptes AVS/AC/LPP/LAA, certificats de salaire, impôt à la source --- relève
d'une application dédiée ; Kesh reçoit l'écriture récapitulative. Et la \textbf{mise en forme
légale des comptes annuels} --- structures des art.~959a et 959b CO, annexe de l'art.~959c ---
reste à produire avec votre fiduciaire : Kesh fournit les chiffres, pas la présentation
normée.
...
\begin{itemize}
    \item Architecture multi-tenant : isolation par \texttt{company\_id}, porté par le jeton, chaque utilisateur étant rattaché à une société. \emph{(La bascule entre sociétés depuis l'interface reste à venir.)}
    \item Isolation stricte des données par société (conformité protection des données et secret professionnel). \emph{(Un sélecteur permettant de basculer entre dossiers depuis un même compte est prévu pour une version ultérieure.)}
    \item Journal d'audit centralisé pour la responsabilité fiduciaire.
...
\begin{tabularx}{\linewidth}{>{\bfseries\color{keshPrimary}}lX}
    ...
    \midrule
    Backend & Rust 1.96+ (édition 2024) avec le framework Axum \\
    Frontend & SvelteKit 2 + Svelte 5, TypeScript, Tailwind CSS 4 \\
    Base de données & MariaDB 10.11+ (compatible $\geq$ 10.6) \\
...
\textbf{\color{keshSuccess}\faCheckCircle\ Déjà livré (v0.1 à v0.3.2)}

\begin{itemize}
    \item Fondations et authentification multi-tenant, cookies sécurisés httpOnly.
    \item Onboarding guidé et expert ; récupération de mot de passe oublié par email.
    ...
    \item Clôture d'exercice comptable (blocage des écritures sur période clôturée).
    \item Avoirs (notes de crédit) : contre-passation liée d'une facture validée, avec PDF. \emph{(v0.3.2)}
    \item Factures fournisseurs avec règlement binaire (virement bancaire ou compte interne). \emph{(v0.3.x)}
    \item Paiements sortants : fichier ISO 20022 \texttt{pain.001.001.09} (Swiss Payment Standards / SIX). \emph{(v0.3.x)}
    \item Import de factures fournisseurs depuis un dossier surveillé, avec décodage du QR-facture côté serveur. \emph{(v0.3.x)}
    \item Exports PDF et CSV par rapport + ZIP global de souveraineté avec SHA-256.
    \item Export / import complet d'installation (\texttt{.keshbackup}) pour migration ou restauration sans accès SSH.
    \item API externe à clés d'accès personnelles (PAT) pour intégrations IA et logiciels tiers.
    \item Déploiement \& Opérations : CI/CD release automatisée, images Docker officielles, migrations automatiques, résilience frontend, manuels PDF.
\end{itemize}

\vspace{0.4cm}

\textbf{\color{keshSuccess}\faCheckCircle\ TVA Suisse (Epic 11 \& 18, livré)}

\begin{itemize}
    \item Configuration des taux + calcul et rapport de TVA par période.
    \item Comptabilisation de la TVA due aux ventes + impôt préalable sur les achats.
    \item Décompte TVA (solde net dû à l'AFC), avec contrôle de cohérence entre le décompte et les écritures des factures qu'il retient. \emph{(Format de décompte officiel e-décompte ESTV et taux de la dette fiscale nette à venir ; le rapprochement du décompte avec le solde du compte de TVA reste à la charge de l'utilisateur.)}
\end{itemize}

\vspace{0.4cm}

\textbf{\color{keshWarning}\faClock\ Backlog (Epic 13 à 15)}

\begin{itemize}
    \item Report automatique des soldes de bilan à la clôture d'exercice.
...
\textbf{\color{keshPrimaryLight}\faRocket\ Vision (v0.4+)}

\begin{itemize}
    \item Signature électronique qualifiée (QES) sur les exports.
    \item Décompte TVA officiel AFC (e-décompte ESTV).
