...
\textbf{\color{keshSuccess}\faCheckCircle\ Déjà livré (v0.2.0)}
...
\begin{itemize}
    ...
    \item Clôture d'exercice comptable avec report automatique des soldes de bilan.
    \item Exports PDF et CSV par rapport + ZIP global de souveraineté avec SHA-256.
    \item Export/import complet d'installation (\texttt{.keshbackup}) depuis l'interface d'administration.
    \item API externe avec clés d'accès personnelles (PAT) pour les intégrations tierces.
...
\textbf{\color{keshSuccess}\faCheckCircle\ Déploiement \& Souveraineté (Epic 10 \& 17, livré)}
...
\begin{itemize}
    ...
    \item API externe à clés d'accès (PAT) pour intégrations IA et logiciels tiers.
    \item Export / import complet d'installation (\texttt{.keshbackup}) pour migration ou restauration sans accès SSH.
    \item Récupération de mot de passe par email (self-service).
\end{itemize}
...
\textbf{\color{keshWarning}\faClock\ Backlog (Epic 12 à 15)}
...
\textbf{\color{keshPrimaryLight}\faRocket\ Vision (v0.3+)}
