...
| **projet parent** | **übergeordnetes Projekt** | **progetto superiore** | **parent project** | `account-field-parent` — « Compte parent / Übergeordnetes Konto / Conto superiore ». ⚠️ **L'idiome n'est PAS un calque** : jamais *Eltern-Konto*. ⚠️ **Le catalogue porte DEUX formes en italien** (`conto superiore` et `conto padre`) ; `superiore` est retenu, la divergence est relevée. ⚠️ **`projet racine` (`Stammprojekt` / `progetto radice`) n'est attesté NULLE PART** — il relève de la partie B, pas d'ici |
| **clôture (d'exercice)** | **Abschluss** | **chiusura** | **closing** | ⚠️ **POSÉ par la 23-4, sans antécédent au catalogue** — la première rédaction citait `closing`, c'est-à-dire la clé qu'elle écrivait elle-même. ⚠️⚠️ **NE PAS confondre avec « fermer »** (`Schliessen` / `Chiudi` / `Close`) — c'est **[KF-041 (#323)](https://github.com/guycorbaz/kesh/issues/323)**, et `fiscal-year-close-button` porte encore la confusion |
| **modèle (d'e-mail)** | **Vorlage** | **modello** | **template** | `reminders-error-content-empty` — « Modèle de rappel vide / Mahnvorlage leer / Modello di sollecito vuoto ». ⚠️ **La première rédaction citait `email-templates-title`, une clé que cette story CRÉE** : une clé qu'on écrit n'est pas un précédent |
